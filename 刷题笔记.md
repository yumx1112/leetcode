...
public:
    int trap(vector<int>& heights) {
        int n = heights.size();
        vector<int> leftMaxHeights(n), rightMaxHeights(n);
        ...
        int res = 0;
        for (int i = 1; i < n - 2; ++i) { // i 位置上有多少水
            res += max(min(leftMaxHeights[i - 1], rightMaxHeights[i + 1]) - heights[i], 0);
        }
        return res;
    }
};
...
public:
    int trap(vector<int>& heights) {
        int n = heights.size(), l = 1, r = n - 2, leftMax = heights[0], rightMax = heights[n - 1], res = 0;
        while (l <= r) {
            ...
            if (leftMax <= rightMax) {
                res += max(leftMax - heights[l], 0);
                leftMax = max(leftMax, heights[l++]);
            } else {
                res += max(rightMax - heights[r], 0);
...
public:
    int lengthOfLongestSubstring(string str) {
        int n = str.size(), ht[256], res = 0;
        fill_n(ht, 256, -1);
        for (int i = 0, j = 0; j < n; ++j) { // 以 j 位置结尾，最多往前扩到哪里
            i = max(i, ht[str[j]] + 1);
            res = max(res, j - i + 1);
            ht[str[j]] = j;
        }
...
public:
    vector<int> findAnagrams(string s, string p) {
        ...
        vector<int> sCnts(26), pCnts(26), res;
        for (int i = 0; i < np; ++i) {
            ++sCnts[s[i] - 'a'];
            ++pCnts[p[i] - 'a'];
        }
