...
struct Comp {
    ...
    bool operator()(const ListNode* listNode1, const ListNode* listNode2) const {
...
struct BiListNode {
    ...
    BiListNode() : key(-1), value(-1), prev(nullptr), next(nullptr) {
    }

    BiListNode(int key, int value) : key(key), value(value), prev(nullptr), next(nullptr) {
    }
};
...
public:
    BiLinkedList() {
        dummyHead = new BiListNode();
        dummyTail = new BiListNode();
        dummyHead->next = dummyTail;
        dummyTail->prev = dummyHead;
    }

    void addToTail(BiListNode* node) {
    ...
    int removeHead() {
        BiListNode* head = dummyHead->next;
        skipNode(head);
        int res = head->key;
        delete head;
        return res;
    ...
    void moveToTail(BiListNode* node) {
...
public:
    ...
    int get(int key) {
        ...
        BiListNode* node = ht[key];
        biLinkedList.moveToTail(node);
        return node->value;
    }
    ...
    void put(int key, int value) {
        if (!ht.count(key)) {
            if (ht.size() == capacity) {
                ht.erase(biLinkedList.removeHead());
            }
            ht[key] = new BiListNode(key, value);
            biLinkedList.addToTail(ht[key]);
        } else {
            ht[key]->value = value;
            biLinkedList.moveToTail(ht[key]);
        }
    }
};
```
