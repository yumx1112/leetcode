    ...
    int search(vector<int>& nums, int target) {
        ...
        while (left <= right) {
            ...
            if (nums[middle] == target) {
                return middle;
            } else if (nums[middle] >= nums[0] && target >= nums[0] || nums[middle] <= nums[n - 1] && target <= nums[n - 1]) {  // target (if present) and nums[middle] are on the same side.
                if (nums[middle] < target) {
                    left = middle + 1;
                } else {
                    right = middle - 1;
                }
            } else {   // target (if present) and nums[middle] are on the opposite side.
                if (nums[middle] < target) {
                    right = middle - 1;
                } else {
                    left = middle + 1;
