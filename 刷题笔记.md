...
public:
    int subarraySum(vector<int>& nums, int k) {
        ...
        unordered_map<int, int> ht; // kv 分别是 [0,i) 上前缀和及其出现的次数
        ht[0] = 1;
        for (int i = 0; i < n; ++i) {
            sum += nums[i];
            if (ht.count(sum - k)) {
...
public:
    string minWindow(string str, string tar) {
        ...
        int strLen = str.size(), debt = tar.size(), start, minLen = INT_MAX;
        for (int i = 0, j = 0; j < strLen; ++j) { // 以 j 位置结尾最短往左边扩多少
            if (cnts[str[j]]++ < 0) {
                --debt;
            }
            // 如果 debt>0，则这时以 j 位置结尾没有答案
            if (!debt) {
                while (cnts[str[i]] > 0) { // 不欠债的前提下，往右移动左边界，找以 j 位置结尾时最短的子串
                    --cnts[str[i++]];
                }
                if (j - i + 1 < minLen) {
                    start = i;
