# 模拟

## 题目

### 字符串相加

https://leetcode.cn/problems/add-strings

```c++
class Solution {
public:
    string addStrings(string num1, string num2) {
        int i1 = num1.size() - 1;
        int i2 = num2.size() - 1;
        int carry = 0;
        string res;
        while (i1 >= 0 || i2 >= 0 || carry) {
            int val = carry;
            if (i1 >= 0) {
                val += num1[i1--] - '0';
            }
            if (i2 >= 0) {
                val += num2[i2--] - '0';
            }
            res.push_back((char) (48 + val % 10));
            carry = val / 10;
        }
        reverse(res.begin(), res.end());
        return res;
    }
};
```

时间复杂度：$O\left ( \max \left ( M,N \right )  \right ) $

空间复杂度：$O\left (1\right ) $

### 汉明距离

https://leetcode.cn/problems/hamming-distance

模拟 1：

```c++
class Solution {
public:
    int hammingDistance(int x, int y) {
        int xor_ = x ^ y;
        int res = 0;
        while (xor_) {
            res += xor_ & 1;
            xor_ >>= 1;
        }
        return res;
    }
};
```

时间复杂度：$O\left(1\right)$

空间复杂度：$O\left(1\right)$

------

模拟 2（内置函数）：

```c++
class Solution {
public:
    int hammingDistance(int x, int y) {
        return __builtin_popcount(x ^ y);
    }
};
```

时间复杂度：$O\left(1\right)$

空间复杂度：$O\left(1\right)$

------

模拟 3（Brian Kernighan 算法）：

进行 `xor_ &= xor_ - 1` 这个操作之后，`xor_` 最右侧的 1 变成了 0。

```c++
class Solution {
public:
    int hammingDistance(int x, int y) {
        int xor_ = x ^ y;
        int res = 0;
        while (xor_) {
            xor_ &= xor_ - 1;
            ++res;
        }
        return res;
    }
};
```

时间复杂度：$O\left(1\right)$

空间复杂度：$O\left(1\right)$

### Rotate Array

https://leetcode.cn/problems/rotate-array

**Reverse.**

```c++
class Solution {
public:
    void rotate(vector<int>& nums, int k) {
        reverse(nums.begin(), nums.end());
        int n = nums.size();
        k %= n;
        reverse(nums.begin(), nums.begin() + k);
        reverse(nums.begin() + k, nums.end());
    }
};
```

Time Complexity: $O\left(n\right)$, where $n$ is the length of the array.

Space Complexity: $O\left(1\right)$.

### 整数反转

https://leetcode.cn/problems/reverse-integer

题解：https://leetcode.cn/problems/reverse-integer/solutions/211865/tu-jie-7-zheng-shu-fan-zhuan-by-wang_ni_ma

下图中，绿色的是最大 32 位整数。第二排数字中，橘子的是 5，它是大于上面同位置的 4，这就意味着 5 后跟任何数字，都会比最大 32 位整数都大。所以，我们到【最大数的 1/10】时，就要开始判断了。如果某个数字大于 214748364 那后面就不用再判断了，肯定溢出了。如果某个数字等于 214748364 ，这对应到上图中第三、第四、第五排的数字，需要要跟最大数的末尾数字比较，如果这个数字比 7 还大，说明溢出了。负数同理。

![42c736510f4914af169907d61b22d1a39bd5a16bbd7eca0466d90350e2763164-2](./刷题笔记.assets/42c736510f4914af169907d61b22d1a39bd5a16bbd7eca0466d90350e2763164-2.jpg)

```c++
class Solution {
public:
    int reverse(int x) {
        const int NEGATIVE_COMP = INT_MIN / 10;
        const int POSITIVE_COMP = INT_MAX / 10;
        const int NEGATIVE_LAST_BIT = -INT_MIN % 10;
        const int POSITIVE_LAST_BIT = INT_MAX % 10;
        int res = 0;
        while (x != 0) {
            int lastBit = x % 10;
            if ((res > POSITIVE_COMP ||
                 res == POSITIVE_COMP && lastBit > POSITIVE_LAST_BIT) ||
                (res < NEGATIVE_COMP ||
                 res == NEGATIVE_COMP && lastBit < NEGATIVE_LAST_BIT)) {
                return 0;
            }
            res = 10 * res + lastBit;
            x /= 10;
        }
        return res;
    }
};
```

时间复杂度：$O\left(1\right)$

空间复杂度：$O\left(1\right)$

### 字符串转换整数 (atoi)

https://leetcode.cn/problems/string-to-integer-atoi

自动机：

![fig1](./刷题笔记.assets/fig1.png)

![image-20240724142336694](./刷题笔记.assets/image-20240724142336694.png)

```c++
class FSM {
private:
    string state = "start";
    unordered_map<string, vector<string>> table = {
        {"start", {"start", "signed", "in_number", "end"}},
        {"signed", {"end", "end", "in_number", "end"}},
        {"in_number", {"end", "end", "in_number", "end"}},
        {"end", {"end", "end", "end", "end"}}
    };
    int sign = 1;
    long long val = 0; // >= 0
public:
    int getCol(char c) {
        if (c == ' ') {
            return 0;
        } else if (c == '+' || c == '-') {
            return 1;
        } else if (c >= '0' && c <= '9') {
            return 2;
        } else {
            return 3;
        }
    }

    void step(char c) {
        state = table[state][getCol(c)];
        if (state == "in_number") {
            val = 10 * val + c - '0';
            val = sign == 1
                      ? min(val, (long long) INT_MAX)
                      : min(val, -(long long) INT_MIN);
        } else if (state == "signed") {
            sign = c == '+' ? 1 : -1;
        }
    }

    int getRes() {
        return sign * val;
    }
};

class Solution {
public:
    int myAtoi(string s) {
        FSM fsm;
        for (char& c: s) {
            fsm.step(c);
        }
        return fsm.getRes();
    }
};
```

时间复杂度：$O\left(n\right)$，$n$ 是字符串的长度

空间复杂度：$O\left(1\right)$

# 双指针

## 题目

### 删除有序数组中的重复项 II

https://leetcode.cn/problems/remove-duplicates-from-sorted-array-ii

```c++
class Solution {
public:
    int removeDuplicates(vector<int>& nums, int k = 2) {
        int n = nums.size();
        if (n < k + 1) {
            return n;
        }
        int i = k;
        for (int j = k; j < n; ++j) {
            if (nums[j] != nums[i - k]) {
                nums[i++] = nums[j];
            }
        }
        return i;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

# 栈

## 题目

# 哈希表

## 题目

### 寻找文件副本

https://leetcode.cn/problems/shu-zu-zhong-zhong-fu-de-shu-zi-lcof

暴力解法是遍历数组时用 `unordered_set` 记录下存在哪些元素，这种解法的空间复杂度为 $O\left(n\right)$。但是注意 `0 <= documents[i] <= n-1`，所有可以使用题目中给出的原数组代替 `unordered_set`，如果 `i == documents[i]`，则代表 `documents[i]` 这个数字是存在的。

```c++
class Solution {
public:
    int findRepeatDocument(vector<int>& documents) {
        int n = documents.size(), i = 0;
        while (i < n) {
            if (i == documents[i]) {
                ++i;
                continue;
            }
            if (documents[i] == documents[documents[i]]) { // 这里的 documents[documents[i]] 等价于在哈希表中查找
                return documents[i];
            }
            swap(documents[i], documents[documents[i]]);
        }
        return -1;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

# 二分查找

## 题目

# 链表

## 知识点

感觉链表的题主要考的就是编码能力，多注意边界条件，这类题都不会很难。

## 题目

### 删除链表中的节点

https://leetcode.cn/problems/delete-node-in-a-linked-list，注意这题没有头结点。

```c++
class Solution {
public:
    void deleteNode(ListNode* node) {
        node->val = node->next->val;
        node->next = node->next->next;
    }
};
```

时间复杂度：$O\left(1\right)$

空间复杂度：$O\left(1\right)$

## 删除排序链表中的重复元素

https://leetcode.cn/problems/remove-duplicates-from-sorted-list

```c++
class Solution {
public:
    ListNode* deleteDuplicates(ListNode* head) {
        if (!head) {
            return NULL;
        }

        ListNode* p = head;
        while (p->next) {
            if (p->val == p->next->val) {
                p->next = p->next->next;
            } else {
                p = p->next;
            }
        }
        return head;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 删除排序链表中的重复元素 II

https://leetcode.cn/problems/remove-duplicates-from-sorted-list-ii

```c++
class Solution {
public:
    ListNode* deleteDuplicates(ListNode* head) {
        ListNode* dummyHead = new ListNode(0, head);
        ListNode* p = dummyHead;
        while (p->next && p->next->next) {
            if (p->next->val == p->next->next->val) {
                int val = p->next->val;
                while (p->next && p->next->val == val) {
                    p->next = p->next->next;
                }
            } else {
                p = p->next;
            }
        }
        return dummyHead->next;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

# 树

## 题目

### 将二叉搜索树变平衡

https://leetcode.cn/problems/balance-a-binary-search-tree

树形动态规划。

```c++
#define ll long long

struct Info {
    ll minVal;
    ll maxVal;
    bool isBST;

    Info(ll minVal, ll maxVal, bool isBST) : minVal(minVal), maxVal(maxVal), isBST(isBST) {}
};

class Solution {
public:
    bool isValidBST(TreeNode* root) {
        return f(root).isBST;
    }

    Info f(TreeNode* root) {
        if (root == NULL) {
            return Info(LLONG_MAX, LLONG_MIN, true);
        }
        Info leftInfo = f(root->left);
        Info rightInfo = f(root->right);
        return Info(min((ll)root->val, min(leftInfo.minVal, rightInfo.minVal)), max((ll)root->val, max(leftInfo.maxVal, rightInfo.maxVal)), leftInfo.isBST && rightInfo.isBST && root->val > leftInfo.maxVal && root->val < rightInfo.minVal);
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

中序遍历。**注意下面的关系！**
$$
T\ 是\ \text{BST}\Longleftrightarrow T\ 的中序遍历序列递增
$$

```c++
#define ll long long

class Solution {
    ll prevVal = LLONG_MIN;

public:
    bool isValidBST(TreeNode* root) {
        if (root == NULL) {
            return true;
        }
        if (!isValidBST(root->left) || root->val <= prevVal) {
            return false;
        }
        prevVal = root->val;
        return isValidBST(root->right);
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

### 将有序数组转换为二叉搜索树

https://leetcode.cn/problems/convert-sorted-array-to-binary-search-tree

证明很复杂：https://leetcode.cn/problems/balance-a-binary-search-tree/solutions/241897/jiang-er-cha-sou-suo-shu-bian-ping-heng-by-leetcod

```c++
class Solution {
public:
    TreeNode* sortedArrayToBST(vector<int>& nums) {
        return f(nums, 0, nums.size() - 1);
    }

    TreeNode* f(vector<int>& nums, int l, int r) {
        if (l > r) {
            return nullptr;
        }
        int m = l + ((r - l) >> 1);
        return new TreeNode(nums[m], f(nums, l, m - 1), f(nums, m + 1, r));
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：最坏情况下，$O\left(n\right)$

### 从中序与后序遍历序列构造二叉树

https://leetcode.cn/problems/construct-binary-tree-from-inorder-and-postorder-traversal

```c++
class Solution {
private:
    unordered_map<int, int> valIdxMap;

public:
    TreeNode* buildTree(vector<int>& inorder, vector<int>& postorder) {
        int n = inorder.size();
        for (int i = 0; i < n; ++i) {
            valIdxMap[inorder[i]] = i;
        }
        return buildTree(inorder, postorder, 0, n - 1, 0, n - 1);
    }

    TreeNode* buildTree(vector<int>& inorder, vector<int>& postorder, int p1, int p2, int p3, int p4) {
        if (p1 > p2) {
            return nullptr;
        }
        int r = valIdxMap[postorder[p4]];
        return new TreeNode(postorder[p4],
                            buildTree(inorder, postorder, p1, r - 1, p3, p3 + r - p1 - 1),
                            buildTree(inorder, postorder, r + 1, p2, p3 + r - p1, p2 + p3 - p1 - 1));
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n+h\right)$，其中 $O\left(n\right)$ 为哈希表用的空间，$O\left(h\right)$ 为递归时栈用的空间

# 回溯

## 题目

# Stack

## Problems

# Hash Table

## Problems

### Longest Consecutive Sequence

https://leetcode.com/problems/longest-consecutive-sequence

**Sort.**

```c++
class Solution {
public:
    int longestConsecutive(vector<int>& nums) {
        if (nums.empty()) {
            return 0;
        }

        sort(nums.begin(), nums.end());

        int n = nums.size();
        int lastNum = nums[0];
        int length = 1;
        int res = 1;
        for (int i = 1; i < n; ++i) {
            if (nums[i] == lastNum) {
                continue;
            } else if (nums[i] == lastNum + 1) {
                ++lastNum;
                ++length;
            } else {
                lastNum = nums[i];
                res = max(res, length);
                length = 1;
            }
        }
        res = max(res, length);
        return res;
    }
};
```

Time Complexity: $O\left(n\log n\right)$, where $n$ is the length of the array.

Space Complexity: $O\left(\log n\right)$, because quick sort is used.

**Hash set 1**.

```c++
class Solution {
public:
    int longestConsecutive(vector<int>& nums) {
        if (nums.empty()) {
            return 0;
        }

        unordered_set<int> numSet;
        for (int& num: nums) {
            numSet.emplace(num);
        }

        int n = nums.size();
        int res = 1;
        for (int& num: nums) {
            int lastNum = num;
            int length = 1;
            while (numSet.find(lastNum + 1) != numSet.end()) {
                ++lastNum;
                ++length;
            }
            res = max(res, length);
        }
        return res;
    }
};
```

Time Complexity: $O\left(n^2\right)$, where $n$ is the length of the array.

Space Complexity: $O\left(n\right)$.

**Hash set 2.**

If $x,x+1,\dots,x+y$ is a consecutive sequence, there are no $x-1$ and $x+y+1$ in the array. Then the exploration from $x+1$ to $x+y$ as the starting point is unnecessary.

```c++
class Solution {
public:
    int longestConsecutive(vector<int>& nums) {
        if (nums.empty()) {
            return 0;
        }

        unordered_set<int> numSet;
        for (int& num: nums) {
            numSet.emplace(num);
        }

        int n = nums.size();
        int res = 1;
        for (int& num: nums) {
            if (numSet.find(num - 1) != numSet.end()) {
                continue;
            }

            int lastNum = num;
            int length = 1;
            while (numSet.find(lastNum + 1) != numSet.end()) {
                ++lastNum;
                ++length;
            }
            res = max(res, length);
        }
        return res;
    }
};
```

Time Complexity: $O\left(n\right)$, where $n$ is the length of the array.

Space Complexity: $O\left(n\right)$.

### Subarray Sum Equals K

https://leetcode.com/problems/subarray-sum-equals-k

**Hash table.**

```c++
class Solution {
public:
    int subarraySum(vector<int>& nums, int k) {
        int n = nums.size();
        // `sumCountMap` records the number of same prefix sum on the [0,i-1] interval as written later.
        unordered_map<int, int> sumCountMap{{0, 1}};
        int currSum = 0;
        int res = 0;
        for (int i = 0; i < n; ++i) {
            currSum += nums[i];
            if (sumCountMap.find(currSum - k) != sumCountMap.end()) {
                res += sumCountMap[currSum - k];
            }
            ++sumCountMap[currSum];
        }
        return res;
    }
};
```

Time Complexity: $O\left(n\right)$, where $n$ is the length of the array.

Space Complexity: $O\left(n\right)$.

# 单调队列

## 知识点

除了单调队列最经典的用法之外，在很多问题里单调队列还可以维持求解答案的可能性：

1. 单调队列里的所有对象按照规定好的单调性来组织
2. 当某个对象从队尾进入单调队列时，会从队头或者队尾依次淘汰单调队列里对后续求解答案没有帮助的对象
3. 每个对象一旦从单调队列弹出，可以结算此时这个对象参与的答案，随后这个对象不再参与后续求解答案的过程
4. 其实是先有对题目的分析!进而发现单调性，然后利用单调队列的特征去实现

## 题目

### 设计自助结算系统

https://leetcode.cn/problems/dui-lie-de-zui-da-zhi-lcof

```c++
class Checkout {
private:
    int queue[10000];
    int deque[10000];
    int hQue = 0, tQue = 0, hDeq = 0, tDeq = 0;

public:
    Checkout() {}

    int get_max() {
        return hDeq < tDeq ? queue[deque[hDeq]] : -1;
    }

    void add(int value) {
        queue[tQue++] = value;
        while (hDeq < tDeq && queue[deque[tDeq - 1]] <= value) {
            --tDeq;
        }
        deque[tDeq++] = tQue - 1;
    }

    int remove() {
        if (hQue == tQue) {
            return -1;
        }
        if (deque[hDeq] == hQue) {
            ++hDeq;
        }
        return queue[hQue++];
    }
};
```

时间复杂度：$O\left(1\right)$

空间复杂度：$O\left(n\right)$

### 跳跃游戏 VI

https://leetcode.cn/problems/jump-game-vi

```c++
class Solution {
public:
    int maxResult(vector<int>& nums, int k) {
        int n = nums.size();
        vector<int> dp(n, INT_MIN);  // [0...i] 上的答案为 dp[i]
        dp[0] = nums[0];
        for (int i = 1; i < n; ++i) {
            for (int j = max(0, i - k); j < i; ++j) {  // 求 dp[max(0,i-k)...i-1] 上的最大值，可以使用单调队列优化
                dp[i] = max(dp[i], dp[j] + nums[i]);
            }
        }
        return dp[n - 1];
    }
};
```

时间复杂度：$O\left(nk\right)$

空间复杂度：$O\left(n\right)$

单调队列优化：

```c++
class Solution {
private:
    int deque[100000];
    int h = 0, t = 0;

public:
    int maxResult(vector<int>& nums, int k) {
        int n = nums.size();
        vector<int> dp(n, INT_MIN);
        dp[0] = nums[0];
        for (int i = 1; i < n; ++i) {
            while (h < t && dp[deque[t - 1]] <= dp[i - 1]) {
                --t;
            }
            deque[t++] = i - 1;
            if (i > k) {  // 这里不能写成 t-h>k，因为 t-h 只是单调队列中的元素个数
                if (h < t && deque[h] == i - k - 1) {
                    ++h;
                }
            }
            dp[i] = max(dp[i], dp[deque[h]] + nums[i]);
        }
        return dp[n - 1];
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

### 不间断子数组

https://leetcode.cn/problems/continuous-subarrays

```c++
class Solution {
    int dequeMin[100000] = {}, dequeMax[100000] = {};
    int hMin = 0, tMin = 0, hMax = 0, tMax = 0;

public:
    long long continuousSubarrays(vector<int>& nums) {
        int n = nums.size();
        long long res = 0;
        for (int l = 0, r = 0; r < n; ++r) { // 以 r 位置为右端点，此时有多少个合法的子数组
            while (hMin < tMin && nums[r] <= nums[dequeMin[tMin - 1]]) {
                --tMin;
            }
            dequeMin[tMin++] = r;
            while (hMax < tMax && nums[r] >= nums[dequeMax[tMax - 1]]) {
                --tMax;
            }
            dequeMax[tMax++] = r;
            while (l < r && nums[dequeMax[hMax]] - nums[dequeMin[hMin]] > 2) {
                // 如果 [l,r] 这个子数组不合法，那么 [l,r_bigger] 一定也不合法，所以这时可以 ++l
                if (dequeMin[hMin] == l) {
                    ++hMin;
                }
                if (dequeMax[hMax] == l) {
                    ++hMax;
                }
                ++l;
            }
            res += r - l + 1;
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

### 绝对差不超过限制的最长连续子数组

https://leetcode.cn/problems/longest-continuous-subarray-with-absolute-diff-less-than-or-equal-to-limit

左边界可以往右移一个单位的原因：`l` 位置往右扩最长可以扩到 `r - 1`，`l + 1` 位置往右扩最长至少可以扩到 `r - 1`，因为 `[l + 1, r)` 是 `[l, r)` 的一个子集，既然 `[l, r)` 是合法的那么 `[l + 1, r)` 也一定是合法的，所以这时 `r` 位置继续往右扩即可。

```c++
class Solution {
private:
    int dequeMin[100000], dequeMax[100000];
    int hMin = 0, tMin = 0, hMax = 0, tMax = 0;

public:
    int longestSubarray(vector<int>& nums, int limit) {
        int n = nums.size();
        int res = 0;
        // 计算每个以 l 开头最大合法子数组的长度，记录最大值
        for (int l = 0, r = 0; l < n; ++l) {
            while (r < n && isValid(nums, limit, r)) {
                push(nums, r++);
            }
            res = max(res, r - l);
            pop(nums, l);
        }
        return res;
    }

    // 如果将 nums[i] 加入到子数组中，新的子数组是否合法
    bool isValid(vector<int>& nums, int limit, int i) {
        int currMin = hMin == tMin ? nums[i] : min(nums[dequeMin[hMin]], nums[i]);
        int currMax = hMax == tMax ? nums[i] : max(nums[dequeMax[hMax]], nums[i]);
        return currMax - currMin <= limit;
    }

    void push(vector<int>& nums, int i) {
        while (hMin < tMin && nums[dequeMin[tMin - 1]] >= nums[i]) {
            --tMin;
        }
        dequeMin[tMin++] = i;
        while (hMax < tMax && nums[dequeMax[tMax - 1]] <= nums[i]) {
            --tMax;
        }
        dequeMax[tMax++] = i;
    }

    void pop(vector<int>& nums, int i) {
        if (hMin < tMin && dequeMin[hMin] == i) {
            ++hMin;
        }
        if (hMax < tMax && dequeMax[hMax] == i) {
            ++hMax;
        }
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

### Flowerpot S

https://www.luogu.com.cn/problem/P2698

思路和 https://leetcode.cn/problems/longest-continuous-subarray-with-absolute-diff-less-than-or-equal-to-limit 一样。

```c++
#include <bits/stdc++.h>
using namespace std;

const int MAXN = 100000;
int n, d;
vector<vector<int>> points(MAXN, vector<int>(2));
vector<int> dequeMin(MAXN), dequeMax(MAXN);
int hMin, tMin, hMax, tMax;

bool isValid() {
    return points[dequeMax[hMax]][1] - points[dequeMin[hMin]][1] >= d;
}

void push(int i) {
    while (hMin < tMin && points[dequeMin[tMin - 1]][1] >= points[i][1]) {
        --tMin;
    }
    dequeMin[tMin++] = i;
    while (hMax < tMax && points[dequeMax[tMax - 1]][1] <= points[i][1]) {
        --tMax;
    }
    dequeMax[tMax++] = i;
}

void pop(int i) {
    if (hMin < tMin && dequeMin[hMin] == i) {
        ++hMin;
    }
    if (hMax < tMax && dequeMax[hMax] == i) {
        ++hMax;
    }
}

int getMinLength() {
    sort(points.begin(), points.begin() + n, [](vector<int>& p1, vector<int>& p2) { return p1[0] < p2[0]; });
    hMin = tMin = hMax = tMax = 0;
    int res = INT_MAX;
    for (int l = 0, r = 0; l < n; ++l) {
        while (r < n && !isValid()) {
            push(r++);
        }
        if (isValid()) {
            res = min(res, points[r - 1][0] - points[l][0]);
        }
        pop(l);
    }
    return res == INT_MAX ? -1 : res;
}

int main() {
    while (cin >> n >> d) {
        for (int i = 0; i < n; ++i) {
            cin >> points[i][0] >> points[i][1];
        }
        cout << getMinLength() << endl;
    }
    return 0;
}
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

### 和至少为 K 的最短子数组

https://leetcode.cn/problems/shortest-subarray-with-sum-at-least-k

为方便计算，常用左闭右开区间 `[l, r)` 来表示子数组，此时子数组的和为 `sums[r] - sums[l]`，子数组的长度为 `r - l`。

用单调队列的原因：

1. 尾部（右边）弹出元素：如果 `sums[l] < sums[r]`，假如后续有数字 `x` 能和 `sums[l]` 组成满足要求的子数组，即 `x - sums[l] >= k`，那么必然也有 `x - sums[r] >= k`，由于从 `sums[r]` 到 `x` 的这段子数组更短。因此 `sums[l]` 没有任何作用了，将其弹出。
2. 头部（左边）弹出元素：遍历到 `sums[r]` 时，考虑左边的某个 `sums[l]`，`sums[r] - sums[l] >= k`，那么无论 `sums[r]` 右边的数字是大是小，都不可能把 `sums[l]` 当作子数组的左端点，得到一个比 `r - l` 更短的子数组。因此 `sums[l]` 没有任何作用了，将其弹出。

```c++
class Solution {
public:
    int shortestSubarray(vector<int>& nums, int k) {
        int n = nums.size();
        int deque[100001];
        int h = 0, t = 0;
        int res = INT_MAX;

        vector<long long> sums(n + 1);  // 前 i 个数的累加和
        for (int i = 0; i < n; ++i) {
            sums[i + 1] = sums[i] + nums[i];
        }

        for (int r = 0; r <= n; ++r) {
            while (h < t && sums[r] - sums[deque[h]] >= k) {
                res = min(res, r - deque[h++]);
            }
            while (h < t && sums[deque[t - 1]] >= sums[r]) {
                --t;
            }
            deque[t++] = r;
        }
        return res != INT_MAX ? res : -1;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

总结：

除了单调队列最经典的用法之外，在很多问题里单调队列还可以维持求解答案的可能性。

1. 单调队列里的所有对象按照规定好的单调性来组织
2. 当某个对象从队尾进入单调队列时，会从队头或者队尾依次淘汰单调队列里，对后续求解答案没有帮助的对象
3. 每个对象一旦从单调队列弹出，可以结算此时这个对象参与的答案，随后这个对象不再参与后续求解答案的过程
4. 其实是先有对题目的分析!进而发现单调性，然后利用单调队列的特征去实现

### 满足不等式的最大值

https://leetcode.cn/problems/max-value-of-equation

当前点的贡献为 $y_j+x_j$，前面点的贡献为 $y_i-x_i$。在单调队列中，维持的值是 $y_i−x_i$。使用单调队列的原因如下：

- 从队列头部弹出元素：弹出已过期的元素
- 从队列尾部弹出元素：这些元素与后面的元素结合无法形成更大的结果

```c++
class Solution {
    int deque[100000];
    int h = 0, t = 0;

public:
    int findMaxValueOfEquation(vector<vector<int>>& points, int k) {
        int n = points.size();
        int res = INT_MIN;
        deque[t++] = 0;
        for (int i, j = 1; j < n; ++j) {  // i<j
            while (h < t && points[j][0] - points[deque[h]][0] > k) {
                ++h;
            }
            i = deque[h];
            if (h < t) {
                res = max(res, points[i][1] - points[i][0] + points[j][1] + points[j][0]);
            }
            while (h < t && points[j][1] - points[j][0] >= points[deque[t - 1]][1] - points[deque[t - 1]][0]) {
                --t;
            }
            deque[t++] = j;
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

### 带限制的子序列和

https://leetcode.cn/problems/constrained-subsequence-sum

```c++
class Solution {
    int deque[100000];  // 存放 dp 中的下标
    int h = 0, t = 0;

public:
    int constrainedSubsetSum(vector<int>& nums, int k) {
        int n = nums.size();
        vector<int> dp(n, INT_MIN);  // 子序列必须以 i 位置结尾，答案是 dp[i]
        dp[0] = nums[0];
        deque[t++] = 0;
        int res = dp[0];
        for (int j = 1; j < n; ++j) {
            dp[j] = nums[j];
            while (h < t && deque[h] < j - k) {
                ++h;
            }
            if (dp[deque[h]] > 0) {
                dp[j] += dp[deque[h]];
            }
            while (h < t && dp[deque[t - 1]] <= dp[j]) {
                --t;
            }
            deque[t++] = j;
            res = max(res, dp[j]);
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

### 预算内的最多机器人数目

https://leetcode.cn/problems/maximum-number-of-robots-within-budget

```c++
class Solution {
    int deque[50000] = {}; // 维护 chargeTimes 中窗口内的最大值
    int h = 0, t = 0;

public:
    int maximumRobots(vector<int>& chargeTimes, vector<int>& runningCosts, long long budget) {
        int n = chargeTimes.size();
        int res = 0;
        long long currSum = 0;
        // 双指针
        for (int l = 0, r = 0; r < n; ++r) { // 必须以 i 位置结尾，往左边延伸，最长子数组长度是多少
            while (h < t && chargeTimes[r] >= chargeTimes[deque[t - 1]]) {
                --t;
            }
            deque[t++] = r;
            currSum += (long long) runningCosts[r];
            while (l <= r && (long long) chargeTimes[deque[h]] + (long long) (r - l + 1) * currSum > budget) {
                currSum -= runningCosts[l++];
                if (h < t && deque[h] < l) {
                    ++h;
                }
            }
            res = max(res, r - l + 1);
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

### 统计定界子数组的数目

https://leetcode.cn/problems/count-subarrays-with-fixed-bounds

题解：https://leetcode.cn/problems/count-subarrays-with-fixed-bounds/solutions/1895713/jian-ji-xie-fa-pythonjavacgo-by-endlessc-gag2

```c++
class Solution {
public:
    long long countSubarrays(vector<int>& nums, int minK, int maxK) {
        int n = nums.size(),
                minI = -1, // minK 最近一次出现在哪
                maxI = -1, // maxK 最近一次出现在哪
                exceedingI = -1; // 越界的元素最近一次出现在哪
        long long res = 0;
        for (int j = 0; j < n; ++j) { // 以 j 位置结尾时有多少个合法的子数组
            if (nums[j] == minK) {
                minI = j;
            }
            if (nums[j] == maxK) {
                maxI = j;
            }
            if (nums[j] < minK || nums[j] > maxK) {
                exceedingI = j;
            }
            /*
             情况一：minI 和 maxI 中有 -1，那么 min(minI, maxI) 为 -1，此时算出的子数组数量为 0
             情况二：minI 和 maxI 都不等于 -1，exceedingI > max(min(minI, maxI)，此时算出的子数组数量为 0
             情况三：minI 和 maxI 都不等于 -1，exceedingI < max(min(minI, maxI)，此时算出的子数组数量大于 0
            */
            res += max(min(minI, maxI) - exceedingI, 0);
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 你可以安排的最多任务数目

https://leetcode.cn/problems/maximum-number-of-tasks-you-can-assign

```c++
class Solution {
private:
    int deque[50000]; // 存放一定可以被完成的任务（可能吃药）
    int h, t;

public:
    int maxTaskAssign(vector<int>& tasks, vector<int>& workers, int numPills, int strength) {
        sort(tasks.begin(), tasks.end());
        sort(workers.begin(), workers.end());
        int n1 = tasks.size(), n2 = workers.size(), res;
        // 必须完成 m 个任务，m 越大，越难完成，即 m 越大，「能否完成」的值由 true 变为 false，存在单调性，可以使用二分查找
        for (int l = 0, r = min(n1, n2), m; l <= r;) {
            m = l + (r - l >> 1);
            if (f(tasks, workers, 0, m - 1, n2 - m, n2 - 1, numPills, strength)) {
                res = m;
                l = m + 1;
            } else {
                r = m - 1;
            }
        }
        return res;
    }

    // 必须完成 [tl...tr] 上的任务，[wl...wr] 和 [tl...tr] 的长度相同，是否能够完成
    bool f(vector<int>& tasks, vector<int>& workers, int tl, int tr, int wl, int wr, int numPills, int strength) {
        h = t = 0;
        int numUsedPills = 0; // 完成这些任务最少吃多少颗药
        for (int i = tl, j = wl; j <= wr; ++j) {
            // workers[i] 不吃药可以完成的任务
            for (; i <= tr && workers[j] >= tasks[i]; ++i) {
                deque[t++] = i;
            }
            // workers[i] 不吃药时完成力量最小的任务
            if (h < t && workers[j] >= tasks[deque[h]]) {
                ++h;
            } else { // 吃药
                // workers[i] 吃药可以完成的任务
                for (; i <= tr && workers[j] + strength >= tasks[i]; ++i) {
                    deque[t++] = i;
                }
                if (h < t) {
                    ++numUsedPills;
                    --t; // 贪心：为了不造成力量值浪费，workers[i] 吃药时完成力量最大的任务
                } else {
                    // workers[i] 吃药也无法完成任何任务，此时至少有一个任务无法被完成
                    return false;
                }
            }
        }
        return numUsedPills <= numPills;
    }
};
```

时间复杂度：$O\left(n\log n+m\log m+\min\left(n,m\right)\log\min\left(n,m\right)\right)=O\left(n\log n+m\log m\right)$，其中考虑了排序、二分查找和函数 `f`

空间复杂度：$O\left(\log n+\log m+\min\left(n,m\right)\right)$，其中考虑了排序时的栈和双端队列的空间

# 贪心

## 题目

# 递归

## 题目

### 用递归函数和栈逆序一个栈

https://www.nowcoder.com/questionTerminal/1de82c89cc0e43e9aa6ee8243f4dbefd

```c++
#include <bits/stdc++.h>

using namespace std;

// 弹出 stk 底部的元素
int f(stack<int>& stk) {
    int top = stk.top();
    stk.pop();

    if (stk.empty()) {
        return top;
    }

    int bottom = f(stk);
    stk.emplace(top);
    return bottom;
}

void reverseStack(stack<int>& stk) {
    if (stk.size() < 2) {
        return;
    }

    int bottom = f(stk);
    reverseStack(stk);
    stk.emplace(bottom);
}

int main() {
    int N;
    while (cin >> N) {
        stack<int> stk;
        while (N--) {
            int x;
            cin >> x;
            stk.emplace(x);
        }

        reverseStack(stk);

        cout << stk.top();
        stk.pop();
        while (!stk.empty()) {
            cout << ' ' << stk.top();
            stk.pop();
        }
        cout << endl;
    }

    return 0;
}
```

时间复杂度：$O\left(N\right)$

空间复杂度：$O\left(N\right)$

# 动态规划

## 知识点

### 记忆化搜索和动态规划

将记忆化搜索改成动态规划是为了根据动态规划的表结构中的 **依赖关系** 进行空间压缩和斜率优化。

### 动态规划的步骤

1. 根据可变参数的范围定义 `dp`
2. 根据递归函数的 base case 对 `dp` 进行初始化，同时注意 `dp` 中有没有无意义的格子
3. 分析依赖关系并填充 `dp`

### 什么是好的动态规划函数

1. 一个可变参数的维度越小越好（大部分题目的每个可变参数都是零维的）
2. 可变参数的个数越少越好

### 动态规划的优化方法

1. 空间压缩用于优化空间复杂度

   - 例 1：依赖关系如下图中的 g 所示。声明一个一维的 `dp`，其长度是下图中表的列数。假设现在要计算 f，在 f 覆盖掉 a 之前需要用一个临时变量 t 把 a 的值记录下来，后面就可以使用 a（t）、f 和 b 计算 g 了。

     <img src="刷题笔记.assets/image-20230829111240914.png" alt="image-20230829111240914" style="zoom:50%;" />

   - 例 2：依赖关系如下图中的 l 所示。声明 3 个一维的 `dp1`、`dp2` 和 `dp3`，它们的长度都是下图中表的列数。最开始 `dp1`、`dp2` 和 `dp3` 分别代表表中第 0、1 和 2 行的值，在计算表中的第 3 行时，直接根据 `dp2` 和 `dp3` 计算 `dp1` 即可。这个例子说明如果依赖关系不远就可以用空间压缩优化。

     <img src="刷题笔记.assets/image-20230829155213079.png" alt="image-20230829155213079" style="zoom:50%;" />
     
   - 使用真实的表结构进行空间压缩：在进行空间压缩中，在真实的表结构上标注出哪些元素是存在的以及哪些元素被覆盖了，这样不容易出错。例如在这个依赖关系中

     <img src="刷题笔记.assets/image-20230829160948809.png" alt="image-20230829160948809" style="zoom:50%;" />

     ，更新的步骤是这样的（红色表示值被覆盖了）：

     <img src="刷题笔记.assets/image-20230829161105467.png" alt="image-20230829161105467" style="zoom:50%;" />

     <img src="刷题笔记.assets/image-20230829161151158.png" alt="image-20230829161151158" style="zoom:50%;" />

2. 斜率优化用于优化时间复杂度

## 题目

### 0-1 背包

https://kamacoder.com/problempage.php?pid=1046

```c++
#include <bits/stdc++.h>
using namespace std;

int main() {
    int n, capacity;
    while (cin >> n >> capacity) {
        vector<int> weights(n), values(n);
        for (int i = 0; i < n; ++i) {
            cin >> weights[i];
        }
        for (int i = 0; i < n; ++i) {
            cin >> values[i];
        }
        vector<vector<int>> dp(n, vector<int>(capacity + 1)); // [0...i] 上背包容量为 j 时最大价值为 dp[i][j]
        for (int j = weights[0]; j <= capacity; ++j) {
            dp[0][j] = values[0];
        }
        for (int i = 1; i < n; ++i) {
            for (int j = 0; j <= capacity; ++j) {
                dp[i][j] = dp[i - 1][j];
                if (weights[i] <= j) {
                    dp[i][j] = max(dp[i][j], dp[i - 1][j - weights[i]] + values[i]);
                }
            }
        }
        cout << dp[n - 1][capacity] << endl;
    }
    return 0;
}
```

时间复杂度：$O\left(nc\right)$

空间复杂度：$O\left(nc\right)$

其中，$n$ 为 `weights` 和 `values` 的长度，$c$ 为 `capacity`

空间压缩：

```c++
#include <bits/stdc++.h>
using namespace std;

int main() {
    int n, capacity;
    while (cin >> n >> capacity) {
        vector<int> weights(n), values(n);
        for (int i = 0; i < n; ++i) {
            cin >> weights[i];
        }
        for (int i = 0; i < n; ++i) {
            cin >> values[i];
        }
        vector<int> dp(capacity + 1);
        for (int i = 0; i < n; ++i) {
            for (int j = capacity; j >= weights[i]; --j) {
                dp[j] = max(dp[j], dp[j - weights[i]] + values[i]);
            }
        }
        cout << dp[capacity] << endl;
    }
    return 0;
}
```

时间复杂度：$O\left(nc\right)$

空间复杂度：$O\left(c\right)$

### 最后一块石头的重量 II

https://leetcode.cn/problems/last-stone-weight-ii

讲解：https://www.bilibili.com/video/BV1NN41137jF

这题的本质就是把数组分成两个子集，这两个子集的累加和之间的差要尽可能小。

```c++
class Solution {
public:
    int lastStoneWeightII(vector<int>& stones) {
        int n = stones.size(), sum = accumulate(stones.begin(), stones.end(), 0), target = sum >> 1;
        vector<int> dp(target + 1);
        for (int i = 0; i < n; ++i) {
            for (int j = target; j >= stones[i]; --j) {
                dp[j] = max(dp[j], dp[j - stones[i]] + stones[i]);
            }
        }
        return sum - (dp[target] << 1); // return (sum - dp[target]) - dp[target];
    }
};
```

时间复杂度：$O\left(nt\right)$

空间复杂度：$O\left(t\right)$

其中，$t$ 为 `target`

### 目标和

https://leetcode.cn/problems/target-sum

回溯：

```c++
class Solution {
public:
    int findTargetSumWays(vector<int>& nums, int target) {
        return backtrack(nums, target, 0, 0);
    }

    // 在 [i...] 上面加正负号，[0...i-1] 上的和为 sum，有多少种方法
    int backtrack(vector<int>& nums, int target, int i, int sum) {
        int n = nums.size();
        if (i == n) {
            return sum == target;
        }
        return backtrack(nums, target, i + 1, sum + nums[i]) + backtrack(nums, target, i + 1, sum - nums[i]);
    }
};
```

时间复杂度：$O\left(2^n\right)$

空间复杂度：$O\left(n\right)$

动态规划：

```c++
class Solution {
public:
    int findTargetSumWays(vector<int>& nums, int target) {
        int n = nums.size(), sum = accumulate(nums.begin(), nums.end(), 0), m = (sum << 1) + 1;
        if (target < -sum || target > sum) {
            return 0;
        }
        // 想象中的 dp[i][j] 中 j 的范围是 [-sum,sum]，实际上平移后 j 的范围是 [0,2*sum]
        // [0...i] 范围上加了正负号之后再累加，凑 j 有 dp[i][j]（这里的 dp 是想象中的）种凑法
        vector<vector<int>> dp(n, vector<int>(m));
        dp[0][nums[0] + sum] += 1;
        dp[0][-nums[0] + sum] += 1;
        for (int i = 1; i < n; ++i) {
            for (int j = -sum; j <= sum; ++j) {
                if (j - nums[i] + sum >= 0) {
                    dp[i][j + sum] += dp[i - 1][j - nums[i] + sum];
                }
                if (j + nums[i] + sum < m) {
                    dp[i][j + sum] += dp[i - 1][j + nums[i] + sum];
                }
            }
        }
        return dp[n - 1][target + sum];
    }
};
```

时间复杂度：$O\left(nm\right)$

空间复杂度：$O\left(nm\right)$

0-1 背包：

一个集合包含所有前面带有正号的数值，其和记为 $p$；另一个集合包含所有前面带有负号的数值，其和记为 $n$，易得 $p+n=s,\ p-n=t$，其中，$t$ 代表目标值（target），$s$ 代表数组元素总和（sum）。通过解这两个方程，可以推导出 $p$ 的值为 $p=\left(s+t\right)/2$。现在问题转化成了在数组中取一个子集，这个子集的元素之和为 $\left(s+t\right)/2$。

```c++
class Solution {
public:
    int findTargetSumWays(vector<int>& nums, int target) {
        int n = nums.size(), sum = accumulate(nums.begin(), nums.end(), 0);
        if (target < -sum || target > sum || (sum + target) & 1) {
            return 0;
        }
        target = (sum + target) >> 1; // 0<=target<=sum
        vector<vector<int>> dp(n, vector<int>(target + 1)); // [0...i] 上有 dp[i][j] 种凑出 j 的方式
        dp[0][0] += 1;
        if (nums[0] <= target) {
            dp[0][nums[0]] += 1;
        }
        for (int i = 1; i < n; ++i) {
            for (int j = 0; j <= target; ++j) {
                dp[i][j] = dp[i - 1][j];
                if (j - nums[i] >= 0) {
                    dp[i][j] += dp[i - 1][j - nums[i]];
                }
            }
        }
        return dp[n - 1][target];
    }
};
```

时间复杂度：$O\left(nt\right)$

空间复杂度：$O\left(nt\right)$

空间压缩：

```c++
class Solution {
public:
    int findTargetSumWays(vector<int>& nums, int target) {
        int n = nums.size(), sum = accumulate(nums.begin(), nums.end(), 0);
        if (target < -sum || target > sum || (sum + target) & 1) {
            return 0;
        }
        target = (sum + target) >> 1;
        vector<int> dp(target + 1);
        dp[0] += 1;
        if (nums[0] <= target) {
            dp[nums[0]] += 1;
        }
        for (int i = 1; i < n; ++i) {
            for (int j = target; j >= nums[i]; --j) {
                dp[j] += dp[j - nums[i]];
            }
        }
        return dp[target];
    }
};
```

时间复杂度：$O\left(nt\right)$

空间复杂度：$O\left(t\right)$

### 一和零

背包是两个维度的 0-1 背包。

```c++
class Solution {
public:
	int findMaxForm(vector<string>& strs, int a, int b) {
		int n = strs.size();
		vector<vector<vector<int>>> dp(n, vector<vector<int>>(a + 1, vector<int>(b + 1)));
		vector<int> numZeros(n);
		vector<int> numOnes(n);
		for (int i = 0; i < n; ++i) {
			for (char& c: strs[i]) {
				if (c == '0') {
					++numZeros[i];
				} else {
					++numOnes[i];
				}
			}
		}
		for (int j = numZeros[0]; j <= a; ++j) {
			for (int k = numOnes[0]; k <= b; ++k) {
				dp[0][j][k] = 1;
			}
		}
		for (int i = 1; i < n; ++i) {
			for (int j = 0; j <= a; ++j) {
				for (int k = 0; k <= b; ++k) {
					dp[i][j][k] = dp[i - 1][j][k];
					if (j >= numZeros[i] && k >= numOnes[i]) {
						dp[i][j][k] = max(dp[i - 1][j][k], dp[i - 1][j - numZeros[i]][k - numOnes[i]] + 1);
					}
				}
			}
		}
		return dp[n - 1][a][b];
	}
};
```

时间复杂度：$O\left(nab\right)$

空间复杂度：$O\left(nab\right)$

空间压缩：

```c++
class Solution {
public:
	int findMaxForm(vector<string>& strs, int a, int b) {
		int n = strs.size();
		vector<vector<int>> dp(a + 1, vector<int>(b + 1));
		vector<int> numZeros(n);
		vector<int> numOnes(n);
		for (int i = 0; i < n; ++i) {
			for (char& c: strs[i]) {
				if (c == '0') {
					++numZeros[i];
				} else {
					++numOnes[i];
				}
			}
		}
		for (int j = numZeros[0]; j <= a; ++j) {
			for (int k = numOnes[0]; k <= b; ++k) {
				dp[j][k] = 1;
			}
		}
		for (int i = 1; i < n; ++i) {
			for (int j = a; j >= numZeros[i]; --j) {
				for (int k = b; k >= numOnes[i]; --k) {
					dp[j][k] = max(dp[j][k], dp[j - numZeros[i]][k - numOnes[i]] + 1);
				}
			}
		}
		return dp[a][b];
	}
};
```

时间复杂度：$O\left(nab\right)$

空间复杂度：$O\left(ab\right)$

### 完全背包

https://kamacoder.com/problempage.php?pid=1052

```c++
#include <bits/stdc++.h>
using namespace std;

int main() {
    int n, capacity;
    while (cin >> n >> capacity) {
        vector<int> weights(n), values(n);
        for (int i = 0; i < n; ++i) {
            cin >> weights[i] >> values[i];
        }
        vector<vector<int>> dp(n, vector<int>(capacity + 1));
        for (int j = 0; j <= capacity; ++j) {
            dp[0][j] = j / weights[0] * values[0];
        }
        for (int i = 1; i < n; ++i) {
            for (int j = 0; j <= capacity; ++j) {
                for (int currNum = 0; currNum * weights[i] <= j; ++currNum) {
                    dp[i][j] = max(dp[i][j], dp[i - 1][j - currNum * weights[i]] + currNum * values[i]);
                }
            }
        }
        cout << dp[n - 1][capacity] << endl;
    }
    return 0;
}
```

时间复杂度：$O\left(nc^2\right)$

空间复杂度：$O\left(nc\right)$

空间压缩 + 斜率优化：

```c++
#include <bits/stdc++.h>
using namespace std;

int main() {
    int n, capacity;
    while (cin >> n >> capacity) {
        vector<int> weights(n), values(n);
        for (int i = 0; i < n; ++i) {
            cin >> weights[i] >> values[i];
        }
        vector<int> dp(capacity + 1);
        for (int i = 0; i < n; ++i) {
            for (int j = weights[i]; j <= capacity; ++j) {
                dp[j] = max(dp[j], dp[j - weights[i]] + values[i]);
            }
        }
        cout << dp[capacity] << endl;
    }
    return 0;
}
```

时间复杂度：$O\left(nc\right)$

空间复杂度：$O\left(c\right)$

### 零钱兑换 II

https://leetcode.cn/problems/coin-change-ii

```c++
class Solution {
public:
    int change(int amount, vector<int>& coins) {
        int n = coins.size();
        vector<int> dp(amount + 1);
        dp[0] = 1;
        for (int i = 0; i < n; ++i) {
            for (int j = coins[i]; j <= amount; ++j) {
                if (dp[j] <= INT_MAX - dp[j - coins[i]]) {
                    dp[j] += dp[j - coins[i]];
                }
            }
        }
        return dp[amount];
    }
};
```

时间复杂度：$O\left(na\right)$

空间复杂度：$O\left(a\right)$

### 组合总和 IV

回溯：

```c++
class Solution {
public:
    int combinationSum4(vector<int>& nums, int target) {
        return backtrack(nums, target);
    }

    int backtrack(vector<int>& nums, int rest) {
        if (rest == 0) {
            return 1;
        }
        int n = nums.size(), res = 0;
        for (int i = 0, currRest; i < n; ++i) {
            currRest = rest - nums[i];
            if (currRest >= 0) {
                res += backtrack(nums, currRest);
            }
        }
        return res;
    }
};
```

时间复杂度：$O\left(t^n\right)$

空间复杂度：$O\left(t\right)$

递归（即上面的回溯）改成动态规划：

```c++
class Solution {
public:
    int combinationSum4(vector<int>& nums, int target) {
        int n = nums.size();
        vector<int> dp(target + 1);
        dp[0] = 1;
        int currRest;
        for (int j = 1; j <= target; ++j) {
            for (int i = 0; i < n; ++i) {
                currRest = j - nums[i];
                if (currRest >= 0 && dp[j] <= INT_MAX - dp[currRest]) {
                    dp[j] += dp[currRest];
                }
            }
        }
        return dp[target];
    }
};
```

时间复杂度：$O\left(tn\right)$

空间复杂度：$O\left(t\right)$

### 多重背包

https://kamacoder.com/problempage.php?pid=1066

原始版动态规划：

```c++
#include <bits/stdc++.h>
using namespace std;

int main() {
    int c, n;
    while (cin >> c >> n) {
        vector<int> weights(n), prices(n), nums(n);
        for (int i = 0; i < n; ++i) {
            cin >> weights[i];
        }
        for (int i = 0; i < n; ++i) {
            cin >> prices[i];
        }
        for (int i = 0; i < n; ++i) {
            cin >> nums[i];
        }
        vector<vector<int>> dp(n, vector<int>(c + 1));
        for (int j = 0; j <= c; ++j) {
            dp[0][j] = min(j / weights[0] * prices[0], nums[0] * prices[0]);
        }
        for (int i = 1; i < n; ++i) {
            for (int j = 0; j <= c; ++j) {
                for (int currNum = 0; currNum <= nums[i] && currNum * weights[i] <= j; ++currNum) {
                    dp[i][j] = max(dp[i][j], dp[i - 1][j - currNum * weights[i]] + currNum * prices[i]);
                }
            }
        }
        cout << dp[n - 1][c] << endl;
    }
    return 0;
}
```

时间复杂度：$O\left(nck\right)$，$k$ 为单个物品的最大数量

空间复杂度：$O\left(nc\right)$

空间压缩：

```c++
#include <bits/stdc++.h>
using namespace std;

int main() {
    int c, n;
    while (cin >> c >> n) {
        vector<int> weights(n), prices(n), nums(n);
        for (int i = 0; i < n; ++i) {
            cin >> weights[i];
        }
        for (int i = 0; i < n; ++i) {
            cin >> prices[i];
        }
        for (int i = 0; i < n; ++i) {
            cin >> nums[i];
        }
        vector<int> dp(c + 1);
        for (int i = 0; i < n; ++i) {
            for (int j = c; j >= weights[i]; --j) {
                for (int currNum = 1; currNum <= nums[i] && currNum * weights[i] <= j; ++currNum) {
                    dp[j] = max(dp[j], dp[j - currNum * weights[i]] + currNum * prices[i]);
                }
            }
        }
        cout << dp[c] << endl;
    }
    return 0;
}
```

时间复杂度：$O\left(nck\right)$，$k$ 为单个物品的最大数量

空间复杂度：$O\left(c\right)$

### 打家劫舍 II

https://leetcode.cn/problems/house-robber-ii

```c++
class Solution {
public:
    int rob(vector<int>& nums) {
        int n = nums.size();
        if (n == 1) {
            return nums[0];
        }
        // 也可以这么写：
        // return max(subRob(nums, 1, n - 1), // 一定不要 nums[0]
        //            subRob(nums, 0, n - 2)); // 一定不要 nums[n-1]
        return max(subRob(nums, 1, n - 1), // 一定不要 nums[0]
                   nums[0] + subRob(nums, 2, n - 2)); // 一定要 nums[0]
    }

    int subRob(vector<int>& nums, int l, int r) {
        if (l > r) {
            return 0;
        } else if (l == r) {
            return nums[l];
        }
        int prev1 = nums[l], prev2 = max(nums[l], nums[l + 1]);
        for (int i = l + 2, curr; i <= r; ++i) {
            curr = max(prev2, prev1 + nums[i]);
            prev1 = prev2;
            prev2 = curr;
        }
        return prev2;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 打家劫舍 IV

https://leetcode.cn/problems/house-robber-iv

二分 + 动态规划：

```c++
class Solution {
public:
    int minCapability(vector<int>& nums, int k) {
        int n = nums.size(), l = nums[0], r = nums[0];
        for (int i = 1; i < n; ++i) {
            l = min(l, nums[i]);
            r = max(r, nums[i]);
        }
        int m, mostRob;
        while (l <= r) {
            m = l + (r - l >> 1);
            mostRob = getMostRob(nums, m);
            if (mostRob < k) {
                l = m + 1;
            } else {
                r = m - 1;
            }
        }
        return l;
    }

    // nums 上最多能选多少个
    int getMostRob(vector<int>& nums, int c) {
        int n = nums.size();
        if (n == 1) {
            return c >= nums[0];
        }
        vector<int> dp(n); // nums[0...i] 上最多能选 dp[i] 个
        dp[0] = c >= nums[0];
        dp[1] = dp[0] || c >= nums[1];
        for (int i = 2; i < n; ++i) {
            dp[i] = dp[i - 1];
            if (c >= nums[i]) {
                dp[i] = max(dp[i], dp[i - 2] + 1);
            }
        }
        return dp[n - 1];
    }
};
```

时间复杂度：$O\left(n\log d\right)$，$d$ 是数组最大值与最小值之差

空间复杂度：$O\left(n\right)$

空间压缩：

```c++
class Solution {
public:
    int minCapability(vector<int>& nums, int k) {
        int n = nums.size(), l = nums[0], r = nums[0];
        for (int i = 1; i < n; ++i) {
            l = min(l, nums[i]);
            r = max(r, nums[i]);
        }
        int m, res;
        while (l <= r) {
            m = l + ((r - l) >> 1);
            if (mostRob(nums, m) >= k) {
                res = m;
                r = m - 1;
            } else {
                l = m + 1;
            }
        }
        return res;
    }

    int mostRob(vector<int>& nums, int c) {
        int n = nums.size();
        if (n == 1) {
            return nums[0] <= c;
        }

        int prev1 = nums[0] <= c;
        int prev2 = nums[0] <= c || nums[1] <= c;
        int temp;
        for (int i = 2; i < n; ++i) {
            temp = prev2;
            if (nums[i] <= c) {
                temp = max(temp, prev1 + 1);
            }
            prev1 = prev2;
            prev2 = temp;
        }
        return prev2;
    }
};
```

时间复杂度：$O\left(n\log d\right)$

空间复杂度：$O\left(1\right)$

贪心：因为选的收益都是 1，所有能尽早选就为后面留了更大的决策空间，这样优化了常数时间。

```c++
class Solution {
public:
    int minCapability(vector<int>& nums, int k) {
        int n = nums.size(), l = nums[0], r = nums[0];
        for (int i = 1; i < n; ++i) {
            l = min(l, nums[i]);
            r = max(r, nums[i]);
        }
        int m, res;
        while (l <= r) {
            m = l + ((r - l) >> 1);
            if (mostRob(nums, m) >= k) {
                res = m;
                r = m - 1;
            } else {
                l = m + 1;
            }
        }
        return res;
    }

    int mostRob(vector<int>& nums, int c) {
        int n = nums.size();
        int res = 0;
        for (int i = 0; i < n; ++i) {
            if (nums[i] <= c) {
                ++res;
                ++i;
            }
        }
        return res;
    }
};
```

时间复杂度：$O\left(\right)$

空间复杂度：$O\left(1\right)$

### 打家劫舍 III

https://leetcode.cn/problems/house-robber-iii

后序遍历做法，没有重复计算：

```c++
class Solution {
private:
    int yes, no; // 完成了 X 子树的遍历，返回之后 yes/no 变成，X 子树在偷/不偷头节点的情况下，最大的收益

public:
    int rob(TreeNode* root) {
        f(root);
        return max(yes, no);
    }

    void f(TreeNode* root) {
        if (!root) {
            yes = 0;
            no = 0;
        } else {
            int y = root->val, n = 0;
            f(root->left);
            y += no;
            n += max(yes, no);
            f(root->right);
            y += no;
            n += max(yes, no);
            yes = y;
            no = n;
        }
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(h\right)$

### 买卖股票的最佳时机 II

https://leetcode.cn/problems/best-time-to-buy-and-sell-stock-ii

```c++
class Solution {
public:
    int maxProfit(vector<int>& nums) {
        int n = nums.size(), res = 0;
        for (int i = 1; i < n; ++i) {
            res += max(0, nums[i] - nums[i - 1]); // 加上所有的「上坡」
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 买卖股票的最佳时机 III

https://leetcode.cn/problems/best-time-to-buy-and-sell-stock-iii

本题和「买卖股票的最佳时机 IV」都是通过观察具体的例子来优化枚举

超时：

```c++
class Solution {
public:
    int maxProfit(vector<int>& nums) {
        int n = nums.size();
        vector<int> dp1(n), // nums[0...i] 范围上发生一次交易，最大利润是 dp[i]
                dp2(n); // nums[0...i] 范围上发生两次交易，并且第二次交易在 i 时刻卖出，最大利润是 dp[i]
        for (int i = 1, prevMin = nums[0]; i < n; ++i) {
            dp1[i] = max(dp1[i - 1], nums[i] - prevMin);
            prevMin = min(prevMin, nums[i]);
        }
        int res = 0;
        for (int j = 1; j < n; ++j) {
            // 枚举买入时刻
            for (int i = 0; i <= j; ++i) {
                dp2[j] = max(dp2[j], dp1[i] + nums[j] - nums[i]);
            }
            res = max(res, dp2[j]);
        }
        return res;
    }
};
```

时间复杂度：$O\left(n^2\right)$

空间复杂度：$O\left(n\right)$

优化枚举：

```c++
class Solution {
public:
    int maxProfit(vector<int>& nums) {
        int n = nums.size();
        vector<int> dp1(n), helper(n), dp2(n);
        for (int i = 1, prevMin = nums[0]; i < n; ++i) {
            dp1[i] = max(dp1[i - 1], nums[i] - prevMin);
            prevMin = min(prevMin, nums[i]);
        }
        helper[0] = dp1[0] - nums[0];
        for (int i = 1; i < n; ++i) {
            helper[i] = max(helper[i - 1], dp1[i] - nums[i]);
        }
        int res = 0;
        for (int j = 1; j < n; ++j) {
            // 「观察发现」dp2[j]=max(dp1[i]-nums[i])+nums[j]，其中 i 的范围为 [0...j]
            // helper 数组用于记录 [0...i] 范围上 dp1[i]-nums[i] 的最大值
            dp2[j] = max(dp2[j], helper[j] + nums[j]);
            res = max(res, dp2[j]);
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

空间压缩：

```c++
class Solution {
public:
    int maxProfit(vector<int>& nums) {
        int n = nums.size(), res = 0; // res 就是 dp2
        for (int i = 1, prevMin = nums[0], dp1 = 0, helper = -nums[0]; i < n; ++i) {
            dp1 = max(dp1, nums[i] - prevMin);
            prevMin = min(prevMin, nums[i]);
            helper = max(helper, dp1 - nums[i]);
            res = max(res, helper + nums[i]);
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 买卖股票的最佳时机 IV

https://leetcode.cn/problems/best-time-to-buy-and-sell-stock-iv

如果 `k >= n / 2`，那么这个问题就转化为了买卖股票的最佳时机 II

```c++
class Solution {
public:
    int maxProfit(int k, vector<int>& nums) {
        int n = nums.size();
        if (k >= n >> 1) {
            int res = 0;
            for (int i = 1; i < n; ++i) {
                res += max(0, nums[i] - nums[i - 1]);
            }
            return res;
        }
        vector<vector<int>> dp(k + 1, vector<int>(n)); // i 次交易，[0...j] 范围上交易，最大收益为 dp[i][j]
        for (int i = 1; i <= k; ++i) {
            for (int j = 1; j < n; ++j) {
                dp[i][j] = dp[i][j - 1]; // 第 i 笔交易 j 时刻不卖
                // 第 i 笔交易 j 时刻卖，枚举什么时刻买
                for (int p = 0; p < j; ++p) {
                    dp[i][j] = max(dp[i][j], dp[i - 1][p] + nums[j] - nums[p]);
                }
            }
        }
        return dp[k][n - 1];
    }
};
```

时间复杂度：$O\left(kn^2\right)$

空间复杂度：$O\left(kn\right)$

```c++
class Solution {
public:
    int maxProfit(int k, vector<int>& nums) {
        int n = nums.size();
        if (k >= n >> 1) {
            int res = 0;
            for (int i = 1; i < n; ++i) {
                res += max(0, nums[i] - nums[i - 1]);
            }
            return res;
        }
        vector<vector<int>> dp(k + 1, vector<int>(n));
        for (int i = 1, helper; i <= k; ++i) {
            helper = dp[i - 1][0] - nums[0];
            for (int j = 1; j < n; ++j) {
                // 「观察发现」for 循环中的 dp[i][j]=max(dp[i-1][p]-nums[p])+nums[j]，其中 p 的范围为 [0...j)
                // helper 数组用于记录 [0...j) 范围上 dp[i-1][p]-nums[p] 的最大值
                dp[i][j] = max(dp[i][j - 1], helper + nums[j]);
                helper = max(helper, dp[i - 1][j] - nums[j]);
            }
        }
        return dp[k][n - 1];
    }
};
```

时间复杂度：$O\left(kn\right)$

空间复杂度：$O\left(kn\right)$

空间压缩：

```c++
class Solution {
public:
    int maxProfit(int k, vector<int>& nums) {
        int n = nums.size();
        if (k >= n >> 1) {
            int res = 0;
            for (int i = 1; i < n; ++i) {
                res += max(0, nums[i] - nums[i - 1]);
            }
            return res;
        }
        vector<int> dp(n);
        for (int i = 1, helper, up; i <= k; ++i) {
            helper = dp[0] - nums[0];
            for (int j = 1; j < n; ++j) {
                up = dp[j];
                dp[j] = max(dp[j - 1], helper + nums[j]);
                helper = max(helper, up - nums[j]);
            }
        }
        return dp[n - 1];
    }
};
```

时间复杂度：$O\left(kn\right)$

空间复杂度：$O\left(n\right)$

### 买卖股票的最佳时机含手续费

https://leetcode.cn/problems/best-time-to-buy-and-sell-stock-with-transaction-fee

动态规划：

```c++
class Solution {
public:
    int maxProfit(vector<int>& nums, int fee) {
        int n = nums.size();
        vector<int> dp(n); // [0...i] 范围上买卖，最大收益为 dp[i]
        for (int j = 1; j < n; ++j) {
            dp[j] = dp[j - 1];
            for (int i = 0; i < j; ++i) {
                dp[j] = max(dp[j], dp[i] + nums[j] - nums[i] - fee);
            }
        }
        return dp[n - 1];
    }
};
```

时间复杂度：$O\left(n^2\right)$

空间复杂度：$O\left(n\right)$

斜率优化 + 空间压缩：

```c++
class Solution {
public:
    int maxProfit(vector<int>& nums, int fee) {
        int n = nums.size(), res = 0;
        for (int j = 1, helper = -nums[0]; j < n; ++j) {
            res = max(res, helper + nums[j] - fee);
            helper = max(helper, res - nums[j]);
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### DI 序列的有效排列

https://leetcode.cn/problems/valid-permutations-for-di-sequence

回溯：

```c++
class Solution {
public:
    int numPermsDISequence(string s) {
        int n = s.size() + 1;
        vector<bool> used(n);
        int res = 0;
        for (int start = 0; start < n; ++start) {
            used[start] = true;
            res += backtrack(s, 1, start, used);
            used[start] = false;
        }
        return res;
    }

    int backtrack(string& s, int i, int prev, vector<bool>& used) {
        int n = s.size() + 1;
        if (i == n) {
            return 1;
        }
        int res = 0;
        for (int j = 0; j < n; ++j) {
            if (!used[j] && (j > prev && s[i - 1] == 'I' || j < prev && s[i - 1] == 'D')) {
                used[j] = true;
                res += backtrack(s, i + 1, j, used);
                used[j] = false;
            }
        }
        return res;
    }
};
```

时间复杂度：$O\left(n^n\right)$

空间复杂度：$O\left(n\right)$

### 不同的二叉搜索树

https://leetcode.cn/problems/unique-binary-search-trees

动态规划：

```c++
class Solution {
public:
    int numTrees(int n) {
        vector<int> dp(n + 1);
        dp[0] = 1;
        dp[1] = 1;

        for (int n_ = 2; n_ <= n; ++n_) {
            for (int i = 1; i - 1 <= n_ - i; ++i) {  // 讨论节点 i 为根节点的情况
                dp[n_] += i - 1 < n_ - i ? (dp[i - 1] * dp[n_ - i]) << 1 : dp[i - 1] * dp[n_ - i];
            }
        }

        return dp[n];
    }
};
```

时间复杂度：$O\left(N^2\right)$

空间复杂度：$O\left(N\right)$

------

卡塔兰数：

```c++
class Solution {
public:
    int numTrees(int n) {
        long long c = 1;
        for (int n_ = 0; n_ < n; ++n_) {
            c = (((n_ << 1) + 1) << 1) * c / (n_ + 2);
        }
        return c;
    }
};
```

时间复杂度：$O\left(N\right)$

空间复杂度：$O\left(1\right)$

------

打表：

```c++
class Solution {
public:
    int numTrees(int n) {
        switch (n) {
            case 1:
                return 1;
            case 2:
                return 2;
            case 3:
                return 5;
            case 4:
                return 14;
            case 5:
                return 42;
            case 6:
                return 132;
            case 7:
                return 429;
            case 8:
                return 1430;
            case 9:
                return 4862;
            case 10:
                return 16796;
            case 11:
                return 58786;
            case 12:
                return 208012;
            case 13:
                return 742900;
            case 14:
                return 2674440;
            case 15:
                return 9694845;
            case 16:
                return 35357670;
            case 17:
                return 129644790;
            case 18:
                return 477638700;
            case 19:
                return 1767263190;
            default:
                return 0;
        }
    }
};
```

时间复杂度：$O\left(1\right)$

空间复杂度：$O\left(1\right)$

### 不同路径

https://leetcode.cn/problems/unique-paths

动态规划 + 空间压缩：

```c++
class Solution {
public:
    int uniquePaths(int n, int m) {
        vector<int> dp(m, 1);
        for (int i = 1; i < n; ++i) {
            for (int j = 1; j < m; ++j) {
                dp[j] += dp[j - 1];
            }
        }
        return dp[m - 1];
    }
};
```

时间复杂度：$O\left(nm\right)$

空间复杂度：$O\left(m\right)$

排列组合：

组合公式（$n$ 选 $k$）：从 $n$ 一直累乘到 $n-k+1$ 一共有 $k$ 个数
$$
\binom{n}{k} = \frac{n!}{k!(n-k)!}
$$

```c++
class Solution {
public:
    int uniquePaths(int n, int m) {
        // 无论怎么走，走到终点都需要 n+m-2 步，在这 n+m-2 步中，选 n-1 步向下走，即 C_{n+m-2}^{n-1}
        long long res = 1;
        int divisor = n - 1, numDownSteps = n - 1, numTotalSteps = n + m - 2;
        while (numDownSteps--) {
            res *= numTotalSteps--;
            // 需要在计算分子的时候不断除以分母来防止溢出
            while (divisor && !(res % divisor)) {
                res /= divisor--;
            }
        }
        return res;
    }
};
```

时间复杂度：$O\left(nm\right)$

空间复杂度：$O\left(m\right)$

### 整数拆分

https://leetcode.cn/problems/integer-break

```c++
class Solution {
public:
    int integerBreak(int n) {
        vector<int> dp(n + 1,INT_MIN); // i 至少拆成两个部分，最大累积为 dp[i]，dp[0] 和 dp[1] 无意义
        for (int i = 2; i <= n; ++i) {
            // 拆出的第一个数是 j
            for (int j = 1; j < i; ++j) {
                dp[i] = i - j > 1
                            ? max(dp[i],
                                  max(dp[i - j] * j, // 拆 i-j
                                      (i - j) * j //不拆 i-j
                                  ))
                            : max(dp[i], (i - j) * j);
            }
        }
        return dp[n];
    }
};
```

时间复杂度：$O\left(n^2\right)$

空间复杂度：$O\left(n\right)$

简化写法：

```c++
class Solution {
public:
    int integerBreak(int n) {
        vector<int> dp(n + 1,INT_MIN);
        dp[1] = 0;
        for (int i = 2; i <= n; ++i) {
            for (int j = 1; j < i; ++j) {
                dp[i] = max(dp[i], max(dp[i - j] * j, (i - j) * j));
            }
        }
        return dp[n];
    }
};
```

时间复杂度：$O\left(n^2\right)$

空间复杂度：$O\left(n\right)$

贪心：

```c++
class Solution {
public:
    int integerBreak(int n) {
        // 每次拆成 n 个 3，如果剩下是 4，则保留 4，然后相乘，证明略
        if (n == 2) {
            return 1;
        }
        if (n == 3) {
            return 2;
        }
        if (n == 4) {
            return 4;
        }
        int res = 1;
        while (n > 4) {
            res *= 3;
            n -= 3;
        }
        res *= n;
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 环形子数组的最大和

https://leetcode.cn/problems/maximum-sum-circular-subarray

如果子数组未被隔断，则问题化为 [最大子数组和](https://leetcode.cn/problems/maximum-subarray)；如果子数组被隔断，则用数组所有元素累加和减最小子数组和即可，这样相当于把数组的中间一部分去掉。

一些特例说明：

1. 如果去除的子数组在整个数组边界，例如 `[1, 3, -4, -9]` 中的 `[-4, -9]`，这样实际留下的子数组并没用被隔断。但是如果出现这种情况，那么答案数组肯定未被隔断（反证法证明，如果答案数组被隔断了那么去除的子数组肯定不会出现在边界），返回 `maxSum` 即可。
2. 判断 `minSum == allSum` 其实并不能说明去除的是整个数组，因为整个数组的边界可能有 0 未被去除。但是如果出现这种情况，那么答案数组没必要隔断，仍然是返回 `maxSum` 即可。

```c++
class Solution {
public:
    int maxSubarraySumCircular(vector<int>& nums) {
        int n = nums.size(), minSum = nums[0], maxSum = nums[0], allSum = nums[0];
        for (int i = 1, minPrev = nums[0], maxPrev = nums[0]; i < n; ++i) {
            allSum += nums[i];
            minPrev = min(nums[i], minPrev + nums[i]);
            maxPrev = max(nums[i], maxPrev + nums[i]);
            minSum = min(minSum, minPrev);
            maxSum = max(maxSum, maxPrev);
        }
        return minSum == allSum ? maxSum : max(maxSum, allSum - minSum);  // 特例：去除的子数组是整个数组
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 最大子矩阵

https://leetcode.cn/problems/max-submatrix-lcci

```c++
public:
    vector<int> getMaxMatrix(vector<vector<int>>& matrix) {
        int n = matrix.size(), m = matrix[0].size();
        vector<int> nums(m);
        vector<int> res(4);
        int maxSum = INT_MIN;
        // 必须包含 r1~r2 行且仅包含 r1~r2 行上的子矩阵
        for (int r1 = 0; r1 < n; ++r1) {
            fill(nums.begin(), nums.end(), 0);
            for (int r2 = r1; r2 < n; ++r2) {
                for (int i = 0; i < m; ++i) {
                    nums[i] += matrix[r2][i];  // 压缩数组，问题被转化为了最大子数组和问题
                }
                vector<int> temp = maxSubArray(nums);
                if (temp[2] > maxSum) {
                    res[0] = r1, res[1] = temp[0], res[2] = r2, res[3] = temp[1];
                    maxSum = temp[2];
                }
            }
        }
        return res;
    }

    vector<int> maxSubArray(vector<int>& nums) {
        int n = nums.size();
        int left = -1;
        int right = -1;
        int res = INT_MIN;
        for (int l = -1, r = 0, prev = INT_MIN; r < n; ++r) {
            if (prev > 0) {
                prev += nums[r];
            } else {
                l = r;
                prev = nums[r];
            }
            if (prev > res) {
                left = l;
                right = r;
                res = prev;
            }
        }
        return vector<int>{left, right, res};
    }
};
```

时间复杂度：$O\left(n^2m\right)$

空间复杂度：$O\left(1\right)$

### 子序列累加和必须被 7 整除的最大累加和

给定一个非负数组 `nums` 可以任意选择数字组成子序列，但是子序列的累加和必须被 7 整除返回最大累加和。

```c++
#include <bits/stdc++.h>

using namespace std;

int backtrack(vector<int>& nums, int i, int sum) {
    int n = nums.size();
    if (i == n) {
        return sum % 7 == 0 ? sum : -1;  // -1 表示不存在这样的序列
    }
    return max(backtrack(nums, i + 1, sum), backtrack(nums, i + 1, sum == -1 ? nums[i] : sum + nums[i]));
}

// 用回溯暴力求解
int comparator(vector<int>& nums) {
    return backtrack(nums, 0, -1);
}

int maxSumDivisibleBy7(vector<int>& nums) {
    int n = nums.size();
    vector<vector<int>> dp(n, vector<int>(7, -1));  // nums[0...i] 上余数为 j 的子序列中最大累加和为 dp[i][j]，-1 表示不存在这样的子序列
    dp[0][nums[0] % 7] = nums[0];
    int need;
    for (int i = 1; i < n; ++i) {
        for (int j = 0; j < 7; ++j) {
            dp[i][j] = dp[i - 1][j];  // 不要 nums[i]
            need = (j - nums[i] % 7 + 7) % 7;
            if (dp[i - 1][need] != -1) {
                dp[i][j] = max(dp[i][j], dp[i - 1][need] + nums[i]);  // 要 nums[i]
            }
        }
    }
    return dp[n - 1][0];
}

vector<int> generateRandomArray(int n, int maxVal) {
    vector<int> nums(n);
    srand(time(NULL));
    for (int i = 0; i < n; ++i) {
        nums[i] = rand() % (maxVal + 1);
    }
    return nums;
}

int main() {
    int numTests = 2000;
    int n = 15;
    int maxVal = 30;
    vector<int> nums;
    for (int i = 0, res1, res2; i < numTests; ++i) {
        nums = generateRandomArray(n, maxVal);
        res1 = comparator(nums);
        res2 = maxSumDivisibleBy7(nums);
        if (res1 != res2) {
            cout << "Fail!";
            return 0;
        }
    }
    cout << "Pass!";
    return 0;
}
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 三个无重叠子数组的最大和

https://leetcode.cn/problems/maximum-sum-of-3-non-overlapping-subarrays

这题需要先得到预处理数组。

```c++
class Solution {
public:
    vector<int> maxSumOfThreeSubarrays(vector<int>& nums, int k) {
        int n = nums.size();
        vector<int> sums(n);  // 以 i 位置开头长度为 k 的子数组的累加和是 sums[i]
        for (int l = 0, r = 0, sum = 0; r < n; ++r) {
            sum += nums[r];
            if (r - l + 1 == k) {
                sums[l] = sum;
                sum -= nums[l];
                ++l;
            }
        }
        // 下面的 prefix 和 suffix 数组都是由动态规划求得的
        vector<int> prefix(n);  // nums[0...i] 上的所有长度为 k 的子数组中，拥有最大累加和的子数组，是以 i 位置开头的
        for (int l = 1, r = k; r < n; ++l, ++r) {
            prefix[r] = sums[l] > sums[prefix[r - 1]] ? l : prefix[r - 1];
        }
        vector<int> suffix(n);  // nums[i...n-1] 上的所有长度为 k 的子数组中，拥有最大累加和的子数组，是以 i 位置开头的
        suffix[n - k] = n - k;
        for (int l = n - k - 1; l >= 0; --l) {
            suffix[l] = sums[l] >= sums[suffix[l + 1]] ? l : suffix[l + 1];
        }
        int a, b, c, maxSum = 0;
        // 枚举第二个子数组所有可能的情况
        for (int s, e, i = k, j = (k << 1) - 1, sum; j < n - k; ++i, ++j) {
            // nums[0...i-1] nums[i...j] nums[j+1...]
            s = prefix[i - 1];
            e = suffix[j + 1];
            sum = sums[s] + sums[i] + sums[e];
            if (sum > maxSum) {
                a = s;
                b = i;
                c = e;
                maxSum = sum;
            }
        }
        return {a, b, c};
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

### 魔法卷轴

给定一个数组 `nums`，其中可能有正、负、0。每个魔法卷轴可以把 `nums` 中连续的一段全变成 0，你希望数组整体的累加和尽可能大。卷轴使不使用、使用多少随意，但一共只有 2 个魔法卷轴。请返回数组尽可能大的累加和。

做这道题需要构造 **前缀和** 和 **后缀和** 数组。

```c++
#include <bits/stdc++.h>
using namespace std;

// 在 [l...r] 范围上使用且仅使用 1 个卷轴，返回最大累加和
int mustOneScroll(vector<int>& nums, int l, int r) {
    int res = INT_MIN;
    int currSum;
    for (int a = l; a <= r; ++a) {
        for (int b = a; b <= r; ++b) {
            currSum = 0;
            for (int i = l; i < a; ++i) {
                currSum += nums[i];
            }
            for (int i = b + 1; i <= r; ++i) {
                currSum += nums[i];
            }
            res = max(res, currSum);
        }
    }
    return res;
}

int maxSumWithScrolls1(vector<int>& nums) {
    int n = nums.size();
    int p1 = 0;
    for (int i = 0; i < n; ++i) {
        p1 += nums[i];
    }
    int p2 = mustOneScroll(nums, 0, n - 1);
    int p3 = INT_MIN;
    for (int i = 1; i < n - 1; ++i) {
        p3 = max(p3, mustOneScroll(nums, 0, i - 1) + nums[i] + mustOneScroll(nums, i + 1, n - 1));
    }
    return max(p1, max(p2, p3));
}

int maxSumWithScrolls2(vector<int>& nums) {
    int n = nums.size();
    // 可能性 1：不使用卷轴
    int p1 = 0;
    for (int i = 0; i < n; ++i) {
        p1 += nums[i];
    }
    // 可能性 2：使用且仅使用 1 个卷轴
    int sum = nums[0];                 // 前缀和
    int maxPrevSum = max(0, nums[0]);  // 前缀和中的最大值
    vector<int> prefix(n);             // [0...i] 范围上使用且仅使用 1 个卷轴后的最大累加和
    for (int i = 1; i < n; ++i) {
        prefix[i] = max(prefix[i - 1] + nums[i],  // 卷轴不影响 i 位置
                        maxPrevSum);              // 卷轴影响 i 位置
        sum += nums[i];
        maxPrevSum = max(maxPrevSum, sum);
    }
    int p2 = prefix[n - 1];
    // 可能性 3：使用且仅使用 2 个卷轴
    sum = nums[n - 1];
    maxPrevSum = max(0, nums[n - 1]);
    vector<int> suffix(n);  // [i...n-1] 范围上使用且仅使用 1 个卷轴后的最大累加和
    for (int i = n - 2; i >= 0; --i) {
        suffix[i] = max(suffix[i + 1] + nums[i], maxPrevSum);
        sum += nums[i];
        maxPrevSum = max(maxPrevSum, sum);
    }
    int p3 = INT_MIN;
    for (int i = 1; i < n - 1; ++i) {
        p3 = max(p3, prefix[i - 1] + nums[i] + suffix[i + 1]);
    }
    return max(p1, max(p2, p3));
}

vector<int> generateRandomArray(int n, int minVal, int maxVal) {
    vector<int> nums(n);
    srand(time(nullptr));
    for (int i = 0; i < n; ++i) {
        nums[i] = rand() % (maxVal - minVal + 1) + minVal;
    }
    return nums;
}

int main() {
    int numTests = 2000;
    int n = 15;
    int minVal = -30;
    int maxVal = 30;
    vector<int> nums;
    for (int i = 0, res1, res2; i < numTests; ++i) {
        nums = generateRandomArray(15, -30, 30);
        res1 = maxSumWithScrolls1(nums);
        res2 = maxSumWithScrolls2(nums);
        if (res1 != res2) {
            cout << "Fail!";
            return 0;
        }
    }
    cout << "Pass!";
    return 0;
}
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

### 可以翻转 1 次的情况下子数组最大累加和

给定一个数组 `nums`，现在允许你随意选择数组连续一段进行翻转，也就是子数组逆序的调整比如翻转 `[1, 2, 3, 4, 5, 6]` 的 `[2...4]` 范围，得到的是 `[1, 2, 5, 4, 3, 6]`，返回必须随意翻转 1 次之后，子数组的最大累加和。

原数组翻转后，若要使最大子数组累加和变大，考虑下面 3 种情况：

记答案区域为 $A$，翻转区域为 $F$。

情况 1：$F\subseteq A$。这种情况不存在，因为新数组的最大累加和不会变大（这时新数组的答案区域和原数组一样）。

情况 2：$A\subseteq F$。这种情况不存在，理由同上。

情况 3：$A\cap F\ne\emptyset\ \text{and}\ A-F\ne\emptyset\ \text{and}\ F-A\ne\emptyset$。也就是答案区域和翻转区域是存在交集的，且各自去掉交集后各不为空。可能是答案区域在左或者翻转区域在左，但是下面只讨论翻转区域在左的情况，因为答案区域在左是可以转化为翻转区域在左的。例如 `[-4, -3, 3, 1, -5, -6, 4, 2, -9]` 翻转 `[-5, -6, 4, 2]` 区域后答案区域在左，但是如果翻转 `[3, 1, -5, -6]` 这个区域就是翻转区域在左，答案都是 `3, 1, 4, 2` 这 4 个数的累加和。

整体思路：选取 `[1...n-2]` 上的每个点作为原始部分（不翻转部分）的起始点，枚举所有答案，最后再考虑不翻转的情况。其中，每次枚举需要考虑 `[0...i-1]` 范围上选择哪一个子数组可以和原始部分拼接起来，这等价于最大子数组和问题。

```c++
#include <bits/stdc++.h>
using namespace std;

int maxSubArray(vector<int>& nums) {
    int n = nums.size();
    vector<int> dp(n);
    dp[0] = nums[0];
    int res = dp[0];
    for (int i = 1; i < n; ++i) {
        dp[i] = max(nums[i], dp[i - 1] + nums[i]);
        res = max(res, dp[i]);
    }
    return res;
}

int reverseSubarrayMaxSum1(vector<int>& nums) {
    int n = nums.size();
    int res = INT_MIN;
    for (int l = 0; l < n; ++l) {
        for (int r = l; r < n; ++r) {
            reverse(nums.begin() + l, nums.begin() + r);
            res = max(res, maxSubArray(nums));
            reverse(nums.begin() + l, nums.begin() + r);
        }
    }
    return res;
}

int reverseSubarrayMaxSum2(vector<int>& nums) {
    int n = nums.size();

    vector<int> suffix(n);  // 子数组必须以 i 位置开头往右延伸，子数组最大累加和为 suffix[i]，这个子数组为答案区域的右部分
    suffix[n - 1] = nums[n - 1];
    for (int i = n - 2; i >= 0; --i) {
        suffix[i] = max(nums[i], suffix[i + 1] + nums[i]);
    }

    int res = INT_MIN;
    int currPrefix = nums[0];  // 子数组必须以 i 位置结尾往左延伸，子数组最大累加和为 currPrefix
    int maxPrefix = nums[0];   // [0...i-1] 范围上子数组最大累加和为 maxPrefix
    for (int i = 1; i < n - 1; ++i) {
        res = max(res, maxPrefix + suffix[i]);
        currPrefix = max(nums[i], currPrefix + nums[i]);
        maxPrefix = max(maxPrefix, currPrefix);
    }
    res = max(res, maxPrefix);  // 不进行翻转（等价于选择单个元素进行翻转）
    return res;
}

vector<int> generateRandomArray(int n, int minVal, int maxVal) {
    vector<int> nums(n);
    srand(time(nullptr));
    for (int i = 0; i < n; ++i) {
        nums[i] = rand() % (maxVal - minVal + 1) + minVal;
    }
    return nums;
}

int main() {
    int numTests = 20000;
    int n = 15;
    int minVal = -30;
    int maxVal = 30;
    vector<int> nums;
    for (int i = 0, res1, res2; i < numTests; ++i) {
        nums = generateRandomArray(15, -30, 30);
        res1 = reverseSubarrayMaxSum1(nums);
        res2 = reverseSubarrayMaxSum2(nums);
        if (res1 != res2) {
            cout << "Fail!";
            return 0;
        }
    }
    cout << "Pass!";
    return 0;
}
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

### 删掉一个数字后长度为 k 的子数组最大累加和

给定一个数组 `nums`，求必须删除一个数字后的新数组中长度为 `k` 的子数组最大累加和，删除哪个数字随意。

该问题等价于长度为 `k + 1` 的子数组的元素累加和减去这个子数组中最小的元素，这样的结果最大是多少。

证明：

情况 1：删除的元素在答案区域内部。例如 `[-1, 1, 1, -1, 1, -1]`，删除 3 位置的 `-1` 后答案为 `[1, 1, 1]`，转化后的问题可以涵盖这种情况。

情况 2：删除的元素在答案区域外部。例如 `[-1, -1, 1, 1, 1, -1]`，删除 0 位置或 1 位置或 5 位置的 `-1` 后答案为 `[1, 1, 1]`，转化后的问题也可以涵盖这种情况。

```c++
// TODO：学习单调队列后做这题
```

### 裂开的方法数

给定一个正数 `n`，求 `n` 的裂开方法数。规定：后面的数不能比前面的数小 。比如 4 的裂开方法有：1 + 1 + 1 + 1、1 + 1 +  2、1 + 3、2 + 2、4，共 5 种，所以返回 5。

暴力递归：

```c++
#include <bits/stdc++.h>

using namespace std;

int f(int prev, int rest) {
    if (!rest) {
        return 1;
    }
    if (prev > rest) {
        return 0;
    }

    int res = 0;
    for (int curr = prev; curr <= rest; ++curr) {
        res += f(curr, rest - curr);
    }
    return res;
}

int splitWaysNum(int n) {
    return f(1, n);
}

int main() {
    int n;
    while (cin >> n) {
        cout << splitWaysNum(n) << endl;
    }

    return 0;
}
```

时间复杂度：

空间复杂度：

------

动态规划 1：

```c++
#include <bits/stdc++.h>

using namespace std;

int splitWaysNum(int n) {
    vector<vector<int>> dp(n + 1, vector<int>(n + 1));

    for (int prev = 1; prev <= n; ++prev) {
        dp[prev][0] = 1;
    }

    for (int prev = n; prev > 0; --prev) {
        for (int rest = prev; rest <= n; ++rest) {
            for (int curr = prev; curr <= rest; ++curr) {
                dp[prev][rest] += dp[curr][rest - curr];
            }
        }
    }

    return dp[1][n];
}

int main() {
    int n;
    while (cin >> n) {
        cout << splitWaysNum(n) << endl;
    }

    return 0;
}
```

时间复杂度：$O\left(n^3\right)$

空间复杂度：$O\left(n^2\right)$

------

动态规划 2（斜率优化）：

```c++
#include <bits/stdc++.h>

using namespace std;

int splitWaysNum(int n) {
    vector<vector<int>> dp(n + 1, vector<int>(n + 1));

    for (int prev = 1; prev <= n; ++prev) {
        dp[prev][0] = 1;
    }

    dp[n][n] = 1;
    for (int prev = n - 1; prev > 0; --prev) {
        for (int rest = prev; rest <= n; ++rest) {
            dp[prev][rest] = dp[prev + 1][rest] + dp[prev][rest - prev];
        }
    }

    return dp[1][n];
}

int main() {
    int n;
    while (cin >> n) {
        cout << splitWaysNum(n) << endl;
    }

    return 0;
}
```

时间复杂度：$O\left(n^2\right)$

空间复杂度：$O\left(n^2\right)$

------

动态规划 3（斜率优化 + 空间压缩）：

```c++
#include <bits/stdc++.h>

using namespace std;

int splitWaysNum(int n) {
    vector<int> dp(n + 1);

    dp[0] = 1;

    dp[n] = 1;
    for (int prev = n - 1; prev > 0; --prev) {
        for (int rest = prev; rest <= n; ++rest) {
            dp[rest] += dp[rest - prev];
        }
    }

    return dp[n];
}

int main() {
    int n;
    while (cin >> n) {
        cout << splitWaysNum(n) << endl;
    }

    return 0;
}
```

时间复杂度：$O\left(n^2\right)$

空间复杂度：$O\left(n\right)$

### 零钱兑换（有限制）

题目：同 https://leetcode.cn/problems/gaM7Ch，但 `coins` 中可能有重复值且每个值只能用一次。

暴力递归：

```c++
class Solution {
public:
    int coinChange(vector<int>& coins, int amount) {
        int res = f(coins, 0, amount);
        return res == INT_MAX ? -1 : res;
    }

    int f(vector<int>& coins, int i, int remain) {
        int N = coins.size();
        if (!remain) {
            return 0;
        } else if (i == N) {
            return INT_MAX;
        }

        int p1Next = remain - coins[i] >= 0 ? f(coins, i + 1, remain - coins[i]) : INT_MAX;
        int p2 = f(coins, i + 1, remain);
        return p1Next == INT_MAX ? p2 : min(p1Next + 1, p2);
    }
};
```

时间复杂度：

空间复杂度：

------

动态规划：

```c++
class Solution {
public:
    int coinChange(vector<int>& coins, int amount) {
        int N = coins.size();
        vector<vector<int>> dp(N + 1, vector<int>(amount + 1));

        for (int remain = 1; remain <= N; ++remain) {
            dp[N][remain] = INT_MAX;
        }

        for (int i = N - 1; i >= 0; --i) {
            for (int remain = 1; remain <= N; ++remain) {
                int p1Next = remain - coins[i] >= 0 ? dp[i + 1][remain - coins[i]] : INT_MAX;
                int p2 = dp[i + 1][remain];
                dp[i][remain] = p1Next == INT_MAX ? p2 : min(p1Next + 1, p2);
            }
        }
        
        return dp[0][amount];
    }
};
```

时间复杂度：$O\left(NA\right)$

空间复杂度：$O\left(NA\right)$

### 目标和

https://leetcode.cn/problems/YaVDxD

该题很容易写出暴力递归 1 所示的代码，但是会发现在改动态规划时 `remain` 的范围并不好判断，所以需要将原问题转化成另一个让可变参数的范围容易判断的问题。该题可以这样转化：

记 `nums` 的元素之和为 $S$，在 `nums` 中的元素前面添加正负号后，记所有前面添加符号的元素之和为 $M$，易得 $target=\left(S-M\right)-M$，即 $M=\left(S-target\right)/2$。现在需要在 `nums` 中找出一些元素使得这些元素之和为 $\left(S-target\right)/2$，求有多少种不同的找法。

暴力递归 1：

```c++
class Solution {
public:
    int findTargetSumWays(vector<int>& nums, int target) {
        return f(nums, 0, target);
    }

    int f(vector<int>& nums, int i, int remain) {
        int N = nums.size();
        if (i == N) {
            return !remain;
        }

        return f(nums, i + 1, remain + nums[i]) + f(nums, i + 1, remain - nums[i]);
    }
};
```

暴力递归 2：

```c++
class Solution {
public:
    int findTargetSumWays(vector<int>& nums, int target) {
        int S = reduce(nums.begin(), nums.end());
        return S - target < 0 || (S - target) & 1 ? 0 : f(nums, 0, (S - target) >> 1);
    }

    // remain >= 0
    int f(vector<int>& nums, int i, int remain) {
        int N = nums.size();
        if (i == N) {
            return !remain;
        }

        return (remain - nums[i] >= 0 ? f(nums, i + 1, remain - nums[i]) : 0) + f(nums, i + 1, remain);
    }
};
```

时间复杂度：

空间复杂度：

------

动态规划 1：

```c++
class Solution {
public:
    int findTargetSumWays(vector<int>& nums, int target) {
        int S = reduce(nums.begin(), nums.end());
        if (S - target < 0 || (S - target) & 1) {
            return 0;
        }

        int M = (S - target) >> 1;
        int N = nums.size();
        vector<vector<int>> dp(N + 1, vector<int>(M + 1));

        dp[N][0] = 1;

        for (int i = N - 1; i >= 0; --i) {
            for (int remain = M; remain >= 0; --remain) {
                dp[i][remain] = (remain - nums[i] >= 0 ? dp[i + 1][remain - nums[i]] : 0) + dp[i + 1][remain];
            }
        }

        return dp[0][M];
    }
};
```

时间复杂度：$O\left(NM\right)$

空间复杂度：$O\left(NM\right)$

------

动态规划 2（空间压缩）：

```
class Solution {
public:
    int findTargetSumWays(vector<int>& nums, int target) {
        int S = reduce(nums.begin(), nums.end());
        if (S - target < 0 || (S - target) & 1) {
            return 0;
        }

        int M = (S - target) >> 1;
        int N = nums.size();
        vector<int> dp(M + 1);

        dp[0] = 1;

        for (int i = N - 1; i >= 0; --i) {
            for (int remain = M; remain >= 0; --remain) {
                dp[remain] = (remain - nums[i] >= 0 ? dp[remain - nums[i]] : 0) + dp[remain];
            }
        }

        return dp[M];
    }
};
```

时间复杂度：$O\left(NM\right)$

空间复杂度：$O\left(M\right)$

### 机器人达到指定位置方法数

https://www.nowcoder.com/questionTerminal/54679e44604f44d48d1bcadb1fe6eb61

暴力递归：

```c++
#include <bits/stdc++.h>

using namespace std;

long long f(int N, int M, int K, int P) {
    if (!K) {
        return M == P;
    }

    return ((P > 1 ? f(N, M, K - 1, P - 1) : 0) + (P < N ? f(N, M, K - 1, P + 1) : 0)) % (long long) (1e9 + 7);
}

int main() {
    int N;
    int M;
    int K;
    int P;
    while (cin >> N >> M >> K >> P) {
        cout << f(N, M, K, P) << endl;
    }

    return 0;
}
```

时间复杂度：

空间复杂度：

------

动态规划 1：

```c++
#include <bits/stdc++.h>

using namespace std;

long long waysNum(int N, int M, int K, int P) {
    vector<vector<long long>> dp(K + 1, vector<long long>(N + 1));

    dp[0][M] = 1;

    for (int K_ = 1; K_ <= K; ++K_) {
        for (int P_ = 1; P_ <= N; ++P_) {
            dp[K_][P_] = ((P_ > 1 ? dp[K_ - 1][P_ - 1] : 0) + (P_ < N ? dp[K_ - 1][P_ + 1] : 0)) % (long long) (1e9 + 7);
        }
    }

    return dp[K][P];
}

int main() {
    int N;
    int M;
    int K;
    int P;
    while (cin >> N >> M >> K >> P) {
        cout << waysNum(N, M, K, P) << endl;
    }

    return 0;
}
```

时间复杂度：$O\left(KN\right)$

空间复杂度：$O\left(KN\right)$

------

动态规划 2（空间压缩）：

```c++
#include <bits/stdc++.h>

using namespace std;

long long waysNum(int N, int M, int K, int P) {
    vector<long long> dp(N + 1);

    dp[M] = 1;

    for (int K_ = 1; K_ <= K; ++K_) {
        int temp1 = -1;
        int temp2 = -1;
        for (int P_ = 1; P_ <= N; ++P_) {
            temp2 = dp[P_];
            dp[P_] = ((P_ > 1 ? temp1 : 0) + (P_ < N ? dp[P_ + 1] : 0)) % (long long) (1e9 + 7);
            temp1 = temp2;
        }
    }

    return dp[P];
}

int main() {
    int N;
    int M;
    int K;
    int P;
    while (cin >> N >> M >> K >> P) {
        cout << waysNum(N, M, K, P) << endl;
    }

    return 0;
}
```

时间复杂度：$O\left(KN\right)$

空间复杂度：$O\left(N\right)$

### 最长公共子串

https://www.nowcoder.com/questionTerminal/02e7cc263f8a49e8b1e1dc9c116f7602

暴力递归：

```c++
class LongestSubstring {
public:
    int findLongest(string str1, int M, string str2, int N) {
        int res = 0;
        for (int i = 0; i < M; ++i) {
            for (int j = 0; j < N; ++j) {
                res = max(res, f(str1, M, str2, N, i, j));
            }
        }
        return res;
    }

    int f(string& str1, int M, string& str2, int N, int i, int j) {
        if (!i || !j) {
            return str1[i] == str2[j];
        }

        return str1[i] == str2[j] ? 1 + f(str1, M, str2, N, i - 1, j - 1) : 0;
    }
};
```

时间复杂度：

空间复杂度：

------

动态规划 1：

```c++
class LongestSubstring {
public:
    int findLongest(string str1, int M, string str2, int N) {
        vector<vector<int>> dp(M, vector<int>(N));
        int res = 0;

        for (int i = 0; i < M; ++i) {
            dp[i][0] = str1[i] == str2[0];
            res = max(res, dp[i][0]);
        }
        for (int j = 1; j < N; ++j) {
            dp[0][j] = str1[0] == str2[j];
            res = max(res, dp[0][j]);
        }

        for (int i = 1; i < M; ++i) {
            for (int j = 1; j < N; ++j) {
                dp[i][j] = str1[i] == str2[j] ? 1 + dp[i - 1][j - 1] : 0;
                res = max(res, dp[i][j]);
            }
        }

        return res;
    }
};
```

时间复杂度：$O\left(MN\right)$

空间复杂度：$O\left(MN\right)$

------

动态规划 2（空间压缩）：

```c++
class LongestSubstring {
public:
    int findLongest(string str1, int M, string str2, int N) {
        vector<int> dp(N);
        int res = 0;

        for (int j = 0; j < N; ++j) {
            dp[j] = str1[0] == str2[j];
            res = max(res, dp[j]);
        }

        for (int i = 1; i < M; ++i) {
            int temp1 = dp[0];
            dp[0] = str1[i] == str2[0];
            res = max(res, dp[0]);
            for (int j = 1; j < N; ++j) {
                int temp2 = dp[j];
                dp[j] = str1[i] == str2[j] ? 1 + temp1 : 0;
                res = max(res, dp[j]);
                temp1 = temp2;
            }
        }

        return res;
    }
};
```

时间复杂度：$O\left(MN\right)$

空间复杂度：$O\left(N\right)$

------

动态规划 3（空间压缩）：

求二维表中的最大值，不需要真正把整张二维表都表示出来，我们只需要让二维表中的每个值都出现一次并记录下最大值就行了。

填表顺序如下：

<img src="刷题笔记.assets/image-20230921203809766.png" alt="image-20230921203809766" style="zoom:33%;" />

```c++
class LongestSubstring {
public:
    int findLongest(string str1, int M, string str2, int N) {
        int row = 0;
        int col = N - 1;
        int res = 0;

        while (row < M) {
            int i = row;
            int j = col;
            int prev = 0;

            if (!i) {
                prev = str1[0] == str2[j];
                res = max(res, prev);
            } else if (!j) {
                prev = str1[i] == str2[0];
                res = max(res, prev);
            }
            ++i;
            ++j;

            while (i < M && j < N) {
                prev = str1[i] == str2[j] ? 1 + prev : 0;
                res = max(res, prev);
                ++i;
                ++j;
            }

            col ? --col : ++row;
        }

        return res;
    }
};
```

时间复杂度：$O\left(MN\right)$

空间复杂度：$O\left(1\right)$

------

动态规划 4（空间压缩，同动态规划 3，只是换了一种写法）：

```c++
class LongestSubstring {
public:
    int findLongest(string str1, int M, string str2, int N) {
        int row = 0;
        int col = N - 1;
        int res = 0;

        while (row < M) {
            int i = row;
            int j = col;
            int len = 0;

            while (i < M && j < N) {
                if (str1[i] == str2[j]) {
                    res = max(res, ++len);
                } else {
                    len = 0;
                }
                ++i;
                ++j;
            }

            col ? --col : ++row;
        }

        return res;
    }
};
```

时间复杂度：$O\left(MN\right)$

空间复杂度：$O\left(1\right)$

### 最长回文子序列

https://leetcode.cn/problems/longest-palindromic-subsequence

最长公共子序列的变式：

```c++
class Solution {
public:
    int longestPalindromeSubseq(string str) {
        string strReversed = str;
        reverse(str.begin(), str.end());
        return longestCommonSubsequence(str, strReversed);
    }

    int longestCommonSubsequence(string str1, string str2) {
        int M = str1.size();
        int N = str2.size();
        vector<int> dp(N);

        dp[0] = str1[0] == str2[0];
        for (int j = 1; j < N; ++j) {
            dp[j] = str1[0] == str2[j] ? 1 : dp[j - 1];
        }

        for (int i = 1; i < M; ++i) {
            int temp1 = dp[0];
            dp[0] = str1[i] == str2[0] ? 1 : dp[0];
            for (int j = 1; j < N; ++j) {
                int temp2 = dp[j];
                dp[j] = max(dp[j - 1], dp[j]);
                if (str1[i] == str2[j]) {
                    dp[j] = max(dp[j], temp1 + 1);
                }
                temp1 = temp2;
            }
        }

        return dp[N - 1];
    }
};
```

时间复杂度：$O\left(N^2\right)$

空间复杂度：$O\left(N\right)$

暴力递归：

```c++
class Solution {
public:
    int longestPalindromeSubseq(string str) {
        int N = str.size();
        return f(str, 0, N - 1);
    }

    // str[i...j] 上的最长回文子序列
    int f(string& str, int i, int j) {
        if (i == j) {
            return 1;
        } else if (i > j) {
            return 0;
        }

        int res = max(f(str, i + 1, j), f(str, i, j - 1));
        if (str[i] == str[j]) {
            res = max(res, f(str, i + 1, j - 1) + 2);  // 这里可能出现 i + 1 > j - 1 的情况
        }
        return res;
    }
};
```

时间复杂度：$O\left(N^2\right)$

空间复杂度：$O\left(N\right)$

动态规划：

```c++
class Solution {
public:
    int longestPalindromeSubseq(string str) {
        int N = str.size();
        vector<vector<int>> dp(N, vector<int>(N));

        for (int i = 0; i < N; ++i) {
            dp[i][i] = 1;
        }

        for (int i = N - 2; i >= 0; --i) {
            for (int j = i + 1; j < N; ++j) {
                dp[i][j] = max(dp[i + 1][j], dp[i][j - 1]);
                if (str[i] == str[j]) {
                    dp[i][j] = max(dp[i][j], dp[i + 1][j - 1] + 2);
                }
            }
        }

        return dp[0][N - 1];
    }
};
```

时间复杂度：$O\left(N^2\right)$

空间复杂度：$O\left(N^2\right)$

动态规划 2（空间压缩）：

```c++
class Solution {
public:
    int longestPalindromeSubseq(string str) {
        int N = str.size();
        vector<int> dp(N);

        dp[N - 1] = 1;

        for (int i = N - 2; i >= 0; --i) {
            int temp1 = 0;
            dp[i] = 1;
            for (int j = i + 1; j < N; ++j) {
                int temp2 = dp[j];
                dp[j] = max(temp2, dp[j - 1]);
                if (str[i] == str[j]) {
                    dp[j] = max(dp[j], temp1 + 2);
                }
                temp1 = temp2;
            }
        }

        return dp[N - 1];
    }
};
```

时间复杂度：$O\left(N^2\right)$

空间复杂度：$O\left(N\right)$

### Bob 的生存概率

https://www.nowcoder.com/questionTerminal/be311584a3d64ee0a7462dbeb79f00f5

暴力递归：

```c++
#include <bits/stdc++.h>

using namespace std;

long long f(int M, int N, int i, int j, int k) {
    if (i < 0 || i > M - 1 || j < 0 || j > N - 1) {
        return 0;
    } else if (!k) {
        return 1;
    }

    return f(M, N, i - 1, j, k - 1) + f(M, N, i + 1, j, k - 1) + f(M, N, i, j - 1, k - 1) + f(M, N, i, j + 1, k - 1);
}

string survivalProbability(int M, int N, int i, int j, int k) {
    long long totalWaysNum = pow(4, k);
    long long survivalWaysNum = f(M, N, i, j, k);
    long long gcdVal = gcd(survivalWaysNum, totalWaysNum);
    return to_string(survivalWaysNum / gcdVal) + "/" + to_string(totalWaysNum / gcdVal);
}

int main() {
    int M;
    int N;
    int i;
    int j;
    int k;
    while (cin >> M >> N >> i >> j >> k) {
        cout << survivalProbability(M, N, i, j, k) << endl;
    }

    return 0;
}
```

时间复杂度：

空间复杂度：

------

动态规划 1：

```c++
#include <bits/stdc++.h>

using namespace std;

long long getSurvivalWaysNum(int M, int N, int i, int j, int k) {
    vector<vector<vector<long long>>> dp(M, vector<vector<long long>>(N, vector<long long>(k + 1)));

    for (int i_ = 0; i_ < M; ++i_) {
        for (int j_ = 0; j_ < N; ++j_) {
            dp[i_][j_][0] = 1;
        }
    }

    auto getValue = [&dp](int i_, int j_, int k_) -> long long {
        int M = dp.size();
        int N = dp[0].size();
        return i_ < 0 || i_ > M - 1 || j_ < 0 || j_ > N - 1 ? 0 : dp[i_][j_][k_];
    };
    for (int k_ = 1; k_ <= k; ++k_) {
        for (int i_ = 0; i_ < M; ++i_) {
            for (int j_ = 0; j_ < N; ++j_) {
                dp[i_][j_][k_] = getValue(i_ - 1, j_, k_ - 1) + getValue(i_ + 1, j_, k_ - 1) + getValue(i_, j_ - 1, k_ - 1) + getValue(i_, j_ + 1, k_ - 1);
            }
        }
    }
    return dp[i][j][k];
}

string survivalProbability(int M, int N, int i, int j, int k) {
    long long totalWaysNum = pow(4, k);
    long long survivalWaysNum = getSurvivalWaysNum(M, N, i, j, k);
    long long gcdVal = gcd(survivalWaysNum, totalWaysNum);
    return to_string(survivalWaysNum / gcdVal) + "/" + to_string(totalWaysNum / gcdVal);
}

int main() {
    int M;
    int N;
    int i;
    int j;
    int k;
    while (cin >> M >> N >> i >> j >> k) {
        cout << survivalProbability(M, N, i, j, k) << endl;
    }

    return 0;
}
```

时间复杂度：$O\left(MNk\right)$

空间复杂度：$O\left(MNk\right)$

------

动态规划 2（空间压缩）：

```c++
#include <bits/stdc++.h>

using namespace std;

long long getSurvivalWaysNum(int M, int N, int i, int j, int k) {
    vector<vector<long long>> dp(M, vector<long long>(N, 1));

    for (int k_ = 1; k_ <= k; ++k_) {
        vector<long long> temp1(N);
        vector<long long> temp2(N);
        for (int i_ = 0; i_ < M; ++i_) {
            temp2 = dp[i_];
            for (int j_ = 0; j_ < N; ++j_) {
                dp[i_][j_] = (i_ > 0 ? temp1[j_] : 0) + (i_ < M - 1 ? dp[i_ + 1][j_] : 0) + (j_ > 0 ? temp2[j_ - 1] : 0) + (j_ < N - 1 ? dp[i_][j_ + 1] : 0);
            }
            temp1 = temp2;
        }
    }
    return dp[i][j];
}

string survivalProbability(int M, int N, int i, int j, int k) {
    long long totalWaysNum = pow(4, k);
    long long survivalWaysNum = getSurvivalWaysNum(M, N, i, j, k);
    long long gcdVal = gcd(survivalWaysNum, totalWaysNum);
    return to_string(survivalWaysNum / gcdVal) + "/" + to_string(totalWaysNum / gcdVal);
}

int main() {
    int M;
    int N;
    int i;
    int j;
    int k;
    while (cin >> M >> N >> i >> j >> k) {
        cout << survivalProbability(M, N, i, j, k) << endl;
    }

    return 0;
}
```

时间复杂度：$O\left(MNk\right)$

空间复杂度：$O\left(MN\right)$

### 跳马

https://www.nowcoder.com/questionTerminal/c45704a41617402fb5c34a1778bb2645

暴力递归：

递归函数直接对前一步的位置进行分类的话可以不用在递归函数的参数列表中传入当前位置，这样会使得递归函数的入参更少。

```c++
#include <bits/stdc++.h>

using namespace std;

int waysNum(int x, int y, int k) {
    if (x < 0 || x > 8 || y < 0 || y > 9) {
        return 0;
    }
    if (!k) {
        return !x && !y;
    }

    return waysNum(x + 2, y + 1, k - 1) + waysNum(x + 2, y - 1, k - 1) + waysNum(x - 2, y + 1, k - 1) + waysNum(x - 2, y - 1, k - 1) + waysNum(x + 1, y + 2, k - 1) + waysNum(x + 1, y - 2, k - 1) + waysNum(x - 1, y + 2, k - 1) + waysNum(x - 1, y - 2, k - 1);
}

int main() {
    int x;
    int y;
    int k;
    while (cin >> x >> y >> k) {
        cout << waysNum(x, y, k) << endl;
    }

    return 0;
}
```

时间复杂度：

空间复杂度：

------

动态规划 1：

```c++
#include <bits/stdc++.h>

using namespace std;

int getVal(vector<vector<vector<int>>>& dp, int x_, int y_, int k_) {
    if (x_ < 0 || x_ > 8 || y_ < 0 || y_ > 9) {
        return 0;
    }

    return dp[x_][y_][k_];
}

int waysNum(int x, int y, int k) {
    vector<vector<vector<int>>> dp(9, vector<vector<int>>(10, vector<int>(k + 1)));

    dp[0][0][0] = 1;

    for (int k_ = 1; k_ <= k; ++k_) {
        for (int x_ = 0; x_ < 9; ++x_) {
            for (int y_ = 0; y_ < 10; ++y_) {
                dp[x_][y_][k_] = getVal(dp, x_ - 1, y_ + 2, k_ - 1) + getVal(dp, x_ + 1, y_ + 2, k_ - 1) + getVal(dp, x_ - 2, y_ + 1, k_ - 1) + getVal(dp, x_ + 2, y_ + 1, k_ - 1) + getVal(dp, x_ - 2, y_ - 1, k_ - 1) + getVal(dp, x_ + 2, y_ - 1, k_ - 1) + getVal(dp, x_ - 1, y_ - 2, k_ - 1) + getVal(dp, x_ + 1, y_ - 2, k_ - 1);
            }
        }
    }

    return dp[x][y][k];
}

int main() {
    int x;
    int y;
    int k;
    while (cin >> x >> y >> k) {
        cout << waysNum(x, y, k) << endl;
    }

    return 0;
}
```

时间复杂度：$O\left(k\right)$

空间复杂度：$O\left(k\right)$

------

动态规划 2（空间压缩）：

```c++
#include <bits/stdc++.h>

using namespace std;

int getVal(vector<vector<int>>& temp, int x_, int y_) {
    if (x_ < 0 || x_ > 8 || y_ < 0 || y_ > 9) {
        return 0;
    }

    return temp[x_][y_];
}

int waysNum(int x, int y, int k) {
    vector<vector<int>> dp(9, vector<int>(10));

    dp[0][0] = 1;

    for (int k_ = 1; k_ <= k; ++k_) {
        vector<vector<int>> temp = dp;
        for (int x_ = 0; x_ < 9; ++x_) {
            for (int y_ = 0; y_ < 10; ++y_) {
                dp[x_][y_] = getVal(temp, x_ - 1, y_ + 2) + getVal(temp, x_ + 1, y_ + 2) + getVal(temp, x_ - 2, y_ + 1) + getVal(temp, x_ + 2, y_ + 1) + getVal(temp, x_ - 2, y_ - 1) + getVal(temp, x_ + 2, y_ - 1) + getVal(temp, x_ - 1, y_ - 2) + getVal(temp, x_ + 1, y_ - 2);
            }
        }
    }

    return dp[x][y];
}

int main() {
    int x;
    int y;
    int k;
    while (cin >> x >> y >> k) {
        cout << waysNum(x, y, k) << endl;
    }

    return 0;
}
```

时间复杂度：$O\left(k\right)$

空间复杂度：$O\left(1\right)$

### 序列

https://noobdream.com/DreamJudge/Issue/page/995

暴力递归：

```c++
#include <bits/stdc++.h>

using namespace std;

// 0 < i <= N
int f(vector<int>& A, int i, int prev) {
    int N = A.size();
    if (i == N) {
        return 0;
    }

    int followingWeight = INT_MAX;
    for (int num = 0; num < 10; ++num) {
        followingWeight = min(followingWeight, abs(A[i] - num) + (num - prev) * (num - prev) + f(A, i + 1, num));
    }
    return followingWeight;
}

int minWeight(vector<int>& A) {
    int N = A.size();
    if (N == 1) {
        return 0;
    }

    int res = INT_MAX;
    for (int num = 0; num < 10; ++num) {
        res = min(res, abs(A[0] - num) + f(A, 1, num));
    }
    return res;
}

int main() {
    int N;
    while (cin >> N) {
        vector<int> A(N);
        for (int& num: A) {
            cin >> num;
        }

        cout << minWeight(A) << endl;
    }

    return 0;
}
```

时间复杂度：

空间复杂度：

------

动态规划 1：

```c++
#include <bits/stdc++.h>

using namespace std;

int minWeight(vector<int>& A) {
    int N = A.size();
    if (N == 1) {
        return 0;
    }

    vector<vector<int>> dp(N + 1, vector<int>(10));

    for (int i = N - 1; i > 0; --i) {
        for (int prev = 0; prev < 10; ++prev) {
            dp[i][prev] = INT_MAX;
            for (int num = 0; num < 10; ++num) {
                dp[i][prev] = min(dp[i][prev], abs(A[i] - num) + (num - prev) * (num - prev) + dp[i + 1][num]);
            }
        }
    }

    int res = INT_MAX;
    for (int num = 0; num < 10; ++num) {
        res = min(res, abs(A[0] - num) + dp[1][num]);
    }
    return res;
}

int main() {
    int N;
    while (cin >> N) {
        vector<int> A(N);
        for (int& num: A) {
            cin >> num;
        }

        cout << minWeight(A) << endl;
    }

    return 0;
}
```

时间复杂度：$O\left(N\right)$

空间复杂度：$O\left(N\right)$

------

动态规划 2（空间压缩）：

```c++
#include <bits/stdc++.h>

using namespace std;

int minWeight(vector<int>& A) {
    int N = A.size();
    if (N == 1) {
        return 0;
    }

    vector<int> dp(10);

    for (int i = N - 1; i > 0; --i) {
        vector<int> temp = dp;
        for (int prev = 0; prev < 10; ++prev) {
            dp[prev] = INT_MAX;
            for (int num = 0; num < 10; ++num) {
                dp[prev] = min(dp[prev], abs(A[i] - num) + (num - prev) * (num - prev) + temp[num]);
            }
        }
    }

    int res = INT_MAX;
    for (int num = 0; num < 10; ++num) {
        res = min(res, abs(A[0] - num) + dp[num]);
    }
    return res;
}

int main() {
    int N;
    while (cin >> N) {
        vector<int> A(N);
        for (int& num: A) {
            cin >> num;
        }

        cout << minWeight(A) << endl;
    }

    return 0;
}
```

时间复杂度：$O\left(N\right)$

空间复杂度：$O\left(1\right)$

### 预测赢家

https://leetcode.cn/problems/predict-the-winner

这个题目如果要改动态规划的话，需要一个表 `fdp` 和一个表 `sdp`，因为每个表记录的是一个递归函数在确定的可变参数下的取值，现在有两个递归函数，所以需要两个表。假设 `L` 表示行，`R` 表示列，因为 `L <= R`，所以两个表的左下部分是不会被使用的（但是对角线是可以使用的）。

暴力递归 1：

```c++
class Solution {
public:
    bool predictTheWinner(vector<int>& nums) {
        int N = nums.size();
        return f(nums, 0, N - 1) >= s(nums, 0, N - 1);
    }

    // 如果我是先手，我能获得的最大分数
    int f(vector<int>& arr, int L, int R) {
        if (L == R) {
            return arr[L];
        }

        // 我拿左端和右端两种情况，取max
        return max(arr[L] + s(arr, L + 1, R), arr[R] + s(arr, L, R - 1));
    }

    // 如果我是后手，我能获得的最大分数
    int s(vector<int>& arr, int L, int R) {
        if (L == R) {
            return 0;
        }

        // 这一步是对手决定的，对手肯定是选择一个对我最不利的拿法，所以取min，所以分对手拿左端和右端两种情况
        return min(f(arr, L + 1, R), f(arr, L, R - 1));
    }
};
```

也可以只用一个递归函数实现。

暴力递归 2：

```c++
class Solution {
public:
    bool predictTheWinner(vector<int>& nums) {
        int N = nums.size();
        return f(nums, 0, N - 1) >= 0;
    }

    // 如果返回的是负数，那么这个负数的绝对值表示对手的得分
    int f(vector<int>& nums, int L, int R) {
        if (L == R) {
            return nums[L];
        }

        return max(nums[L] - f(nums, L + 1, R), nums[R] - f(nums, L, R - 1));
    }
};
```

时间复杂度：

空间复杂度：

------

动态规划 1：

```c++
class Solution {
public:
    bool predictTheWinner(vector<int>& nums) {
        int N = nums.size();
        vector<vector<int>> fdp(N, vector<int>(N));
        vector<vector<int>> sdp(N, vector<int>(N));

        for (int i = 0; i < N; ++i) {
            fdp[i][i] = nums[i];
        }

        for (int L = N - 2; L >= 0; --L) {
            for (int R = L + 1; R < N; ++R) {
                fdp[L][R] = max(nums[L] + sdp[L + 1][R], nums[R] + sdp[L][R - 1]);
                sdp[L][R] = min(fdp[L + 1][R], fdp[L][R - 1]);
            }
        }

        return fdp[0][N - 1] >= sdp[0][N - 1];
    }
};
```

动态规划 2：

```c++
class Solution {
public:
    bool predictTheWinner(vector<int>& nums) {
        int N = nums.size();
        vector<vector<int>> fdp(N, vector<int>(N));
        vector<vector<int>> sdp(N, vector<int>(N));

        for (int i = 0; i < N; ++i) {
            fdp[i][i] = nums[i];
        }

        int slash = 1;
        while (slash < N) {
            int L = 0;
            int R = slash;
            while (R < N) {
                fdp[L][R] = max(nums[L] + sdp[L + 1][R], nums[R] + sdp[L][R - 1]);
                sdp[L][R] = min(fdp[L + 1][R], fdp[L][R - 1]);
                ++L;
                ++R;
            }
            ++slash;
        }

        return fdp[0][N - 1] >= sdp[0][N - 1];
    }
};
```

动态规划 3：

```c++
class Solution {
public:
    bool predictTheWinner(vector<int>& nums) {
        int N = nums.size();
        vector<vector<int>> dp(N, vector<int>(N));

        for (int i = 0; i < N; ++i) {
            dp[i][i] = nums[i];
        }

        for (int L = N - 2; L >= 0; --L) {
            for (int R = L + 1; R < N; ++R) {
                dp[L][R] = max(nums[L] - dp[L + 1][R], nums[R] - dp[L][R - 1]);
            }
        }

        return dp[0][N - 1] >= 0;
    }
};
```

时间复杂度：$O\left(N^2\right)$

空间复杂度：$O\left(N^2\right)$

------

动态规划 4（空间压缩）：

```c++
class Solution {
public:
    bool predictTheWinner(vector<int>& nums) {
        int N = nums.size();
        vector<int> dp(N);

        for (int L = N - 1; L >= 0; --L) {
            dp[L] = nums[L];
            for (int R = L + 1; R < N; ++R) {
                dp[R] = max(nums[L] - dp[R], nums[R] - dp[R - 1]);
            }
        }

        return dp[N - 1] >= 0;
    }
};
```

时间复杂度：$O\left(N^2\right)$

空间复杂度：$O\left(N\right)$

# Dynamic Programming on Trees

## Preliminaries

A subtree with node X as its root node needs to obtain some information (such as the height of the subtree or the number of nodes on the subtree) from its left and right subtrees, and then process the same information belonging to node X. The only consideration for dynamic programming on trees is what this information is and how to process the current node's own information based on the information of the left and right subtrees of the current node.

## Problems

### Maximum Happiness Value of Party (ByteDance Interview)

https://www.nowcoder.com/practice/a5f542742fe24181b28f7d5b82e2e49a

English translation:

The entire company's personnel structure can be viewed as a standard multi-tree. The head node of the tree is the only boss of the company. Except for the boss, every employee has a unique direct superior. The leaf nodes are junior employees who do not have any subordinates. Except for junior employees, each employee has one or more direct subordinates. In addition, each employee has a happiness value.

This company is going to hold a party now, and you can decide which employees will come and which employees will not come. But follow the following principles:

1. If an employee comes, all direct subordinates of this employee cannot come.

2. The overall happiness value of the party is the accumulation of the happiness values of all employees present.

3. Your goal is to maximize the overall happiness of the party.

Given a multi-tree, please output the maximum happiness value of the party.

Input description:

The first line has two integers `n` and `root`, `n` represents the total number of people in the company, and `root` represents the company's boss.

The second line of `n` integers `happy_i` represents the happiness value of employee `i`.

The next `n - 1` lines each contain two integers `u_i ` and `v_i` indicating that `u_i` is the direct superior of `v_i`.

Output description:

Output an integer representing the maximum happiness value.

Example 1:

```markdown
Input:
3 1
5 1 1
1 2
1 3 
output:
5
```

Constraints:

```markdown
1 <= n <= 500000
0 <= happy_i <= 1000
The input is guaranteed to be a tree.
```

**DP on the tree.**

```c++
#include <bits/stdc++.h>

using namespace std;

class TreeNode {
public:
    int val;
    vector<TreeNode*> nexts;

    TreeNode(int happy = -1) { this->val = happy; }
};

class Info {
public:
    int yes;  // `root` joins the party.
    int no;  // `root` does not join the party.

    Info(int yes = -1, int no = -1) : yes(yes), no(no) {}
};

Info f(TreeNode* root) {
    if (root->nexts.empty()) {
        return Info(root->val, 0);
    }

    int childNodesNum = root->nexts.size();
    vector<Info> infos(childNodesNum);
    for (int i = 0; i < childNodesNum; ++i) {
        infos[i] = f(root->nexts[i]);
    }
    int yes = root->val;
    int no = 0;
    for (Info& info: infos) {
        yes += info.no;
        no += max(info.yes, info.no);
    }
    return Info(yes, no);
}

int maxHappiness(TreeNode* root) {
    Info info = f(root);
    return max(info.yes, info.no);
}

int main() {
    int n;
    while (cin >> n) {
        vector<TreeNode*> treeNodes(n + 1);  // Don't write vector<TreeNode*> treeNodes(n + 1, new TreeNode()) here. In this case, all pointers in treeNodes point to the same TreeNode object.
        int rootId;
        cin >> rootId;

        for (int i = 1; i <= n; ++i) {
            int val;
            cin >> val;
            treeNodes[i] = new TreeNode(val);
        }

        for (int i = 0; i < n - 1; ++i) {
            int parentId;
            int childId;
            cin >> parentId >> childId;
            treeNodes[parentId]->nexts.emplace_back(treeNodes[childId]);
        }

        cout << maxHappiness(treeNodes[rootId]) << endl;
    }

    return 0;
}
```

Time complexity: $O\left(n\right)$, where $n$ is the number of nodes of the tree.

Space complexity: $O\left(h\right)$, where $h$ is the height of the tree.

# 数学

## 题目

### 判断素数

https://www.nowcoder.com/questionTerminal/c76fb2da124842079dfefb4da2c5317d

使用平方根并迭代：

如果 $n$ 是合数，那么必然存在两个非 $1$ 的数 $p_1$ 和 $p_2$，其中 $p_1\le\sqrt{n},\quad p_2\ge\sqrt{n}$。

```c++
#include <bits/stdc++.h>

using namespace std;

bool isPrime(long long n) {
    if (n <= 1) {
        return false;
    } else if (n == 2) {
        return true;
    } else if (!(n & 1)) {
        return false;
    }

    long long sqrtn = sqrt(n);
    for (long long i = 3; i <= sqrtn; i += 2) {  // 因为 n 是奇数，所以这里 i 跳过偶数
        if (n % i == 0) {
            return false;
        }
    }
    return true;
}

int main() {
    long long n;
    while (cin >> n) {
        cout << boolalpha << isPrime(n) << endl;
    }

    return 0;
}
```

时间复杂度：$O\left(\sqrt{n}\right)$

空间复杂度：$O\left(1\right)$

直接判断：

```c++
#include <bits/stdc++.h>

using namespace std;

bool isPrime(long long& n) {
    return n == 2 || n == 3 || n % 6 == 1 || n % 6 == 5;
}

int main() {
    long long n;
    while (cin >> n) {
        cout << boolalpha << isPrime(n) << endl;
    }

    return 0;
}
```

时间复杂度：$O\left(1\right)$

空间复杂度：$O\left(1\right)$

打表：

如果 $n$ 的范围小那么可以直接打表，代码略。

时间复杂度：$O\left(1\right)$

空间复杂度：$O\left(1\right)$

### Find Greatest Common Divisor of Array

https://leetcode.com/problems/find-greatest-common-divisor-of-array

```c++
class Solution {
public:
    int findGCD(vector<int>& nums) {
        int minVal = *min_element(nums.begin(), nums.end());
        int maxVal = *max_element(nums.begin(), nums.end());
        return gcd(minVal, maxVal);
    }

    int gcd(int a, int b) {
        return !b ? a : gcd(b, a % b);
    }
};
```

Time Complexity: $O\left(n+\log M\right)$, where $n$ is the length of the array and $M$ is the maximum value in the array.

Space Complexity: $O\left(\log M\right)$

The time and space complexity of the greatest common divisor function are $O\left(\log M\right)$ because each time the function is called, the first parameter of the function is at least halved.

### 求最小公倍数

https://www.nowcoder.com/practice/22948c2cad484e0291350abad86136c3

迭代：

```c++
#include <bits/stdc++.h>

using namespace std;

int lcm(int a, int b) {
    if (a < b) {
        swap(a, b);
    }

    int res = a;
    while (res % b) {
        res += a;
    }
    return res;
}

int main() {
    int a;
    int b;
    while (cin >> a >> b) {
        cout << lcm(a, b) << endl;
    }

    return 0;
}
```

时间复杂度：$O\left(\min\left(a,b\right)\right)$

空间复杂度：$O\left(1\right)$

递归：

```c++
#include <bits/stdc++.h>

using namespace std;

int gcd(int m, int n) {
    return !n ? m : gcd(n, m % n);  // 对 m 取模后 m 至少折半
}

int lcm(int a, int b) {
    return a / gcd(a, b) * b;  // 先算除法防止溢出
}

int main() {
    int a;
    int b;
    while (cin >> a >> b) {
        cout << lcm(a, b) << endl;
    }

    return 0;
}
```

时间复杂度：$O\left ( \log \max \left ( a,b \right )  \right ) $

空间复杂度：$O\left ( \log \max \left ( a,b \right )  \right ) $

### Single Number

https://leetcode.com/problems/single-number

Properties of XOR:

1. XOR is commutative. This means $a \oplus b = b \oplus a$.
2. XOR is associative. This means $\left ( a \oplus b \right ) \oplus c = a \oplus \left ( b \oplus c \right )$.
3. XOR is self-inverse. This means $a \oplus a = 0$.
4. $0$ is the identity element for XOR. This means $a \oplus 0 = 0$.

```c++
class Solution {
public:
    int singleNumber(vector<int>& nums) {
        int res = 0;
        for (int& num: nums) {
            res ^= num;
        }
        return res;
    }
};
```

Time Complexity: $O\left(n\right)$, where $n$ is the length of the array.

Space Complexity: $O\left(1\right)$

### Single Number II

https://leetcode.com/problems/single-number-ii

The solution process is shown in the figure below.

<img src="刷题笔记.assets/image-20231010212442621.png" alt="image-20231010212442621" style="zoom:50%;" />

**Traverse the array.**

```c++
class Solution {
public:
    int singleNumber(vector<int>& nums) {
        int counts[32] = {0};
        for (int num: nums) {
            for (int i = 31; i >= 0; --i) {
                counts[i] += num & 1;
                num >>= 1;
            }
        }

        int res = 0;
        for (int i = 0; i < 32; ++i) {
            res <<= 1;
            res |= counts[i] % 3;
        }

        return res;
    }
};
```

Time Complexity: $O\left(n\right)$, where $n$ is the length of the array.

Space Complexity: $O\left(1\right)$

**Finite state machine.**

The value of any number modulo 3 is 0, 1, or 2. So the range of each value in `counts` can be limited to 0, 1, 2. The finite state machine when each number in counts receives a certain bit (0 or 1) of a binary number is shown in the figure below.

<img src="刷题笔记.assets/image-20231010214740023.png" alt="image-20231010214740023" style="zoom:50%;" />

Now we can use two bits to represent the three numbers 0, 1 and 2. The low bit is marked as `bit0`, and the high bit is marked as `bit1`.

<img src="刷题笔记.assets/image-20231010220523200.png" alt="image-20231010220523200" style="zoom:50%;" />

The above finite state machine can be represented by the following two truth tables.

<img src="刷题笔记.assets/image-20231010221253241.png" alt="image-20231010221253241" style="zoom:50%;" />

Observing the truth table, we can write the following `getBit0` function to obtain `bit0`.

```c++
int getBit0(int bit1, int bit0, int input) {
    if (!bit1) {
        return !input ? bit0 : ~bit0;
    } else {
        return 0;
    }
}
```

`!input ? bit0 : ~bit0` can be simplified using the XOR operation.

```c++
int getBit0(int bit1, int bit0, int input) {
    return !bit1 ? input ^ bit0 : 0;
}
```

`!bit1 ? input ^ bit0 : 0` can also be further simplified using the AND operation.

```c++
int getBit0(int bit1, int bit0, int input) {
    return input ^ bit0 & ~bit1;
}
```

The finite state machine after `bit0` changes is shown in the figure below.

<img src="刷题笔记.assets/image-20231011090156865.png" alt="image-20231011090156865" style="zoom:50%;" />

The finite state machine after swapping the positions of `bit1` and `bit0` is shown in the figure below.

<img src="刷题笔记.assets/image-20231011090907349.png" alt="image-20231011090907349" style="zoom:50%;" />

Now `bit1` needs to be changed according to the changed `bit0`. It can be found that this finite state machine is equivalent to the finite state machine before `bit0` is changed. So the `getBit1` function and the `getBit0` function are very similar.

```c++
int getBit1(int bit1, int bit0, int input) {
    return input ^ bit1 & ~bit0;
}
```

To sum up, the solution to this problem is as follows. This method uses bit operations and is more efficient than the previous method.

```c++
class Solution {
public:
    int singleNumber(vector<int>& nums) {
        int bit1s = 0;
        int bit0s = 0;
        for (int& num: nums) {
            bit0s = bit0s ^ num & ~bit1s;
            bit1s = bit1s ^ num & ~bit0s;
        }
        return bit0s;
    }
};
```

Time Complexity: $O\left(n\right)$, where $n$ is the length of the array.

Space Complexity: $O\left(1\right)$

### Single Number III

https://leetcode.com/problems/single-number-iii

```c++
class Solution {
public:
    vector<int> singleNumber(vector<int>& nums) {
        int res1XORres2 = 0;
        for (int& num: nums) {
            res1XORres2 ^= num;
        }

        int rightMostOne = res1XORres2 == INT_MIN ? res1XORres2 : res1XORres2 & (~res1XORres2 + 1);
        int res1 = 0;
        for (int& num: nums) {
            if (num & rightMostOne) {
                res1 ^= num;
            }
        }

        int res2 = res1XORres2 ^ res1;

        return vector<int>{res1, res2};
    }
};
```

Time Complexity: $O\left(n\right)$, where $n$ is the length of the array.

Space Complexity: $O\left(1\right)$

### Evaluate Reverse Polish Notation

https://leetcode.cn/problems/evaluate-reverse-polish-notation

```c++
class Solution {
public:
    int evalRPN(vector<string>& tokens) {
        stack<int> stk;
        for (string& token: tokens) {
            if (token != "+" && token != "-" && token != "*" && token != "/") {
                stk.emplace(stoi(token));
            } else {
                // Pay attention to the calculation order here.
                int num2 = stk.top();
                stk.pop();
                int num1 = stk.top();
                stk.pop();
                if (token == "+") {
                    stk.emplace(num1 + num2);
                } else if (token == "-") {
                    stk.emplace(num1 - num2);
                } else if (token == "*") {
                    stk.emplace(num1 * num2);
                } else {
                    stk.emplace(num1 / num2);
                }
            }
        }
        return stk.top();
    }
};
```

Time Complexity: $O\left(n\right)$, where $n$ is the length of the array.

Space Complexity: $O\left(n\right)$.

### 分数累加

$$
\frac{1}{a}=\frac{1}{n_1}+\frac{1}{n_2}+\cdots+\frac{1}{n_m}
$$
其中 $n_1$ 至 $n_m$ 单调递增，例如 $a=2,m=2$ 时，$\frac{1}{2}=\frac{1}{3}+\frac{1}{6}$，现给定 $a$ 和 $m$，任意求一组解。

注：假设所有的解都能用 `int` 类型表示。

解答：

令 $n_1=a+1$，则：
$$
\frac{1}{a}=\frac{1}{a+1}+\frac{1}{n_2}+\cdots+\frac{1}{n_m}\\
\frac{1}{a\left(a+1\right)}=\frac{1}{n_2}+\cdots+\frac{1}{n_m}
$$
很明显，这时可以进行迭代求解了。迭代到最后等式右边只有 $n_m$ 这一项：
$$
\frac{1}{a'}=\frac{1}{n_m}
$$
此时 $n_m=a'$。

```c++
#include <bits/stdc++.h>

using namespace std;

int main() {
    int a;
    int m;
    while (cin >> a >> m) {
        int nx = a + 1;
        cout << nx;

        for (int i = 2; i < m; ++i) {
            nx = (nx - 1) * nx + 1;
            cout << ' ' << nx;
        }

        nx = (nx - 1) * nx;
        cout << ' ' << nx << endl;
    }

    return 0;
}
```

时间复杂度：$O\left(m\right)$

空间复杂度：$O\left(1\right)$

# 左程云

# 二进制系统

## 总结

### 位运算

位运算的速度非常快，仅次于赋值操作，常数时间极好

条件判断相比于赋值、位运算、算术运算是稍慢的，如果能将条件判断改为赋值、位运算、算术运算，这是具有现实意义的，但是不需要追求在练算法过程中尽量少写条件判断，那样会带来很多不必要的困扰，还是要写尽量直白、尤其是自己能理解的代码最好

```c++
#include <bits/stdc++.h>
using namespace std;

void printBinary(int num) {
	// 本文件的实现是用 int 来举例的
	// 对于 long long 类型完全同理
	// 不过要注意，如果是 long long 类型的数字 num，有 64 位
	// num & (1 << 48)，这种写法不对
	// 因为 1 是一个 int 类型，只有32位，所以 (1 << 48) 早就溢出了，所以无意义
	// 应该写成 : num & (1L << 48)
	for (int i = 31; i >= 0; i--) {
		cout << ((num & (1 << i)) == 0 ? '0' : '1');
	}
	cout << endl;
}

bool returnTrue() {
	cout << "进入了 returnTrue 函数" << endl;
	return true;
}

bool returnFalse() {
	cout << "进入了 returnFalse 函数" << endl;
	return false;
}

int main() {
	cout << "=== 非负数 ===" << endl;
	int a = 78;
	cout << a << endl;
	printBinary(a);
	cout << "=== 负数 ===" << endl;
	int b = -6;
	cout << b << endl;
	printBinary(b);
	cout << "=== 二进制 ===" << endl;
	int c = 0b1001110;
	cout << c << endl;
	printBinary(c);
	cout << "=== 十六进制 ===" << endl;
	int d = 0x4e;
	cout << d << endl;
	printBinary(d);
	cout << "=== 相反数 ===" << endl;
	cout << a << endl;
	printBinary(a);
	printBinary(~a);
	int e = ~a + 1;
	cout << e << endl;
	printBinary(e);
	cout << "=== 最小值取相反数会溢出，结果不正确 ===" << endl;
	int f = INT_MIN;
	cout << f << endl;
	printBinary(f);
	cout << -f << endl;
	printBinary(-f);
	cout << ~f + 1 << endl;
	printBinary(~f + 1);
	cout << "=== 位运算和逻辑运算的区别，位运算没有逻辑运算的穿透性 ===" << endl;
	int g = 0b0001010;
	int h = 0b0001100;
	printBinary(g | h);
	printBinary(g & h);
	printBinary(g ^ h);
	cout << "test1 测试开始" << endl;
	bool test1 = returnTrue() | returnFalse();
	cout << "test1 结果，" << test1 << endl;
	cout << "test2 测试开始" << endl;
	bool test2 = returnTrue() || returnFalse();
	cout << "test2 结果，" << test2 << endl;
	cout << "test3 测试开始" << endl;
	bool test3 = returnFalse() & returnTrue();
	cout << "test3 结果，" << test3 << endl;
	cout << "test4 测试开始" << endl;
	bool test4 = returnFalse() && returnTrue();
	cout << "test4 结果，" << test4 << endl;
	cout << "=== << ===，可能会移出一个负数" << endl;
	int i = 0b0011010;
	printBinary(i);
	printBinary(i << 1);
	printBinary(i << 2);
	printBinary(i << 3);
	cout << "=== >> ===，补最高位" << endl;
	printBinary(i);
	printBinary(i >> 2);
	int j = 0b11110000000000000000000000000000;
	printBinary(j);
	printBinary(j >> 2);
	cout << "=== 仅对于非负数来说，<<i 等价于乘以 2 的 i 次方，>>i 等价于除以 2 的 i 次方 ===" << endl;
	int k = 10;
	cout << (k << 1) << endl;
	cout << (k >> 1) << endl;
	return 0;
}
```

### 异或

异或运算的性质：

1. 异或运算就是无进位相加
2. 异或运算满足交换律、结合律，也就是同一批数字，不管异或顺序是什么，最终的结果都是一个
3. $x\oplus0=0,\ x\oplus x=0$
4. 整体异或和如果是 $x$，整体中某个部分的异或和如果是 $y$，那么剩下部分的异或和是 $x\oplus y$

这些结论最重要的就是结论 1，所有其他结论都可以由这个结论推论得到，结论 4 相关的题目最多，利用区间上异或和的性质

### 位图

用比特组成的数组来存放值，用比特状态 1 和 0 代表存在和不存在，取值和存值操作都用位运算，限制是必须为连续范围且不能过大，好处是极大的节省空间，因为一个数字只占用一个比特的空间

### 加减乘除

见题目「两数相除」

## 题目

### 黑球的概率

袋子里一共 a 个白球，b 个黑球，每次从袋子里拿 2 个球，每个球每次被拿出机会均等，如果拿出的是 2 个白球或者 2 个黑球，那么就往袋子里重新放入 1 个白球，如果拿出的是 1 个白球和 1 个黑球，那么就往袋子里重新放入 1 个黑球，那么最终袋子里一定会只剩 1 个球，请问最终的球是黑的概率是多少？用 a 和 b 来表达这个概率。

答案：

把白球看成 0，黑球看成 1，这里的操作其实就是在求 a 个白球和 b 个黑球的异或和，黑球的数量如果是偶数，最终的球是黑的概率是 0%；黑球的数量如果是奇数，最终的球是黑的概率是 100%

### 使用异或交换两个数

```c++
#include <bits/stdc++.h>
using namespace std;

// i!=j 时，可以正常交换，i==j 时，会导致 nums[i]==0
void swapByXOR(vector<int>& nums, int i, int j) {
    nums[i] = nums[i] ^ nums[j];
    nums[j] = nums[i] ^ nums[j];
    nums[i] = nums[i] ^ nums[j];
}

int main() {
    vector<int> nums = {3, 5};
    swapByXOR(nums, 0, 1);
    cout << nums[0] << ' ' << nums[1] << endl; // 5 3
    swapByXOR(nums, 0, 0);
    cout << nums[0] << ' ' << nums[1] << endl; // 0 3
    return 0;
}
```

时间复杂度：$O\left(1\right)$

空间复杂度：$O\left(1\right)$

### 获取最大值

不使用判断语句和比较运算符，返回两个数中较大的数

https://www.nowcoder.com/practice/d2707eaf98124f1e8f1d9c18ad487f76

```c++
class Solution {
public:
    int getMax(int x, int y) {
        int sx = sign(x);
        int sy = sign(y);
        int z = x - y; // 可能会溢出，但是 returnX 考虑了这种情况
        int sz = sign(z);
        int diffXY = sx ^ sy;
        int sameXY = flip(diffXY);
        int returnX = diffXY * sx + sameXY * sz; // diffXY 和 sameXY 一个为 1 一个为 0
        int returnY = flip(returnX);
        return returnX * x + returnY * y; // returnX 和 returnY 一个为 1 一个为 0
    }

    // 非负数返回 1，负数返回 0
    int sign(int x) {
        return flip((x >> 31) & 1);
    }

    // 0->1，1->0
    int flip(int x) {
        return x ^ 1;
    }
};
```

时间复杂度：$O\left(1\right)$

空间复杂度：$O\left(1\right)$

### 丢失的数字

https://leetcode.cn/problems/missing-number

```c++
class Solution {
public:
    int missingNumber(vector<int>& nums) {
        int n = nums.size(), xorAll = 0, xorCurr = 0;
        for (int i = 0; i < n; ++i) {
            xorAll ^= i;
            xorCurr ^= nums[i];
        }
        xorAll ^= n;
        return xorAll ^ xorCurr;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 只出现一次的数字

https://leetcode.cn/problems/single-number

```c++
class Solution {
public:
    int singleNumber(vector<int>& nums) {
        int res = 0;
        for (int& num: nums) {
            res ^= num;
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 只出现一次的数字 III

https://leetcode.cn/problems/single-number-iii

```c++
class Solution {
public:
    // 数组中两种数出现了奇数次，其他的数都出现了偶数次，返回这两种出现了奇数次的数
    vector<int> singleNumber(vector<int>& nums) {
        int xorAll = 0;
        for (int& num: nums) {
            xorAll ^= num;
        }
        // Brian Kernighan 算法
        int rightMostOne = xorAll != INT_MIN ? xorAll & (-xorAll) : xorAll, xorZeros = 0;
        for (int& num: nums) {
            if (!(num & rightMostOne)) {
                xorZeros ^= num;
            }
        }
        return vector<int>{xorZeros, xorAll ^ xorZeros};
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 只出现一次的数字 II

https://leetcode.cn/problems/single-number-ii

```c++
class Solution {
public:
    // 数组中只有一种数出现次数少于 m 次，其他数都出现了 m 次，返回出现次数小于 m 次的那种数
    int singleNumber(vector<int>& nums, int m = 3) {
        int cnts[32] = {}; // cnts[i]：第 i 位上有多少个 1
        for (int& num: nums) {
            for (int i = 0; i < 32; ++i) {
                cnts[i] += (num >> i) & 1;
            }
        }
        int res = 0;
        for (int i = 0; i < 32; ++i) {
            if (cnts[i] % m) {
                res |= 1 << i;
            }
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 2 的幂

https://leetcode.cn/problems/power-of-two

```c++
class Solution {
public:
    bool isPowerOfTwo(int n) {
        return n > 0 && n == (n & -n);
    }
};
```

时间复杂度：$O\left(1\right)$

空间复杂度：$O\left(1\right)$

### 3 的幂

https://leetcode.cn/problems/power-of-three

```c++
class Solution {
public:
    // 如果一个数字是 3 的某次幂，那么这个数一定只含有 3 这个质数因子
    // 1162261467 是 int 型范围内，最大的 3 的幂，它是 3^19
    // 1162261467 只含有 3 这个质数因子，如果 n 也是只含有3这个质数因子，那么 1162261467%n==0
    // 反之如果 1162261467%n!=0 说明 n 一定含有其他因子
    bool isPowerOfThree(int n) {
        return n > 0 && !(1162261467 % n);
    }
};
```

时间复杂度：$O\left(1\right)$

空间复杂度：$O\left(1\right)$

### 大于等于 n 最小的 2 的幂

返回大于等于 n 最小的 2 的幂，无测试链接

```c++
#include <bits/stdc++.h>
using namespace std;

int nearTwoPower(int x) {
    if (x <= 0) {
        return 1;
    }
    --x;
    // 下面 5 行的作用是把最左边 1 右边的所有位置都变为 1
    x |= x >> 1;
    x |= x >> 2;
    x |= x >> 4;
    x |= x >> 8;
    x |= x >> 16;
    ++x;
    return x;
}

int main() {
    int x = 100;
    cout << nearTwoPower(x) << endl;
    return 0;
}
```

时间复杂度：$O\left(1\right)$

空间复杂度：$O\left(1\right)$

### 数字范围按位与

https://leetcode.cn/problems/bitwise-and-of-numbers-range

```c++
class Solution {
public:
    int rangeBitwiseAnd(int left, int right) {
        while (left < right) {
            right -= right & -right;
        }
        return right;
    }
};
```

时间复杂度：$O\left(1\right)$

空间复杂度：$O\left(1\right)$

### 颠倒二进制位

https://leetcode.cn/problems/reverse-bits

https://www.bilibili.com/video/BV1ch4y1Q7vd

```c++
class Solution {
public:
    uint32_t reverseBits(uint32_t n) {
        n = ((n & 0xaaaaaaaa) >> 1) | ((n & 0x55555555) << 1);
        n = ((n & 0xcccccccc) >> 2) | ((n & 0x33333333) << 2);
        n = ((n & 0xf0f0f0f0) >> 4) | ((n & 0x0f0f0f0f) << 4);
        n = ((n & 0xff00ff00) >> 8) | ((n & 0x00ff00ff) << 8);
        n = (n >> 16) | (n << 16);
        return n;
    }
};
```

时间复杂度：$O\left(1\right)$

空间复杂度：$O\left(1\right)$

### 汉明距离

https://leetcode.cn/problems/hamming-distance

https://www.bilibili.com/video/BV1ch4y1Q7vd

```c++
class Solution {
public:
    int hammingDistance(int x, int y) {
        return countOnes(x ^ y);
    }

    int countOnes(int n) {
        n = (n & 0x55555555) + ((n >> 1) & 0x55555555);
        n = (n & 0x33333333) + ((n >> 2) & 0x33333333);
        n = (n & 0x0f0f0f0f) + ((n >> 4) & 0x0f0f0f0f);
        n = (n & 0x00ff00ff) + ((n >> 8) & 0x00ff00ff);
        n = (n & 0x0000ffff) + ((n >> 16) & 0x0000ffff);
        return n;
    }
};
```

时间复杂度：$O\left(1\right)$

空间复杂度：$O\left(1\right)$

### 设计位集

https://leetcode.cn/problems/design-bitset

```c++
class Bitset {
private:
    vector<int> set;
    int size;
    int numZeros;
    int numOnes;
    bool reverse; // reverse 存在的意义是 flip 方法不用遍历

public:
    Bitset(int n) {
        // 支持的数字范围为 [0,n-1]
        // a/b 向上取整可以写成 (a+b-1)/b
        set.resize((n + 31) / 32);
        size = n;
        numZeros = n;
        numOnes = 0;
        reverse = false;
    }

    void fix(int x) {
        int numIdx = x / 32, bitIdx = x % 32;
        if (!reverse) {
            if (!(set[numIdx] & (1 << bitIdx))) {
                --numZeros;
                ++numOnes;
                set[numIdx] |= 1 << bitIdx;
            }
        } else {
            if (set[numIdx] & (1 << bitIdx)) {
                --numZeros;
                ++numOnes;
                set[numIdx] ^= 1 << bitIdx;
            }
        }
    }

    void unfix(int x) {
        int numIdx = x / 32, bitIdx = x % 32;
        if (!reverse) {
            if (set[numIdx] & (1 << bitIdx)) {
                ++numZeros;
                --numOnes;
                set[numIdx] ^= 1 << bitIdx;
            }
        } else {
            if (!(set[numIdx] & (1 << bitIdx))) {
                ++numZeros;
                --numOnes;
                set[numIdx] |= 1 << bitIdx;
            }
        }
    }

    void flip() {
        reverse = !reverse;
        swap(numZeros, numOnes);
    }

    bool all() {
        return numOnes == size;
    }

    bool one() {
        return numOnes;
    }

    int count() {
        return numOnes;
    }

    string toString() {
        string res;
        for (int charIdx = 0, numIdx = 0, bit; numIdx < size; ++numIdx) {
            for (int bitIdx = 0; bitIdx < 32 && charIdx < size; ++charIdx, ++bitIdx) {
                bit = (set[numIdx] >> bitIdx) & 1;
                bit ^= !reverse ? 0 : 1;
                res.push_back(bit + '0');
            }
        }
        return res;
    }
};
```

对于 `BitSet` 中各个函数的方法：

时间复杂度：$O\left(1\right)$

空间复杂度：$O\left(1\right)$

### 两数相除

https://leetcode.cn/problems/divide-two-integers

```c++
class Solution {
public:
    int add(int x, int y) {
        int res = x;
        while (y) {
            res = x ^ y; // 无进位相加
            y = (x & y) << 1; // 进位信息
            x = res;
        }
        return res;
    }

    int minus(int x, int y) {
        return add(x, neg(y));
    }

    // 回想小学时候怎么学的乘法
    int multiply(int x, int y) {
        int res = 0;
        while (y) {
            if (y & 1) {
                res = add(res, x);
            }
            x <<= 1;
            y = (unsigned) y >> 1;
        }
        return res;
    }

    int divide(int x, int y) {
        if (x == INT_MIN && y == INT_MIN) {
            return 1;
        }
        if (x != INT_MIN && y != INT_MIN) {
            return div(x, y);
        }
        if (y == INT_MIN) {
            return 0;
        }
        // x 是 INT_MIN
        if (y == neg(1)) {
            return INT_MAX;
        }
        // x 是 INT_MIN 且 b 不是 -1
        x = add(x, y > 0 ? y : neg(y));
        int res = div(x, y);
        int offset = y > 0 ? neg(1) : 1;
        return add(res, offset);
    }

    // x 和 y 都不是 INT_MIN
    int div(int x, int y) {
        int isNeg = x < 0 ^ y < 0;
        x = x < 0 ? neg(x) : x;
        y = y < 0 ? neg(y) : y;
        int res = 0;
        for (int i = 30; i >= 0; i = minus(i, 1)) {
            if ((x >> i) >= y) { // 为了防止溢出，让被除数右移，而不是除数左移
                res |= 1 << i;
                x = minus(x, y << i);
            }
        }
        return isNeg ? neg(res) : res;
    }

    int neg(int x) {
        return add(~x, 1);
    }
};
```

时间复杂度：$O\left(1\right)$

空间复杂度：$O\left(1\right)$

# 最大公约数、同余原理

## 总结

- 最大公约数的证明不必掌握
- 求 `gcd(a, b)`，其中 `a > b`，时间复杂度为 $O\left(\log^3 a\right)$，时间复杂度证明不必掌握
- 同余原理的背景：
  - 题目要求对结果取模
  - 如果每一步算真实结果到最后一步再取模，中间结果的位数可能会很长导致运行时间很长
- 加法和乘法每一步计算完后直接取模，减法则为 `(a - b + m) % m`
- 要确保计算过程中不溢出，所以往往运算中途使用 `long long` 类型

## 题目

### 第 N 个神奇数字

https://leetcode.cn/problems/nth-magical-number

```c++
class Solution {
public:
    typedef long long ll;

    int nthMagicalNumber(int n, int a, int b) {
        ll lcm = getLCM(a, b);
        ll res = 0;
        for (ll l = 1, r = (ll) n * min(a, b), m; l <= r;) { // 二分答案
            m = l + ((r - l) >> 1);
            if (m / a + m / b - m / lcm >= n) { // [1...m] 上有多少个神奇数字
                res = m;
                r = m - 1;
            } else {
                l = m + 1;
            }
        }
        return res % 1000000007;
    }

    ll getGCD(ll x, ll y) {
        return !y ? x : getGCD(y, x % y);
    }

    ll getLCM(ll x, ll y) {
        return x / getGCD(x, y) * y;
    }
};
```

时间复杂度：$O\left(\log\left(n\times\min\left(a,b\right)\right)\right)$

空间复杂度：$O\left(1\right)$

### 用同余原理对表达式求值

计算 `((a + b) * (c - d) + (a * c - b * d)) % mod` 的非负结果

```c++
#include <bits/stdc++.h>
using namespace std;

#define ll long long

// 计算 ((a + b) * (c - d) + (a * c - b * d)) % mod 的非负结果
int f1(int a, int b, int c, int d, int mod) {
    ll o1 = a;
    ll o2 = b;
    ll o3 = c;
    ll o4 = d;
    ll o5 = o1 + o2;
    ll o6 = o3 - o4;
    ll o7 = a * c;
    ll o8 = b * d;
    ll o9 = o5 * o6;
    ll o10 = o7 - o8;
    ll o11 = o9 + o10;
    int ans = o11 % mod;
    if (ans < 0) {
        ans += mod;
    }
    return ans;
}

// 计算 ((a + b) * (c - d) + (a * c - b * d)) % mod 的非负结果
int f2(int a, int b, int c, int d, int mod) {
    int o1 = a % mod;
    int o2 = b % mod;
    int o3 = c % mod;
    int o4 = d % mod;
    int o5 = ((ll) o1 + (ll) o2) % (ll) mod;
    int o6 = ((ll) o3 - (ll) o4 + (ll) mod) % (ll) mod;
    int o7 = ((ll) o1 * (ll) o3) % (ll) mod;
    int o8 = ((ll) o2 * (ll) o4) % (ll) mod;
    int o9 = ((ll) o5 * (ll) o6) % (ll) mod;
    int o10 = ((ll) o7 - (ll) o8 + (ll) mod) % (ll) mod;
    int ans = ((ll) o9 + (ll) o10) % (ll) mod;
    return ans;
}

int main() {
    cout << "测试开始" << endl;
    int testTime = 100000;
    srand(time(nullptr));

    for (int i = 0; i < testTime; i++) {
        int mod = 1000000007;
        int a = rand();
        int b = rand();
        int c = rand();
        int d = rand();
        if (f1(a, b, c, d, mod) != f2(a, b, c, d, mod)) {
            cout << "出错了!" << endl;
        }
    }
    cout << "测试结束" << endl;
    return 0;
}
```

时间复杂度：$O\left(1\right)$

空间复杂度：$O\left(1\right)$

# 排序

## 总结

### 归并排序比排序三傻快的原因

**比较行为没有浪费。**排序三傻每轮 $O\left(n\right)$ 次比较只确定了一个数的位置，大量的比较行为浪费掉了，而归并排序把比较行为变成了有序的子数组，比较行为没有浪费。

### 归并分治

1. 思考一个问题在大范围上的答案，是否等于左部分的答案 + 右部分的答案 + 跨越左右产生的答案
2. 计算跨越左右产生的答案时，如果加上左右各自有序这个设定，会不会获得计算的便利性
3. 如果以上两点都成立，那么该问题很可能被归并分治解决
4. 求解答案的过程中只需要加入归并排序的过程即可，因为要让左、右各自有序，来获得计算的便利性
5. 一些用归并分治解决的问题，往往也可以用线段树、树状数组等解法

### 堆

1. 堆由完全二叉树实现，大/小根堆是指完全二叉树中任何一棵子树的根节点是这棵子树中的最大/小值，如果当前节点在数组中的下标为 `i`，那么该节点的父节点、左孩子节点和右孩子节点的下标分别为 `(i - 1) / 2`、`(i << 1) + 1` 和 `(i << 1) + 2`，注意父节点不要写成 `(i - 1) >> 1`，因为结果可能是 `-1`
2. 堆的核心方法为 `heapInsert` 和 `heapify`，两者时间复杂度都是 $O\left(n\log n\right)$
3. 建堆时，从顶到底（不推荐）和从底到顶（推荐）建堆的时间复杂度分别为 $O\left(n\log n\right)$ 和 $O\left(n\right)$
4. 使用增倍分析法分析从顶到底建堆的时间复杂度：对于长度为 $n$ 的数组，要将其转化为堆，假设每次 `heapInsert` 操作的时间复杂度都是 $O\left(\log n\right)$，那么时间复杂度的上限是 $O\left(n\log n\right)$；对于长度为 $2n$ 的数组，要将其转化为堆，因为对于数组的后半部分来说，时间复杂度的下限是 $O\left(n\log n\right)$，所以整个数组转化为堆的时间复杂度的下限也是 $O\left(n\log n\right)$，因为时间复杂度不考虑常数，长度为 $n$ 和 $2n$ 的数组转化为堆的时间复杂度必须是相同的，而这两者时间复杂度的上限和下限都是 $O\left(n\log n\right)$，所以对于长度为 $n$ 的数组，要将其转化为堆，时间复杂度只能是 $O\left(n\log n\right)$

### 各排序算法对比

|              | 时间复杂度              | 空间复杂度             | 稳定性 | 备注             |
| ------------ | ----------------------- | ---------------------- | ------ | ---------------- |
| 选择排序     | $O\left(n^2\right)$     | $O\left(1\right)$      | N      |                  |
| 冒泡排序     | $O\left(n^2\right)$     | $O\left(1\right)$      | Y      |                  |
| 插入排序     | $O\left(n^2\right)$     | $O\left(1\right)$      | Y      | $n$ 小的时候很快 |
| 归并排序     | $O\left(n\log n\right)$ | $O\left(n\right)$      | Y      |                  |
| 随机快速排序 | $O\left(n\log n\right)$ | $O\left(\log n\right)$ | N      |                  |
| 堆排序       | $O\left(n\log n\right)$ | $O\left(1\right)$      | N      |                  |
| 计数排序     | $O\left(k+n\right)$     | $O\left(k+n\right)$    | Y      | 整数且范围比较窄 |
| 基数排序     | $O\left(kn\right)$      | $O\left(k+n\right)$    | Y      | 非负整数         |

- 计数排序和基数排序是和比较无关的排序，对于对象的数据特征有要求，并不通用
- 基于比较的排序，时间复杂度 $O\left(n\log n\right)$，空间复杂度低于 $O\left(n\right)$，还具有稳定性的排序算法目前没有找到
- 数据量非常小的情况下可以做到非常迅速：插入排序
- 性能优异、实现简单且利于改进（面对不同业务可以选择不同划分策略）、不在乎稳定性：随机快速排序
- 性能优异、不在乎额外空间占用、具有稳定性：归并排序
- 性能优异、额外空间占用要求 $O\left(1\right)$、不在乎稳定性：堆排序

## 题目

### 选择排序

https://leetcode.cn/problems/sort-an-array

`[i,n-1]` 范围上，找到最小值并放在 `i` 位置，然后 `[i+1,n-1]` 范围上继续

```c++
class Solution {
public:
	vector<int> sortArray(vector<int>& nums) {
		int n = nums.size();
		if (n < 2) {
			return nums;
		}
		int minIdx;
		for (int i = 0; i < n - 1; ++i) {
			minIdx = i;
			for (int j = i + 1; j < n; ++j) {
				if (nums[j] < nums[minIdx]) {
					minIdx = j;
				}
			}
			swap(nums[i], nums[minIdx]);
		}
		return nums;
	}
};
```

时间复杂度：$O\left(n^2\right)$

空间复杂度：$O\left(1\right)$

### 冒泡排序

https://leetcode.cn/problems/sort-an-array

`[0,j]` 范围上，相邻位置较大的数滚下去，最大值最终来到 `j` 位置，然后 `[0,j-1]` 范围上继续

```c++
class Solution {
public:
	vector<int> sortArray(vector<int>& nums) {
		int n = nums.size();
		if (n < 2) {
			return nums;
		}
		for (int j = n - 1; j > 0; --j) {
			for (int i = 0; i < j; ++i) {
				if (nums[i] > nums[i + 1]) {
					swap(nums[i], nums[i + 1]);
				}
			}
		}
		return nums;
	}
};
```

时间复杂度：$O\left(n^2\right)$

空间复杂度：$O\left(1\right)$

### 插入排序

https://leetcode.cn/problems/sort-an-array

`[0,i]` 范围上已经有序，新来的数从右到左滑到不再小的位置插入，然后继续

```c++
class Solution {
public:
	vector<int> sortArray(vector<int>& nums) {
		int n = nums.size();
		if (n < 2) {
			return nums;
		}
		for (int j = 1; j < n; ++j) {
			for (int i = j - 1; i >= 0 && nums[i] > nums[i + 1]; --i) {
				swap(nums[i], nums[i + 1]);
			}
		}
		return nums;
	}
};
```

时间复杂度：$O\left(n^2\right)$

空间复杂度：$O\left(1\right)$

### 归并排序

https://leetcode.cn/problems/sort-an-array

递归版：

```c++
class Solution {
private:
    int MAXN;
    vector<int> temp;

public:
    Solution() {
        MAXN = 50000;
        temp.resize(MAXN);
    }

    vector<int> sortArray(vector<int>& nums) {
        int n = nums.size();
        mergeSort(nums, 0, n - 1);
        return nums;
    }

    void mergeSort(vector<int>& nums, int l, int r) {
        if (l == r) {
            return;
        }
        int m = l + ((r - l) >> 1);
        mergeSort(nums, l, m);
        mergeSort(nums, m + 1, r);
        merge(nums, l, m, r);
    }

    void merge(vector<int>& nums, int l, int m, int r) {
        int i = l, j = m + 1, k = l;
        while (i <= m && j <= r) {
            temp[k++] = nums[i] <= nums[j] ? nums[i++] : nums[j++];
        }
        while (i <= m) {
            temp[k++] = nums[i++];
        }
        while (j <= r) {
            temp[k++] = nums[j++];
        }
        for (k = l; k <= r; ++k) {
            nums[k] = temp[k];
        }
    }
};
```

时间复杂度：由 master 公式得 $O\left(n\log n\right)$

空间复杂度：$O\left(n\right)$

非递归版：

```c++
class Solution {
private:
    int MAXN;
    vector<int> temp;

public:
    Solution() {
        MAXN = 50000;
        temp.resize(MAXN);
    }

    vector<int> sortArray(vector<int>& nums) {
        int n = nums.size();
        for (int l, m, r, step = 1; step < n; step <<= 1) {
            l = 0;
            while (l < n) {
                m = l + step - 1;
                if (m + 1 >= n) { // 没有右侧
                    break;
                }
                r = min(l + (step << 1) - 1, n - 1);
                merge(nums, l, m, r);
                l = r + 1;
            }
        }
        return nums;
    }

    void merge(vector<int>& nums, int l, int m, int r) {
        int i = l, j = m + 1, k = l;
        while (i <= m && j <= r) {
            temp[k++] = nums[i] <= nums[j] ? nums[i++] : nums[j++];
        }
        while (i <= m) {
            temp[k++] = nums[i++];
        }
        while (j <= r) {
            temp[k++] = nums[j++];
        }
        for (k = l; k <= r; ++k) {
            nums[k] = temp[k];
        }
    }
};
```

时间复杂度：$O\left(n\log n\right)$

空间复杂度：$O\left(n\right)$

### 计算数组的小和

https://www.nowcoder.com/practice/edfe05a1d45c4ea89101d936cac32469

```c++
#include <bits/stdc++.h>
using namespace std;

int MAXN = 100000, n;
vector<int> nums(MAXN), temp(MAXN);

long long merge(int l, int m, int r) {
    // 统计
    int i = l, j = m + 1, sum = 0;
    long long res = 0;
    for (; j <= r; ++j) {
        while (i <= m && nums[i] <= nums[j]) {
            sum += nums[i++];
        }
        res += sum;
    }
    // 排序
    i = l, j = m + 1;
    int k = l;
    while (i <= m && j <= r) {
        temp[k++] = nums[i] <= nums[j] ? nums[i++] : nums[j++];
    }
    while (i <= m) {
        temp[k++] = nums[i++];
    }
    while (j <= r) {
        temp[k++] = nums[j++];
    }
    for (k = l; k <= r; ++k) {
        nums[k] = temp[k];
    }
    return res;
}

// 除了求小和之外，还需要将 [l,r] 上的元素变有序
long long smallSum(int l, int r) {
    if (l == r) {
        return 0;
    }
    int m = l + ((r - l) >> 1);
    return smallSum(l, m) + smallSum(m + 1, r) + merge(l, m, r);
}

int main() {
    while (cin >> n) {
        for (int i = 0; i < n; ++i) {
            cin >> nums[i];
        }
        cout << smallSum(0, n - 1) << endl;
    }
    return 0;
}
```

时间复杂度：$O\left(n\log n\right)$

空间复杂度：$O\left(n\right)$

### 翻转对

https://leetcode.cn/problems/reverse-pairs

```c++
class Solution {
private:
    int MAXN;
    vector<int> temp;

public:
    Solution() {
        MAXN = 50000;
        temp.resize(MAXN);
    }

    int reversePairs(vector<int>& nums) {
        int n = nums.size();
        return f(nums, 0, n - 1);
    }

    int f(vector<int>& nums, int l, int r) {
        if (l == r) {
            return 0;
        }
        int m = l + ((r - l) >> 1);
        return f(nums, l, m) + f(nums, m + 1, r) + merge(nums, l, m, r);
    }

    int merge(vector<int>& nums, int l, int m, int r) {
        // 统计
        int i = l, j = m + 1, num = 0, res = 0;
        for (; j <= r; ++j) {
            while (i <= m && nums[i] > ((long long) nums[j] << 1)) {
                ++num;
                ++i;
            }
            res += num;
        }
        // 排序
        i = l, j = m + 1;
        int k = l;
        while (i <= m && j <= r) {
            temp[k++] = nums[i] >= nums[j] ? nums[i++] : nums[j++];
        }
        while (i <= m) {
            temp[k++] = nums[i++];
        }
        while (j <= r) {
            temp[k++] = nums[j++];
        }
        for (k = l; k <= r; ++k) {
            nums[k] = temp[k];
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\log n\right)$

空间复杂度：$O\left(n\right)$

### 快速排序

https://leetcode.cn/problems/sort-an-array

原始版：

```c++
class Solution {
public:
    vector<int> sortArray(vector<int>& nums) {
        int n = nums.size();
        srand(time(nullptr));
        quickSort(nums, 0, n - 1);
        return nums;
    }

    void quickSort(vector<int>& nums, int l, int r) {
        if (l >= r) {
            return;
        }
        int x = nums[l + rand() % (r - l + 1)];
        int m = partition(nums, l, r, x);
        quickSort(nums, l, m - 1);
        quickSort(nums, m + 1, r);
    }

    int partition(vector<int>& nums, int l, int r, int x) {
        int i = l, xi;
        for (int j = l; j <= r; ++j) {
            if (nums[j] <= x) {
                swap(nums[i], nums[j]);
                if (nums[i] == x) {
                    xi = i;
                }
                ++i;
            }
        }
        swap(nums[xi], nums[i - 1]);
        return i - 1;
    }
};
```

时间复杂度：根据 Master 公式得 $O\left(n\log n\right)$，定量证明略，因为证明较为复杂

空间复杂度：$O\left(\log n\right)$

荷兰国旗优化版：

```c++
class Solution {
public:
    vector<int> sortArray(vector<int>& nums) {
        int n = nums.size();
        quickSort(nums, 0, n - 1);
        return nums;
    }

    void quickSort(vector<int>& nums, int l, int r) {
        if (l >= r) {
            return;
        }
        srand(time(nullptr));
        int x = nums[l + rand() % (r - l + 1)];
        vector<int> bounds = partition(nums, l, r, x);
        quickSort(nums, l, bounds[0] - 1);
        quickSort(nums, bounds[1] + 1, r);
    }

    vector<int> partition(vector<int>& nums, int l, int r, int x) {
        int i = l, j = l, k = r;
        while (j <= k) {
            if (nums[j] < x) {
                swap(nums[i++], nums[j++]);
            } else if (nums[j] > x) {
                swap(nums[k--], nums[j]);
            } else {
                ++j;
            }
        }
        return vector<int>{i, k};
    }
};
```

时间复杂度：$O\left(n\log n\right)$

空间复杂度：$O\left(\log n\right)$

### 数组中的第 K 个最大元素

https://leetcode.cn/problems/kth-largest-element-in-an-array

```c++
class Solution {
private:
    void buildHeap(vector<int>& nums) {
        for (int i = (nums.size() >> 1) - 1; i >= 0; --i) {
            heapify(nums, nums.size(), i);
        }
    }

    void heapify(vector<int>& nums, int size, int i) {
        int l = (i << 1) + 1;
        int besti = -1;
        while (l < size) {
            besti = l + 1 < size && nums[l + 1] > nums[l] ? l + 1 : l;
            besti = nums[besti] > nums[i] ? besti : i;
            if (besti == i) {
                break;
            }
            swap(nums[i], nums[besti]);
            i = besti;
            l = (i << 1) + 1;
        }
    }

public:
    int findKthLargest(vector<int>& nums, int k) {
        buildHeap(nums);
        int size = nums.size();
        for (int i = 0; i < k - 1; ++i) {
            swap(nums[0], nums[--size]);
            heapify(nums, size, 0);
        }
        return nums[0];
    }
};
```

时间复杂度：$O\left(n\right)+O\left(n\log n\right)=O\left(n\log n\right)$

空间复杂度：$O\left(1\right)$

```c++
class Solution {
private:
    // 返回若数组有序 i 处的值是什么
    int randomizedSelect(vector<int>& nums, int i) {
        int l = 0;
        int r = nums.size() - 1;
        int bounds[2] = {};
        int res = -10001;
        srand(time(nullptr));
        while (l <= r) {
            partition(nums, l, r, nums[l + rand() % (r - l + 1)], bounds);
            if (i < bounds[0]) {
                r = bounds[0] - 1;
            } else if (i > bounds[1]) {
                l = bounds[1] + 1;
            } else {
                res = nums[i];
                break;
            }
        }
        return res;
    }

    void partition(vector<int>& nums, int l, int r, int x, int bounds[]) {
        // [l,i)：小于区
        // [i,j)：等于区
        // [j,k]：待看区
        // (k,r]：大于区
        int i = l;
        int j = l;
        int k = r;
        while (j <= k) {
            if (nums[j] < x) {
                swap(nums[i++], nums[j++]);
            } else if (nums[j] > x) {
                swap(nums[j], nums[k--]);
            } else {
                ++j;
            }
        }
        bounds[0] = i;
        bounds[1] = k;
    }

public:
    int findKthLargest(vector<int>& nums, int k) {
        return randomizedSelect(nums, nums.size() - k);
    }
};
```

时间复杂度：$O\left(\log n\right)$，定量证明略

空间复杂度：$O\left(1\right)$

### 堆排序

https://leetcode.cn/problems/sort-an-array

```c++
class Solution {
public:
    vector<int> sortArray(vector<int>& nums) {
        int n = nums.size();
        // 建堆
        // 方法一，从顶到底（不推荐）：
        // int size = 0;
        // while (size < n) {
        //     heapInsert(nums, size++);
        // }
        // 方法二，从底到顶（推荐）：
        for (int i = n - 1; i >= 0; --i) {
            heapify(nums, n, i);
        }
        int size = n;
        // 排序
        while (size > 1) {
            swap(nums[0], nums[--size]);
            heapify(nums, size, 0);
        }
        return nums;
    }

    void heapInsert(vector<int>& nums, int idx) {
        int fatherIdx = (idx - 1) / 2;
        while (nums[idx] > nums[fatherIdx]) {
            swap(nums[idx], nums[fatherIdx]);
            idx = fatherIdx;
            fatherIdx = (idx - 1) / 2;
        }
    }

    void heapify(vector<int>& nums, int size, int idx) {
        int l = (idx << 1) + 1, bestIdx;
        while (l < size) {
            bestIdx = l + 1 < size && nums[l + 1] > nums[l] ? l + 1 : l;
            bestIdx = nums[bestIdx] > nums[idx] ? bestIdx : idx;
            if (bestIdx == idx) {
                break;
            }
            swap(nums[idx], nums[bestIdx]);
            idx = bestIdx;
            l = (idx << 1) + 1;
        }
    }
};
```

时间复杂度：$O\left(n\log n\right)$

空间复杂度：$O\left(1\right)$

### 计数排序

https://leetcode.cn/problems/sort-an-array

```c++
class Solution {
public:
    vector<int> sortArray(vector<int>& nums) {
        int minNum = INT_MAX, maxNum = INT_MIN;
        for (int& num: nums) {
            minNum = min(minNum, num);
            maxNum = max(maxNum, num);
        }
        vector<int> cnts(maxNum - minNum + 1);
        for (int& num: nums) {
            ++cnts[num - minNum]; // 在数组上划分每个元素所在的区域
        }
        int n = nums.size(), cntsSize = cnts.size();
        for (int i = 1; i < cntsSize; ++i) {
            cnts[i] += cnts[i - 1];
        }
        vector<int> temp(n);
        for (int i = n - 1; i >= 0; --i) {
            temp[--cnts[nums[i] - minNum]] = nums[i];
        }
        for (int i = 0; i < n; ++i) {
            nums[i] = temp[i];
        }
        return nums;
    }
};
```

时间复杂度：$O\left(k+n\right)$，$k$ 为数组中的最大值减最小值

空间复杂度：$O\left(k\right)$

### 基数排序

https://leetcode.cn/problems/sort-an-array

```c++
class Solution {
private:
    int base = 10; // 进制

public:
    vector<int> sortArray(vector<int>& nums) {
        int minNum = INT_MAX, maxNum = INT_MIN;
        for (int& num: nums) {
            minNum = min(minNum, num);
        }
        for (int& num: nums) {
            num -= minNum;
            maxNum = max(maxNum, num); // 基数排序只能处理非负数数组
        }
        radixSort(nums, getNumBits(maxNum));
        for (int& num: nums) {
            num += minNum;
        }
        return nums;
    }

    int getNumBits(int num) {
        int res = 0;
        while (num) {
            num /= base;
            ++res;
        }
        return res;
    }

    void radixSort(vector<int>& nums, int numBits) {
        int n = nums.size();
        vector<int> temp(n), cnts(base);
        for (int bitIdx = 0, offset = 1; bitIdx < numBits; ++bitIdx, offset *= base) {
            fill(cnts.begin(), cnts.end(), 0);
            for (int& num: nums) {
                ++cnts[(num / offset) % base];
            }
            for (int i = 1; i < base; ++i) {
                cnts[i] += cnts[i - 1];
            }
            for (int i = n - 1; i >= 0; --i) {
                temp[--cnts[(nums[i] / offset) % base]] = nums[i];
            }
            for (int i = 0; i < n; ++i) {
                nums[i] = temp[i];
            }
        }
    }
};
```

时间复杂度：$O\left(kn\right)$，$k$ 为最大数字的位数

空间复杂度：$O\left(k+n\right)$

# 二分搜索

## 题目

### 有序数组中找 `>=target` 的最左位置

```c++
#include <bits/stdc++.h>
using namespace std;

int compare(const vector<int>& nums, int target) {
    int n = nums.size();
    for (int i = 0; i < n; ++i) {
        if (nums[i] >= target) {
            return i;
        }
    }
    return -1;
}

vector<int> getRandomArray(int n, int minNum, int maxNum) {
    vector<int> nums(n);
    for (int i = 0; i < n; i++) {
        nums[i] = rand() % (maxNum - minNum + 1) + minNum;
    }
    return nums;
}

int getLeft(vector<int>& nums, int target) {
    int n = nums.size(), l = 0, r = n - 1, m, res = -1;
    while (l <= r) {
        m = l + ((r - l) >> 1);
        if (nums[m] >= target) {
            r = m - 1;
            res = m;
        } else {
            l = m + 1;
        }
    }
    return res;
}

int main() {
    int maxN = 100, minNum = -1000, maxNum = 1000, numTests = 500000, isSuccess = true;
    cout << "测试开始" << endl;
    for (int i = 0, n, target; i < numTests; i++) {
        n = rand() % maxN;
        target = rand() % (maxNum - minNum + 1) + minNum;
        vector<int> nums = getRandomArray(n, maxNum, minNum);
        sort(nums.begin(), nums.end());
        if (compare(nums, target) != getLeft(nums, target)) {
            isSuccess = false;
            break;
        }
    }
    cout << (isSuccess ? "通过测试" : "未通过测试") << endl;
    return 0;
}
```

时间复杂度：$O\left(\log n\right)$

空间复杂度：$O\left(1\right)$

### 寻找峰值

https://leetcode.cn/problems/find-peak-element

二分搜索不一定发生在有序数组上

```c++
class Solution {
public:
    int findPeakElement(vector<int>& nums) {
        int n = nums.size();
        if (n == 1) {
            return 0;
        }
        if (nums[0] > nums[1]) {
            return 0;
        }
        if (nums[n - 1] > nums[n - 2]) {
            return n - 1;
        }
        // 如果 nums[i]>nums[i-1]&&nums[j+1]>nums[j]，即最前面上升最后面下降，那么 [i,j] 上一定存在峰值
        int l = 1, r = n - 2, m, res = -1;
        while (l <= r) {
            m = l + ((r - l) >> 1);
            if (nums[m - 1] > nums[m]) {
                r = m - 1;
            } else if (nums[m + 1] > nums[m]) {
                l = m + 1;
            } else {
                res = m;
                break;
            }
        }
        return res;
    }
};
```

时间复杂度：$O\left(\log n\right)$

空间复杂度：$O\left(1\right)$

# 算法笔试中处理输入和输出

## 总结

ACM 风格（笔试、比赛最常见）：

1. 规定数据量（BufferedReader、StreamTokenizer、PrintWriter），其他语言有对等的写法
2. 按行读（BufferedReader、PrintWriter），其他语言有对等的写法
3. 不要用 Scanner、System.out，IO 效率低
4. 不推荐临时动态空间，推荐全局静态空间

# 对数器打表找规律

## 题目

### 用袋子买苹果问题

有装下 8 个苹果的袋子、装下 6 个苹果的袋子，一定要保证买苹果时所有使用的袋子都装满，对于无法装满所有袋子的方案不予考虑，给定 `n` 个苹果，返回至少要多少个袋子？如果不存在每个袋子都装满的方案返回 -1

```c++
#include <bits/stdc++.h>
using namespace std;

int f(int n) {
    if (n < 0) {
        return INT_MAX;
    }
    if (n == 0) {
        return 0;
    }
    int p1 = f(n - 6);
    int p2 = f(n - 8);
    if (p1 != INT_MAX) {
        ++p1;
    }
    if (p2 != INT_MAX) {
        ++p2;
    }
    return min(p1, p2);
}

int minNumBags1(int n) {
    int ans = f(n);
    return ans != INT_MAX ? ans : -1;
}

// 找规律之后写出的
int minNumBags2(int n) {
    if ((n & 1) == 1) {
        return -1;
    }
    if (n < 18) {
        if (n == 0) {
            return 0;
        }
        if (n == 6 || n == 8) {
            return 1;
        }
        if (n == 12 || n == 14 || n == 16) {
            return 2;
        }
        return -1;
    }
    return (n - 18) / 8 + 3;
}

int main() {
    for (int n = 0; n < 100; ++n) {
        // cout << n << ": " << minNumBags1(n) << endl; // 找规律
        if (minNumBags1(n) != minNumBags2(n)) {
            cout << "测试不通过！" << endl;
            return 1;
        }
    }
    cout << "测试通过！" << endl;
    return 0;
}
```

时间复杂度：$O\left(1\right)$

空间复杂度：$O\left(1\right)$

### 01 轮流吃草的赢家问题

草一共有 `n` 的重量，两只牛轮流吃草，0 牛先吃或 1 牛先吃每只牛在自己的回合，吃草的重量必须是 4 的幂，谁在自己的回合正好把草吃完谁赢，根据输入的 `n`，返回谁赢

```c++
#include <bits/stdc++.h>
using namespace std;

// n>=0
int getWinner1(int n, int p1) {
    int p2 = p1 ^ 1;
    if (n < 5) {
        return n == 0 || n == 2 ? p2 : p1;
    }
    for (int numGrass = 1; numGrass <= n; numGrass <<= 2) {
        if (getWinner1(n - numGrass, p2) == p1) {
            return p1;
        }
    }
    return p2;
}

int getWinner2(int n, int p1) {
    return (n % 5 == 0 || n % 5 == 2 ? 1 : 0) ^ p1;
}

int main() {
    for (int n = 0; n < 50; ++n) {
        // cout << n << ": " << getWinner1(n, 0) << endl;
        if (getWinner1(n, 0) != getWinner2(n, 0)) {
            cout << "测试不通过！" << endl;
            return 1;
        }
    }
    cout << "测试通过！" << endl;
    return 0;
}
```

时间复杂度：$O\left(1\right)$

空间复杂度：$O\left(1\right)$

### 是否为连续正整数的和

判断一个数字是否是若干数量（数量大于 1）的连续正整数的和

```c++
#include <bits/stdc++.h>
using namespace std;

bool isConsecutiveSum1(int n) {
    for (int i = 1; i < n; ++i) {
        for (int j = i + 1, sum = i; sum < n; ++j) {
            sum += j;
            if (sum == n) {
                return true;
            }
        }
    }
    return false;
}

// 2 的幂的位置都是 false，其余都是 true
bool isConsecutiveSum2(int n) {
    return n != (n & -n);
}

int main() {
    for (int n = 1; n < 100; ++n) {
        // cout << n << ": " << isConsecutiveSum1(n) << endl;
        if (isConsecutiveSum1(n) != isConsecutiveSum2(n)) {
            cout << "测试不通过！" << endl;
            return 1;
        }
    }
    cout << "测试通过！" << endl;
    return 0;
}
```

时间复杂度：$O\left(1\right)$

空间复杂度：$O\left(1\right)$

### 好串数量问题

可以用 r、e、d 三种字符拼接字符串，如果拼出来的字符串中有且仅有一个长度大于等于 2 的回文子串，那么这个字符串定义为“好串”，返回长度为 `n` 的所有可能的字符串中，好串有多少个，结果对 1000000007 取模，`1 <= n <= 10^9`

```c++
#include <bits/stdc++.h>
using namespace std;

bool isPalindrome(int path[], int i, int j) {
    while (i < j) {
        if (path[i] != path[j]) {
            return false;
        }
        ++i;
        --j;
    }
    return true;
}

int numPalindromes(int n, int path[]) {
    int res = 0;
    for (int i = 0; i < n - 1; ++i) {
        for (int j = i + 1; j < n; ++j) {
            if (isPalindrome(path, i, j)) {
                ++res;
            }
        }
    }
    return res;
}

int f(int n, int i, int path[]) {
    if (i == n) {
        return numPalindromes(n, path) == 1 ? 1 : 0;
    }
    int res = 0;
    path[i] = 'r';
    res += f(n, i + 1, path);
    path[i] = 'e';
    res += f(n, i + 1, path);
    path[i] = 'd';
    res += f(n, i + 1, path);
    return res;
}

int numGoodStrs1(int n) {
    int path[n];
    return f(n, 0, path);
}

int numGoodStrs2(int n) {
    if (n == 1) {
        return 0;
    }
    if (n == 2) {
        return 3;
    }
    if (n == 3) {
        return 18;
    }
    // return (n - 4) * 6 + 30;
    return ((long long) n + 1) * 6 % 1000000007;
}

int main() {
    for (int n = 1; n < 10; ++n) {
        cout << n << ": " << numGoodStrs1(n) << endl;
        if (numGoodStrs1(n) != numGoodStrs2(n)) {
            cout << "测试不通过！" << endl;
            return 1;
        }
    }
    cout << "测试通过！" << endl;
    return 0;
}
```

时间复杂度：$O\left(1\right)$

空间复杂度：$O\left(1\right)$

# 根据数据量猜解法

## 总结

一个基本事实：C/C++ 运行时间 1s，Java/Python/Go 等其他语言运行时间 1s~2s，对应的常数指令操作量是 `10^7~10^8`，不管什么测试平台，不管什么 CPU，都是这个数量级，所以可以根据这个基本事实，来猜测自己设计的算法最终有没有可能在规定时间内通过。

运用根据数据量猜解法技巧的必要条件：

1. 题目要给定各个入参的范围最大值，正式笔试、比赛的题目一定都会给，面试中要和面试官确认
2. 对于自己设计的算法，时间复杂度要有准确的估计

这个技巧很重要，既可以提前获知自己的方法能不能通过，也可以对题目的分析有引导作用！

问题规模和可用算法：

|      | $O\left(\log n\right)$ | $O\left(n\right)$ | $O\left(n\log n\right)$ | $O\left(n\sqrt n\right)$ | $O\left(n^2\right)$ | $O\left(2^n\right)$ | $O\left(n!\right)$ |
| :--: | :--------------------: | :---------------: | :---------------------: | :--------------: | :--: | :--: | :--: |
| `n<=11` | Y | Y | Y | Y | Y | Y | Y |
| `n<=25` | Y | Y | Y | Y | Y | Y | N |
| `n<=5000` | Y | Y | Y | Y | Y | N | N |
| `n<=10^5` | Y | Y | Y | Y | N | N | N |
| `n<=10^6` | Y | Y | Y | N | N | N | N |
| `n<=10^7` | Y | Y | N | N | N | N | N |
| `n<=10^8` | Y | N | N | N | N | N | N |

这张表其实作用有限，因为时间复杂度的估计很多时候并不是一个入参决定，可能是多个入参共同决定，所以最关键的就是记住常数指令操作量是 `10^7~10^8`，然后方法是什么复杂度就可以估计能否通过了。

## 题目

### 消灭怪物

https://www.nowcoder.com/practice/d88ef50f8dab4850be8cd4b95514bbbd

因为 `n` 的最大值很小，所以哪怕时间复杂度是 $O\left(n!\right)$ 也是能过的

```c++
#include <bits/stdc++.h>
using namespace std;

int n;
int m;
int bloods[10];
int thresholds[10];

// 全排列
// 返回在哪个位置怪的血量小于等于 0
int f(int i, int r) {
    if (r <= 0) {
        return i;
    }
    if (i == n) {
        return INT_MAX;
    }
    int res = INT_MAX;
    for (int j = i; j < n; ++j) {
        swap(bloods[i], bloods[j]);
        swap(thresholds[i], thresholds[j]);
        res = min(res, f(i + 1, r - (r > thresholds[i] ? bloods[i] : bloods[i] << 1)));
        swap(bloods[i], bloods[j]);
        swap(thresholds[i], thresholds[j]);
    }
    return res;
}

int main() {
    int numTests;
    cin >> numTests;
    int res;
    while (numTests--) {
        cin >> n >> m;
        for (int i = 0; i < n; ++i) {
            cin >> bloods[i] >> thresholds[i];
        }
        res = f(0, m);
        cout << (res != INT_MAX ? res : -1) << endl;
    }
    return 0;
}
```

时间复杂度：$O\left(n!\right)$

空间复杂度：$O\left(n\right)$

### 超级回文数

https://leetcode.cn/problems/super-palindromes

利用根据数据量猜解法分析，枚举原始范围大小最大为 `10^18`，超过了 `10^7~10^8`，所以在原始范围上一个一个检验一个数是否是超级回文数是会超时的，如果将原始范围上的数字开根号之后检验，这个范围大小最大为 `10^9`，还是会超时，如果利用回文的定义构造出开根号之后的数，这个枚举的大小最大为 `10^5`，不会超时。

```c++
class Solution {
public:
    typedef long long ll;

    int superpalindromesInRange1(string left, string right) {
        ll l = stoll(left);
        ll r = stoll(right);
        ll limit = sqrt(r);
        ll seed1 = 1;
        ll seed2;
        int res = 0;
        do {
            seed2 = evenEnlarge(seed1);
            if (isSafeSquare(seed2) && isValid(seed2 * seed2, l, r)) {
                ++res;
            }
            seed2 = oddEnlarge(seed1);
            if (isSafeSquare(seed2) && isValid(seed2 * seed2, l, r)) {
                ++res;
            }
            ++seed1;
        } while (seed2 < limit);
        return res;
    }

    ll evenEnlarge(ll num) {
        ll res = num;
        while (num > 0) {
            res = 10 * res + num % 10;
            num /= 10;
        }
        return res;
    }

    ll oddEnlarge(ll num) {
        ll res = num;
        num /= 10;
        while (num > 0) {
            res = 10 * res + num % 10;
            num /= 10;
        }
        return res;
    }

    bool isSafeSquare(ll num) {
        return num <= sqrt(LLONG_MAX);
    }

    bool isValid(ll num, ll l, ll r) {
        return num >= l && num <= r && isPalindrome(num);
    }

    bool isPalindrome(ll num) {
        ll offset = 1;
        while (num / offset >= 10) {
            offset *= 10;
        }
        while (num > 0) {
            if (num / offset != num % 10) {
                return false;
            }
            num = num % offset / 10;
            offset /= 100;
        }
        return true;
    }

    // 找到下面的 table
    vector<ll> collect() {
        ll l = 1;
        ll r = pow(10, 18);
        ll limit = sqrt(r);
        ll seed1 = 1;
        ll seed2;
        vector<ll> res;
        do {
            seed2 = evenEnlarge(seed1);
            if (isSafeSquare(seed2) && isValid(seed2 * seed2, l, r)) {
                res.push_back(seed2 * seed2);
            }
            seed2 = oddEnlarge(seed1);
            if (isSafeSquare(seed2) && isValid(seed2 * seed2, l, r)) {
                res.push_back(seed2 * seed2);
            }
            ++seed1;
        } while (seed2 < limit);
        sort(res.begin(), res.end());
        return res;
    }

    vector<ll> table = {
        1L,
        4L,
        9L,
        121L,
        484L,
        10201L,
        12321L,
        14641L,
        40804L,
        44944L,
        1002001L,
        1234321L,
        4008004L,
        100020001L,
        102030201L,
        104060401L,
        121242121L,
        123454321L,
        125686521L,
        400080004L,
        404090404L,
        10000200001L,
        10221412201L,
        12102420121L,
        12345654321L,
        40000800004L,
        1000002000001L,
        1002003002001L,
        1004006004001L,
        1020304030201L,
        1022325232201L,
        1024348434201L,
        1210024200121L,
        1212225222121L,
        1214428244121L,
        1232346432321L,
        1234567654321L,
        4000008000004L,
        4004009004004L,
        100000020000001L,
        100220141022001L,
        102012040210201L,
        102234363432201L,
        121000242000121L,
        121242363242121L,
        123212464212321L,
        123456787654321L,
        400000080000004L,
        10000000200000001L,
        10002000300020001L,
        10004000600040001L,
        10020210401202001L,
        10022212521222001L,
        10024214841242001L,
        10201020402010201L,
        10203040504030201L,
        10205060806050201L,
        10221432623412201L,
        10223454745432201L,
        12100002420000121L,
        12102202520220121L,
        12104402820440121L,
        12122232623222121L,
        12124434743442121L,
        12321024642012321L,
        12323244744232321L,
        12343456865434321L,
        12345678987654321L,
        40000000800000004L,
        40004000900040004L,
    };

    // 因为 table 很小，所以二分都不需要了
    int superpalindromesInRange2(string left, string right) {
        ll l = stoll(left);
        ll r = stoll(right);
        int res = 0;
        for (ll& num: table) {
            if (num >= l && num <= r) {
                ++res;
            }
        }
        return res;
    }
};
```

记最大的数为 $n$：

时间复杂度：$O\left(n^\frac{1}{4}\right)$

空间复杂度：$O\left(1\right)$

# 递归和 Master 公式

## 总结

### 递归

1. 任何递归函数都一定可以改成非递归，不用系统帮压栈（系统栈空间），自己压栈（内存空间）
2. 递归改成非递归的必要性：
   1. 工程上几乎一定要改，因为内存比系统栈大，除非确定数据量再大递归也一定不深，例如归并排序、快速排序、线段树、很多的平衡树等
   2. 算法笔试或者比赛中（能通过就不改）
3. 一定要理解回溯题目中递归函数的意义，这样就不用扣递归细节做题了！
4. 回溯的题目可以考虑是否要对原数组排序

### Master 公式

所有子问题规模相同的递归才能用 Master 公式：
$$
T\left(n\right)=aT\left(\frac{n}{b}\right)+O\left(n^c\right)
$$

1. 如果 $\log_b{a}<c$，时间复杂度为 $O\left(n^c\right)$
2. 如果 $\log_b{a}>c$，时间复杂度为 $O\left(n^{\log_b{a}}\right)$
3. 如果 $\log_b{a}=c$，时间复杂度为 $O\left(n^c\log{n}\right)$

### 嵌套类问题的递归解题套路

大概过程: 

1. 定义全局变量 `int where`
2. 递归函数 `f(i)`：`s[i...]` 从 `i` 位置出发开始解析，遇到字符串终止或嵌套条件终止就返回
3. 返回值是 `f(i)` 负责这一段的结果
4. `f(i)` 在返回前更新全局变量 `where`，让上级函数通过 `where` 知道解析到了什么位置，进而继续

执行细节:

1. 如果 `f(i)` 遇到嵌套条件开始，就调用下级递归去处理嵌套，下级会负责嵌套部分的计算结果
2. `f(i)` 下级处理完成后，`f(i)` 可以根据下级更新的全局变量 `where`，知道该从什么位置继续解析

## 题目

### 字符串的全部子序列

https://www.nowcoder.com/practice/92e6247998294f2c933906fdedbc6e6a

```c++
class Solution {
public:
    vector<string> generatePermutation(string str) {
        string path;
        unordered_set<string> s; // 通过 unordered_set 去重，并不是剪枝
        f(str, 0, path, s);
        vector<string> res;
        for (const string& curr: s) {
            res.push_back(curr);
        }
        return res;
    }

    // [i...] 范围上的字符串的全部子序列前面加上 path 后放到 s 中，函数执行完后不改变 path
    void f(string& str, int i, string& path, unordered_set<string>& s) {
        if (i == str.size()) {
            s.insert(path);
            return;
        }
        f(str, i + 1, path, s);
        path.push_back(str[i]);
        f(str, i + 1, path, s);
        path.pop_back();
    }
};
```

另一种写法：

```c++
class Solution {
public:
    vector<string> generatePermutation(string str) {
        char path[str.size()];
        unordered_set<string> s;
        f(str, 0, path, 0, s);
        vector<string> res(s.size());
        int i = 0;
        for (const string& curr: s) {
            res[i++] = curr;
        }
        return res;
    }

    // path[0...pathSize-1] 部分不允许改变
    void f(string& str, int i, char path[], int pathSize, unordered_set<string>& s) {
        if (i == str.size()) {
            s.insert(string(path, pathSize));
            return;
        }
        path[pathSize] = str[i];
        f(str, i + 1, path, pathSize + 1, s);
        f(str, i + 1, path, pathSize, s);
    }
};
```

时间复杂度：子序列个数 $O\left(2^n\right)$，拼出每一个子序列（收集 `path`）的时间复杂度为 $O\left(n\right)$，总体时间复杂度为 $O\left(n2^n\right)$

空间复杂度：$O\left(n\right)$

### 子集 II

https://leetcode.cn/problems/subsets-ii

```c++
class Solution {
private:
    // [i...] 范围上的所有组合前面加上 path 后添加到 res 中，path[0...pathSize-1] 部分不允许改变
    void f(vector<int>& nums, int i, int path[], int pathSize, vector<vector<int>>& res) {
        if (i == nums.size()) {
            res.push_back(vector<int>(path, path + pathSize));
            return;
        }
        int j = i + 1;
        while (j < nums.size() && nums[j] == nums[i]) {
            ++j;
        }
        f(nums, j, path, pathSize, res); // 0 个 nums[i] 这样的数字
        for (; i < j; ++i) { // 当前组的数字取多少个
            path[pathSize++] = nums[i];
            f(nums, j, path, pathSize, res);
        }
    }

public:
    vector<vector<int>> subsetsWithDup(vector<int>& nums) {
        sort(nums.begin(), nums.end()); // 通过元素分组去重，是剪枝
        int path[nums.size()];
        vector<vector<int>> res;
        f(nums, 0, path, 0, res);
        return res;
    }
};
```

时间复杂度：组合个数 $O\left(2^n\right)$，拼出每一个组合的时间复杂度为 $O\left(n\right)$，总体时间复杂度为 $O\left(n2^n\right)$

空间复杂度：$O\left(n\right)$

### 全排列

https://leetcode.cn/problems/permutations

```c++
class Solution {
private:
    // nums[0...i-1] 范围不允许改变
    void f(vector<int>& nums, int i, vector<vector<int>>& res) {
        if (i == nums.size()) {
            res.push_back(nums);
            return;
        }
        // 以 nums[j] 开头，后面的部分做全排列
        for (int j = i; j < nums.size(); ++j) {
            swap(nums[i], nums[j]);
            f(nums, i + 1, res);
            swap(nums[i], nums[j]);
        }
    }

public:
    vector<vector<int>> permute(vector<int>& nums) {
        vector<vector<int>> res;
        f(nums, 0, res);
        return res;
    }
};
```

时间复杂度：$O\left(n!\times n\right)$

空间复杂度：$O\left(n\right)$

### 全排列 II

https://leetcode.cn/problems/permutations-ii

```c++
class Solution {
public:
    vector<vector<int>> permuteUnique(vector<int>& nums) {
        vector<vector<int>> res;
        f(nums, 0, res);
        return res;
    }

    // nums[0...i-1] 范围不允许改变
    void f(vector<int>& nums, int i, vector<vector<int>>& res) {
        if (i == nums.size()) {
            res.push_back(nums);
            return;
        }
        unordered_set<int> s;
        for (int j = i; j < nums.size(); ++j) {
            if (!s.count(nums[j])) {
                s.insert(nums[j]);
                swap(nums[i], nums[j]);
                f(nums, i + 1, res);
                swap(nums[i], nums[j]);
            }
        }
    }
};
```

时间复杂度：$O\left(n!\times n\right)$

空间复杂度：$O\left(n\right)$

### 用递归函数逆序栈

只允许用递归函数逆序一个栈。

```c++
#include <bits/stdc++.h>
using namespace std;

// stk.size()>0
// 栈底元素移除掉，上面的元素盖下来，返回移除掉的栈底元素
int bottomOut(stack<int>& stk) {
    int top = stk.top();
    stk.pop();
    if (stk.empty()) {
        return top;
    }
    int res = bottomOut(stk);
    stk.push(top);
    return res;
}

void reverseStack(stack<int>& stk) {
    if (stk.empty()) {
        return;
    }
    int bottom = bottomOut(stk);
    reverseStack(stk);
    stk.push(bottom);
}

int main() {
    stack<int> stk;
    stk.push(1);
    stk.push(2);
    stk.push(3);
    reverseStack(stk);
    while (!stk.empty()) {
        cout << stk.top() << endl;
        stk.pop();
    }
    return 0;
}
```

时间复杂度：$O\left(n^2\right)$

空间复杂度：$O\left(n\right)$

### 用递归函数排序栈

只允许用递归函数排序一个栈。

```c++
#include <bits/stdc++.h>
#include "structures.h"
using namespace std;

// 获取 stk 的大小，假设不允许用 stk.size()
int getDepth(stack<int>& stk) {
    if (stk.empty()) {
        return 0;
    }
    int top = stk.top();
    stk.pop();
    int res = 1 + getDepth(stk);
    stk.push(top);
    return res;
}

// stk 往下扎 depth 个元素，元素最大值是多少
int getMax(stack<int>& stk, int depth) {
    if (!depth) {
        return INT_MIN;
    }
    int top = stk.top();
    stk.pop();
    int res = max(top, getMax(stk, depth - 1));
    stk.push(top);
    return res;
}

// stk 往下扎 depth 个元素，num 出现了多少次
int getTimes(stack<int>& stk, int depth, int num) {
    if (!depth) {
        return 0;
    }
    int top = stk.top();
    stk.pop();
    int res = (top == num) + getTimes(stk, depth - 1, num);
    stk.push(top);
    return res;
}

// 从栈当前的顶部开始，往下数 depth 层，num 出现了 times 次
// 把这 times 个 num 沉淀，剩下的数据状况不变
// depth==0 时直接往 stk 里 push times 个 num
// 类似于计数排序
void precipitate(stack<int>& stk, int depth, int num, int times) {
    if (!depth) {
        while (times--) {
            stk.push(num);
        }
        return;
    }
    int top = stk.top();
    stk.pop();
    precipitate(stk, depth - 1, num, times);
    if (top != num) { // top 未沉淀
        stk.push(top);
    }
    // else，top 沉淀了
}

void sortStack(stack<int>& stk) {
    int depth = getDepth(stk);
    int maxNum;
    int times;
    while (depth) {
        maxNum = getMax(stk, depth);
        times = getTimes(stk, depth, maxNum);
        precipitate(stk, depth, maxNum, times);
        depth -= times;
    }
}

int main() {
    stack<int> stk;
    stk.push(1);
    stk.push(3);
    stk.push(2);
    sortStack(stk);
    while (!stk.empty()) {
        cout << stk.top() << endl;
        stk.pop();
    }
    return 0;
}
```

时间复杂度：$O\left(n^2\right)$

空间复杂度：$O\left(n\right)$

### 汉诺塔问题

https://leetcode.cn/problems/hanota-lcci

```c++
class Solution {
public:
    void hanota(vector<int>& from, vector<int>& other, vector<int>& to) {
        f(from, other, to, from.size());
    }

    // 把 from 最上面的 n 个圆盘移动到 to 上，保证 other 是空的且 to 上的圆盘（若有）都比 from 上的圆盘大
    void f(vector<int>& from, vector<int>& other, vector<int>& to, int n) {
        if (n == 1) {
            to.push_back(from.back());
            from.pop_back();
            return;
        }
        f(from, to, other, n - 1);
        to.push_back(from.back());
        from.pop_back();
        f(other, from, to, n - 1);
    }
};
```

时间复杂度：$O\left(2^n\right)$

空间复杂度：$O\left(n\right)$

### 表达式求值

https://www.nowcoder.com/practice/9566499a2e1546c0a257e885dfdbf30d

```c++
#include <bits/stdc++.h>
using namespace std;

int where;

void push(vector<int>& nums, vector<char>& ops, int num, char op) {
    if (nums.empty() || ops.back() == '+' || ops.back() == '-') {
        nums.push_back(num);
        ops.push_back(op);
    } else {
        if (ops.back() == '*') {
            nums.back() *= num;
        } else { // ops.back()=='/'
            nums.back() /= num;
        }
        ops.back() = op;
    }
}

// str[i...] 开始计算，遇到字符串终止或者遇到 ')' 停止
// 返回: 自己负责的这一段，计算的结果
// 返回之间，更新全局变量 where，为了上游函数知道从哪继续！
int f(string& str, int i) {
    int num = 0;
    vector<int> nums;
    vector<char> ops;
    while (i < str.size() && str[i] != ')') {
        if (str[i] >= '0' && str[i] <= '9') {
            num = 10 * num + str[i++] - '0';
        } else if (str[i] != '(') { // 遇到了运算符
            push(nums, ops, num, str[i++]);
            num = 0;
        } else { // 遇到了左括号
            num = f(str, i + 1);
            i = where + 1;
        }
    }
    push(nums, ops, num, '+'); // 这个 '+' 没用
    where = i;
    int res = nums[0];
    for (int j = 0; j < ops.size() - 1; ++j) {
        res += ops[j] == '+' ? nums[j + 1] : -nums[j + 1];
    }
    return res;
}

int calc(string& str) {
    return f(str, 0);
}

int main() {
    string str;
    while (cin >> str) {
        cout << calc(str) << endl;
    }
    return 0;
}
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

### 字符串解码

https://leetcode.cn/problems/decode-string

```c++
class Solution {
private:
    int where;

    string f(string& str, int i) {
        string res;
        int num = 0;
        string part;
        while (i < str.size() && str[i] != ']') {
            if (str[i] >= 'a' && str[i] <= 'z') {
                res.push_back(str[i++]);
            } else if (str[i] >= '0' && str[i] <= '9') {
                num = 10 * num + str[i++] - '0';
            } else { // '['
                part = f(str, i + 1);
                while (num > 0) { // 这里不要写 while (num--)，因为最后 num 是 -1
                    res += part;
                    --num;
                }
                i = where + 1;
            }
        }
        where = i;
        return res;
    }

public:
    string decodeString(string str) {
        return f(str, 0);
    }
};
```

记解码前后字符串的长度分别为 $s_1$ 和 $s_2$

时间复杂度：$O\left(s_1+s_2\right)$

空间复杂度：$O\left(s_1\right)$

### 原子的数量

https://leetcode.cn/problems/number-of-atoms

```c++
class Solution {
private:
    int where;

public:
    string countOfAtoms(string str) {
        map<string, int> ht = f(str, 0);
        string res;
        for (auto& p: ht) {
            res += p.first;
            if (p.second > 1) {
                res += to_string(p.second);
            }
        }
        return res;
    }

    map<string, int> f(string& str, int i) {
        string atom;
        map<string, int> part;
        int num = 0; // atom 或 part 乘多少
        map<string, int> res;
        while (i < str.size() && str[i] != ')') {
            if (str[i] >= 'A' && str[i] <= 'Z' || str[i] == '(') {
                // 结算之前的「历史」（atom 或 part 的其中一个），开始新的记录
                fill(atom, part, num, res);
                atom.clear();
                part.clear();
                num = 0;
                if (str[i] >= 'A' && str[i] <= 'Z') {
                    atom.push_back(str[i++]);
                } else {
                    part = f(str, i + 1);
                    i = where + 1;
                }
            } else if (str[i] >= 'a' && str[i] <= 'z') {
                atom.push_back(str[i++]);
            } else {
                num = 10 * num + str[i++] - '0';
            }
        }
        fill(atom, part, num, res);
        where = i;
        return res;
    }

    void fill(string& atom, map<string, int>& part, int num, map<string, int>& res) {
        if (atom.empty() && part.empty()) {
            return;
        }
        if (!num) {
            num = 1;
        }
        if (!atom.empty()) {
            res[atom] += num;
        } else { // !part.empty()
            for (auto& p1: part) {
                res[p1.first] += num * p1.second;
            }
        }
    }
};
```

时间复杂度：最坏情况下，递归 $O\left(n\right)$ 层，每层合并有序表的时间复杂度为 $O\left(n\right)$，总体时间复杂度为 $O\left(n^2\right)$

空间复杂度：$O\left(n\right)$

### N 皇后 II

https://leetcode.cn/problems/n-queens-ii

数组实现：

```c++
class Solution {
public:
    int totalNQueens(int n) {
        int path[n];
        return f(n, 0, path);
    }

    int f(int n, int i, int path[]) {
        if (i == n) {
            return 1;
        }
        int res = 0;
        for (int j = 0; j < n; ++j) {
            if (check(i, j, path)) {
                path[i] = j;
                res += f(n, i + 1, path);
            }
        }
        return res;
    }

    bool check(int i, int j, int path[]) {
        for (int k = 0; k < i; ++k) {
            if (j == path[k] || abs(i - k) == abs(j - path[k])) {
                return false;
            }
        }
        return true;
    }
};
```

时间复杂度：$O\left(n!\right)$

空间复杂度：$O\left(n\right)$

位运算实现，省去了很多数组寻址的过程：

```c++
class Solution {
public:
    int totalNQueens(int n) {
        return f((1 << n) - 1, 0, 0, 0);
    }

    int f(int limit, int col, int left, int right) {
        if (col == limit) {
            return 1;
        }
        int ban = col | left | right;
        int candidates = ~ban & limit;
        int place;
        int res = 0;
        while (candidates) {
            place = candidates & -candidates;
            res += f(limit, col | place, (left | place) << 1, (right | place) >> 1);
            candidates ^= place;
        }
        return res;
    }
};
```

时间复杂度：$O\left(n!\right)$

空间复杂度：$O\left(n\right)$

# 链表

## 总结

- 如果笔试中空间要求不严格，直接使用容器来解决链表问题
- 如果笔试中空间要求严格或者在面试中面试官强调空间的优化，需要使用额外空间复杂度 $O\left(1\right)$ 的方法
- 常用的技巧：快慢指针
- 链表类题目往往都是很简单的算法问题，核心考察点也并不是算法设计，是 coding 能力
- 多画图
- `dummyHead` 很常用

链表中常见的循环套路：

```pseudocode
定义初始变量，通常是当前节点 p 为 xxx，其余节点为 nullptr
while (关于 p 的条件判断) {
	为其余节点赋值
	处理
	为 p 赋值
}
```

## 题目

### 反转链表

https://leetcode.cn/problems/reverse-linked-list

```c++
class Solution {
public:
    ListNode* reverseList(ListNode* h) {
        ListNode* p1 = nullptr;
        ListNode* p2 = h;
        ListNode* p3 = nullptr;
        while (p2 != nullptr) {
            p3 = p2->next;
            p2->next = p1;
            p1 = p2;
            p2 = p3;
        }
        return p1;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

拓展：双向链表反转

```c++
class Solution {
public:
    BiListNode* reverseList(BiListNode* head) {
        BiListNode* p1 = nullptr,* p2 = head,* p3;
        while (p2) {
            p3 = p2->next;
            p2->prev = p3;
            p2->next = p1;
            p1 = p2;
            p2 = p3;
        }
        return p1;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 合并两个有序链表

https://leetcode.cn/problems/merge-two-sorted-lists

```c++
class Solution {
public:
    ListNode* mergeTwoLists(ListNode* h1, ListNode* h2) {
        ListNode* dh = new ListNode();
        ListNode* c = dh;
        while (h1 != nullptr && h2 != nullptr) {
            if (h1->val < h2->val) {
                c->next = h1;
                h1 = h1->next;
            } else {
                c->next = h2;
                h2 = h2->next;
            }
            c = c->next;
        }
        c->next = h1 != nullptr ? h1 : h2;
        return dh->next;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 两数相加

https://leetcode.cn/problems/add-two-numbers

```c++
class Solution {
public:
    ListNode* addTwoNumbers(ListNode* h1, ListNode* h2) {
        ListNode* dh = new ListNode();
        ListNode* c = dh;
        int carry = 0;
        int sum = 0;
        while (h1 != nullptr || h2 != nullptr || carry > 0) {
            sum = (h1 != nullptr ? h1->val : 0) + (h2 != nullptr ? h2->val : 0) + carry;
            carry = sum / 10;
            c->next = new ListNode(sum % 10);
            c = c->next;
            if (h1 != nullptr) {
                h1 = h1->next;
            }
            if (h2 != nullptr) {
                h2 = h2->next;
            }
        }
        return dh->next;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$，生成的链表所占的空间是题目必须的，不计入空间复杂度

### 分隔链表

https://leetcode.cn/problems/partition-list

```c++
class Solution {
public:
    ListNode* partition(ListNode* head, int x) {
        ListNode* dummyHead1 = new ListNode(),* dummyHead2 = new ListNode(),* p1 = dummyHead1,* p2 = dummyHead2,* p = head;
        while (p) {
            if (p->val < x) {
                p1->next = p;
                p1 = p;
            } else {
                p2->next = p;
                p2 = p;
            }
            p = p->next;
        }
        p1->next = dummyHead2->next;
        p2->next = nullptr; // 【易漏】可能存在环
        return dummyHead1->next;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 相交链表

https://leetcode.cn/problems/intersection-of-two-linked-lists

```c++
class Solution {
public:
    ListNode* getIntersectionNode(ListNode* headA, ListNode* headB) {
        int diff = 0;
        ListNode* currA = headA,* currB = headB;
        while (currA->next) {
            ++diff;
            currA = currA->next;
        }
        while (currB->next) {
            --diff;
            currB = currB->next;
        }
        if (currA != currB) {
            return nullptr;
        }
        if (diff >= 0) {
            currA = headA;
            currB = headB;
        } else {
            currA = headB;
            currB = headA;
        }
        diff = abs(diff);
        while (diff--) {
            currA = currA->next;
        }
        while (currA != currB) {
            currA = currA->next;
            currB = currB->next;
        }
        return currA;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### K 个一组翻转链表

https://leetcode.cn/problems/reverse-nodes-in-k-group

```c++
class Solution {
public:
    ListNode* reverseKGroup(ListNode* h, int k) {
        ListNode* dh = new ListNode(0, h);
        ListNode* spr = dh; // spr->next==s
        ListNode* s = nullptr;
        ListNode* e = nullptr;
        ListNode* enx = nullptr; // e->next
        while (spr->next != nullptr) {
            s = spr->next;
            e = getTeamEnd(s, k);
            if (e == nullptr) {
                break;
            }
            enx = e->next;
            reverseTeam(s, e);
            spr->next = e;
            s->next = enx;
            spr = s;
        }
        return dh->next;
    }

    ListNode* getTeamEnd(ListNode* h, int k) {
        while (--k > 0 && h != nullptr) {
            h = h->next;
        }
        return h;
    }

    void reverseTeam(ListNode* s, ListNode* e) {
        ListNode* p1 = nullptr;
        ListNode* p2 = s;
        ListNode* p3 = nullptr;
        while (p1 != e) {
            p3 = p2->next;
            p2->next = p1;
            p1 = p2;
            p2 = p3;
        }
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 随机链表的复制

https://leetcode.cn/problems/copy-list-with-random-pointer

```c++
class Solution {
public:
    Node* copyRandomList(Node* h) {
        if (h == nullptr) {
            return nullptr;
        }
        Node* p1 = h;
        Node* p2 = nullptr;
        while (p1 != nullptr) {
            p2 = p1->next;
            p1->next = new Node(p1->val);
            p1->next->next = p2;
            p1 = p2;
        }
        p1 = h;
        while (p1 != nullptr) {
            p1->next->random = p1->random != nullptr ? p1->random->next : nullptr;
            p1 = p1->next->next;
        }
        Node* res = h->next;
        p1 = h;
        p2 = h->next;
        while (p1 != nullptr) {
            p2 = p1->next;
            p1->next = p2->next;
            p2->next = p2->next != nullptr ? p2->next->next : nullptr;
            p1 = p1->next;
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 回文链表

https://leetcode.cn/problems/palindrome-linked-list

```c++
class Solution {
public:
    bool isPalindrome(ListNode* h) {
        if (h == nullptr || h->next == nullptr) {
            return true;
        }
        ListNode* s = h;
        ListNode* f = h;
        while (f->next != nullptr && f->next->next != nullptr) {
            s = s->next;
            f = f->next->next;
        }
        ListNode* next = s->next;
        ListNode* t = reverseList(s);
        ListNode* l = h;
        ListNode* r = t;
        bool res = true;
        while (l != nullptr && r != nullptr) {
            if (l->val != r->val) {
                res = false;
                break;
            }
            l = l->next;
            r = r->next;
        }
        reverseList(t);
        s->next = next;
        return res;
    }

    ListNode* reverseList(ListNode* h) {
        ListNode* p1 = nullptr;
        ListNode* p2 = h;
        ListNode* p3 = nullptr;
        while (p2 != nullptr) {
            p3 = p2->next;
            p2->next = p1;
            p1 = p2;
            p2 = p3;
        }
        return p1;
    }
};

```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 环形链表 II

https://leetcode.cn/problems/linked-list-cycle-ii

```c++
class Solution {
public:
    ListNode* detectCycle(ListNode* h) {
        if (h == nullptr || h->next == nullptr || h->next->next == nullptr) { // 节点个数小于等于 2
            return nullptr;
        }
        ListNode* s = h->next;
        ListNode* f = h->next->next;
        while (s != f) {
            if (f->next == nullptr || f->next->next == nullptr) {
                return nullptr;
            }
            s = s->next;
            f = f->next->next;
        }
        s = h;
        while (s != f) {
            s = s->next;
            f = f->next;
        }
        return s;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 排序链表

https://leetcode.cn/problems/sort-list

```c++
class Solution {
public:
    ListNode* sortList(ListNode* h) {
        ListNode* p = h;
        int len = 0;
        while (p != nullptr) {
            ++len;
            p = p->next;
        }
        ListNode* dh = new ListNode(0, h);
        ListNode* l1 = nullptr;
        ListNode* r1 = nullptr;
        ListNode* l2 = nullptr;
        ListNode* r2 = nullptr;
        ListNode* nx = nullptr; // r2->next==nx
        // 迭代版本的归并排序
        for (int step = 1; step < len; step <<= 1) {
            p = dh; // p->next==l1
            while (p->next != nullptr) {
                l1 = p->next;
                r1 = getTeamEnd(l1, step);
                if (r1->next == nullptr) {
                    break;
                }
                l2 = r1->next;
                r2 = getTeamEnd(l2, step);
                nx = r2->next;
                p = mergeTwoLists(p, l1, r1, l2, r2);
                p->next = nx;
            }
        }
        return dh->next;
    }

    ListNode* getTeamEnd(ListNode* p, int k) {
        while (--k && p->next != nullptr) {
            p = p->next;
        }
        return p;
    }

    // 返回尾节点
    ListNode* mergeTwoLists(ListNode* p, ListNode* l1, ListNode* r1, ListNode* l2, ListNode* r2) {
        while (l1 != nullptr && l2 != nullptr) {
            if (l1->val < l2->val) {
                p->next = l1;
                l1 = l1 != r1 ? l1->next : nullptr;
            } else {
                p->next = l2;
                l2 = l2 != r2 ? l2->next : nullptr;
            }
            p = p->next;
        }
        p->next = l1 != nullptr ? l1 : l2;
        return l1 != nullptr ? r1 : r2;
    }
};
```

时间复杂度：$O\left(n\log n\right)$

空间复杂度：$O\left(1\right)$

# 队列和栈

## 题目

### 设计循环队列

https://leetcode.cn/problems/design-circular-queue

```c++
class MyCircularQueue {
private:
    vector<int> q;
    int limit, h, t, size; // t 是尾部的下一个位置（即将插入元素的位置），这题的重点是 size 变量

public:
    MyCircularQueue(int k) {
        q.resize(k);
        limit = k;
        h = t = size = 0;
    }

    bool enQueue(int value) {
        if (isFull()) {
            return false;
        }
        q[t] = value;
        t = t < limit - 1 ? t + 1 : 0;
        ++size;
        return true;
    }

    bool deQueue() {
        if (isEmpty()) {
            return false;
        }
        h = h < limit - 1 ? h + 1 : 0;
        --size;
        return true;
    }

    int Front() {
        return isEmpty() ? -1 : q[h];
    }

    int Rear() {
        return isEmpty() ? -1 : q[t ? t - 1 : limit - 1];
    }

    bool isEmpty() {
        return !size;
    }

    bool isFull() {
        return size == limit;
    }
};
```

时间复杂度：各操作的时间复杂度均为 $O\left(n\right)$

空间复杂度：整个数据结构以及各操作的时间复杂度均为 $O\left(1\right)$

### 设计循环双端队列

https://leetcode.cn/problems/design-circular-deque

```c++
class MyCircularDeque {
private:
    vector<int> dq;
    int h, t, size, limit;

public:
    MyCircularDeque(int k) {
        dq.resize(k);
        h = t = size = 0;
        limit = k;
    }

    bool insertFront(int value) {
        if (isFull()) {
            return false;
        }
        h = h ? h - 1 : limit - 1;
        dq[h] = value;
        ++size;
        return true;
    }

    bool insertLast(int value) {
        if (isFull()) {
            return false;
        }
        dq[t] = value;
        t = t < limit - 1 ? t + 1 : 0;
        ++size;
        return true;
    }

    bool deleteFront() {
        if (isEmpty()) {
            return false;
        }
        h = h < limit - 1 ? h + 1 : 0;
        --size;
        return true;
    }

    bool deleteLast() {
        if (isEmpty()) {
            return false;
        }
        t = t ? t - 1 : limit - 1;
        --size;
        return true;
    }

    int getFront() {
        return isEmpty() ? -1 : dq[h];
    }

    int getRear() {
        return isEmpty() ? -1 : dq[t ? t - 1 : limit - 1];
    }

    bool isEmpty() {
        return !size;
    }

    bool isFull() {
        return size == limit;
    }
};
```

时间复杂度：各操作的时间复杂度均为 $O\left(n\right)$

空间复杂度：整个数据结构以及各操作的时间复杂度均为 $O\left(1\right)$

### 用栈实现队列

https://leetcode.cn/problems/implement-queue-using-stacks

```c++
class MyQueue {
    stack<int> stackPush, stackPop;

public:
    MyQueue() {
    }

    void pushToPop() {
        if (stackPop.empty()) { // 【易漏】
            while (!stackPush.empty()) { // 必须倒完
                stackPop.push(stackPush.top());
                stackPush.pop();
            }
        }
    }

    void push(int x) {
        stackPush.push(x);
    }

    int pop() {
        pushToPop();
        int x = stackPop.top();
        stackPop.pop();
        return x;
    }

    int peek() {
        pushToPop();
        return stackPop.top();
    }

    bool empty() {
        return stackPush.empty() && stackPop.empty();
    }
};
```

时间复杂度：`push` 和 `empty` 为 $O\left(1\right)$，`pop` 和 `peek` 均摊为 $O\left(1\right)$

空间复杂度：$O\left(n\right)$，$n$ 为 `push` 次数

### 用队列实现栈

https://leetcode.cn/problems/implement-stack-using-queues

```c++
class MyStack {
private:
    queue<int> dataQueue, tempQueue;

public:
    MyStack() {
    }

    void push(int x) {
        tempQueue.push(x);
        while (!dataQueue.empty()) {
            tempQueue.push(dataQueue.front());
            dataQueue.pop();
        }
        swap(dataQueue, tempQueue);
    }

    int pop() {
        int x = dataQueue.front();
        dataQueue.pop();
        return x;
    }

    int top() {
        return dataQueue.front();
    }

    bool empty() {
        return dataQueue.empty();
    }
};
```

时间复杂度：`push` 为 $O\left(n\right)$，其余操作为 $O\left(1\right)$

空间复杂度：$O\left(n\right)$，$n$ 为 `push` 次数

### 最小栈

https://leetcode.cn/problems/min-stack

```c++
const int MAXN = 3e4;

class MinStack {
private:
    int dataStk[MAXN];
    int minStk[MAXN];
    int datat;
    int mint;

public:
    MinStack() {
        datat = mint = 0;
    }

    void push(int val) {
        dataStk[datat++] = val;
        if (mint == 0 || val <= minStk[mint - 1]) {
            minStk[mint++] = val;
        }
    }

    void pop() {
        if (dataStk[--datat] == minStk[mint - 1]) {
            --mint;
        }
    }

    int top() {
        return dataStk[datat - 1];
    }

    int getMin() {
        return minStk[mint - 1];
    }
};
```

时间复杂度：$O\left(1\right)$

空间复杂度：$O\left(n\right)$

# 二叉树

## 总结

一个题目中如果涉及到两个递归函数，注意看一下时间复杂度是不是 $O\left(n^2\right)$ 而不是 $O\left(n\right)$ 的，例如这个题 https://leetcode.cn/problems/path-sum-iii 和这个题 https://leetcode.cn/problems/binary-tree-maximum-path-sum

## 题目

### 二叉树的前序遍历

https://leetcode.cn/problems/binary-tree-preorder-traversal

非递归版：

```c++
class Solution {
public:
    vector<int> preorderTraversal(TreeNode* r) {
        vector<int> res;
        if (r == nullptr) {
            return res;
        }
        stack<TreeNode*> stk;
        stk.push(r);
        while (!stk.empty()) {
            r = stk.top();
            stk.pop();
            res.push_back(r->val);
            if (r->right != nullptr) {
                stk.push(r->right);
            }
            if (r->left != nullptr) {
                stk.push(r->left);
            }
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：最坏情况下，$O\left(n\right)$

### 二叉树的中序遍历

https://leetcode.cn/problems/binary-tree-inorder-traversal

非递归版：

原理：对于每棵子树，都是先打印左子树，再打印根节点，再打印右子树

```c++
class Solution {
public:
    vector<int> inorderTraversal(TreeNode* r) {
        stack<TreeNode*> stk;
        vector<int> res;
        while (!stk.empty() || r != nullptr) {
            if (r != nullptr) {
                stk.push(r);
                r = r->left;
            } else {
                r = stk.top();
                stk.pop();
                res.push_back(r->val);
                r = r->right;
            }
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：最坏情况下，$O\left(n\right)$

### 二叉树的后序遍历

https://leetcode.cn/problems/binary-tree-postorder-traversal

非递归版：

```c++
class Solution {
public:
    vector<int> postorderTraversal(TreeNode* root) {
        vector<int> res;
        if (!root) {
            return res;
        }
        stack<TreeNode*> stk;
        stk.push(root);
        while (!stk.empty()) {
            root = stk.top();
            stk.pop();
            res.push_back(root->val);
            if (root->left) {
                stk.push(root->left);
            }
            if (root->right) {
                stk.push(root->right);
            }
        }
        reverse(res.begin(), res.end());
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：最坏情况下，$O\left(n\right)$

### 二叉树的层序遍历

https://leetcode.cn/problems/binary-tree-level-order-traversal

```c++
class Solution {
public:
    vector<vector<int>> levelOrder(TreeNode* root) {
        vector<vector<int>> res;
        if (!root) {
            return res;
        }
        queue<TreeNode*> q;
        q.push(root);
        unordered_map<TreeNode*, int> ht;
        ht[root] = 0;
        TreeNode* node;
        int depth;
        while (!q.empty()) {
            node = q.front();
            q.pop();
            depth = ht[node];
            if (depth == res.size()) {
                res.push_back({});
            }
            res[depth].push_back(node->val);
            if (node->left) {
                q.push(node->left);
                ht[node->left] = depth + 1;
            }
            if (node->right) {
                q.push(node->right);
                ht[node->right] = depth + 1;
            }
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

更推荐下面这种写法，更省空间：

```c++
const int MAXN = 2000;

class Solution {
private:
    TreeNode* q[MAXN];
    int h;
    int t;

public:
    Solution(): q{}, h(0), t(0) {
    }

    vector<vector<int>> levelOrder(TreeNode* root) {
        vector<vector<int>> res;
        if (root == nullptr) {
            return res;
        }
        h = t = 0;
        q[t++] = root;
        int n = -1;
        while (h < t) {
            n = t - h;
            vector<int> level(n);
            for (int& num: level) {
                root = q[h++];
                num = root->val;
                if (root->left != nullptr) {
                    q[t++] = root->left;
                }
                if (root->right != nullptr) {
                    q[t++] = root->right;
                }
            }
            res.push_back(level);
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

### 二叉树的锯齿形层序遍历

https://leetcode.cn/problems/binary-tree-zigzag-level-order-traversal

```c++
// 这题可以在上一题的基础上对 res 中的小部分进行逆序，下面的不用逆序的解法
class Solution {
private:
    TreeNode* q[2000];
    int h;
    int t;

public:
    Solution(): q{}, h(0), t(0) {
    }

    vector<vector<int>> zigzagLevelOrder(TreeNode* root) {
        vector<vector<int>> res;
        if (!root) {
            return res;
        }
        h = t = 0;
        q[t++] = root;
        TreeNode* node;
        int numCurrLevelNodes;
        bool isReverse = false;
        while (h < t) {
            numCurrLevelNodes = t - h;
            vector<int> level;
            for (int i = isReverse ? t - 1 : h; isReverse ? i >= h : i < t; isReverse ? --i : ++i) {
                level.push_back(q[i]->val);
            }
            for (int i = 0; i < numCurrLevelNodes; ++i) {
                node = q[h++];
                if (node->left) {
                    q[t++] = node->left;
                }
                if (node->right) {
                    q[t++] = node->right;
                }
            }
            res.push_back(level);
            isReverse = !isReverse;
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

### 二叉树最大宽度

https://leetcode.cn/problems/maximum-width-of-binary-tree

```c++
// 和堆一样，用编号记录左右孩子的位置，这样就不用记录空节点了
class Solution {
private:
    TreeNode* nq[3000];
    int iq[3000];
    int h;
    int t;

public:
    Solution(): nq{}, iq{}, h(0), t(0) {
    }

    int widthOfBinaryTree(TreeNode* root) {
        int res = 1;
        h = t = 0;
        nq[t] = root;
        iq[t++] = 0;
        TreeNode* node;
        int idx;
        int numCurrLevelNodes;
        while (h < t) {
            res = max(res, iq[t - 1] - iq[h] + 1);
            numCurrLevelNodes = t - h;
            for (int i = 0; i < numCurrLevelNodes; ++i) {
                node = nq[h];
                idx = iq[h++];
                if (node->left) {
                    nq[t] = node->left;
                    iq[t++] = (idx << 1) + 1;
                }
                if (node->right) {
                    nq[t] = node->right;
                    iq[t++] = (idx << 1) + 2;
                }
            }
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

### 二叉树的最大/小深度

https://leetcode.cn/problems/maximum-depth-of-binary-tree

https://leetcode.cn/problems/minimum-depth-of-binary-tree

```c++
class Solution {
public:
    int maxDepth(TreeNode* r) {
        return r == nullptr ? 0 : max(maxDepth(r->left), maxDepth(r->right)) + 1;
    }

    int minDepth(TreeNode* r) {
        if (r == nullptr) {
            return 0;
        }
        if (r->left == nullptr && r->right == nullptr) {
            return 1;
        }
        int left = INT_MAX;
        int right = INT_MAX;
        if (r->left) {
            left = minDepth(r->left);
        }
        if (r->right) {
            right = minDepth(r->right);
        }
        return min(left, right) + 1;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：最坏情况，$O\left(n\right)$

### 二叉树的序列化与反序列化

https://leetcode.cn/problems/serialize-and-deserialize-binary-tree

二叉树可以通过先序、后序或者按层遍历的方式序列化和反序列化，但是，二叉树无法通过中序遍历的方式实现序列化和反序列化，因为不同的两棵树，可能得到同样的中序序列，即便补了空位置也可能一样，比如如下两棵树：

```xml
  __2
 /
1
和
1__
   \
    2
```

补足空节点位置的中序遍历结果都是 null、1、null、2、null

先序遍历：

```c++
class Codec {
public:
    string serialize(TreeNode* root) {
        string res;
        f(root, res);
        return res;
    }

    TreeNode* deserialize(string str) {
        list<string> vals;
        string s;
        for (char& c: str) {
            if (c == ',') {
                vals.push_back(s);
                s.clear();
            } else {
                s.push_back(c);
            }
        }
        return g(vals);
    }

    void f(TreeNode* root, string& str) {
        if (!root) {
            str += "#,";
        } else {
            str += to_string(root->val) + ",";
            f(root->left, str);
            f(root->right, str);
        }
    }

    TreeNode* g(list<string>& vals) {
        if (vals.front() == "#") {
            vals.pop_front();
            return nullptr;
        }
        TreeNode* root = new TreeNode(stoi(vals.front()));
        vals.pop_front();
        root->left = g(vals);
        root->right = g(vals);
        return root;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：最坏情况，$O\left(n\right)$

层序遍历：

```c++
class Codec {
private:
    TreeNode* q[10000];
    int h;
    int t;

public:
    Codec(): q{}, h(0), t(0) {
    }

    string serialize(TreeNode* root) {
        if (!root) {
            return "#,";
        }
        h = t = 0;
        string res;
        q[t++] = root;
        res += to_string(root->val) + ",";
        TreeNode* node;
        while (h < t) {
            node = q[h++];
            if (node->left) {
                res += to_string(node->left->val) + ",";
                q[t++] = node->left;
            } else {
                res += "#,";
            }
            if (node->right) {
                res += to_string(node->right->val) + ",";
                q[t++] = node->right;
            } else {
                res += "#,";
            }
        }
        return res;
    }

    TreeNode* deserialize(string str) {
        list<string> vals;
        string s;
        for (char& c: str) {
            if (c == ',') {
                vals.push_back(s);
                s.clear();
            } else {
                s.push_back(c);
            }
        }
        TreeNode* root = createNode(vals.front());
        vals.pop_front();
        if (!root) {
            return nullptr;
        }
        h = t = 0;
        q[t++] = root;
        TreeNode* node;
        while (h < t) { // 也可以写成 !vals.empty()
            node = q[h++];
            node->left = createNode(vals.front());
            vals.pop_front();
            node->right = createNode(vals.front());
            vals.pop_front();
            if (node->left) {
                q[t++] = node->left;
            }
            if (node->right) {
                q[t++] = node->right;
            }
        }
        return root;
    }

    TreeNode* createNode(string& s) {
        return s == "#" ? nullptr : new TreeNode(stoi(s));
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

### 从前序与中序遍历序列构造二叉树

https://leetcode.cn/problems/construct-binary-tree-from-preorder-and-inorder-traversal

```c++
class Solution {
public:
    TreeNode* buildTree(vector<int>& preorder, vector<int>& inorder) {
        int n = inorder.size();
        unordered_map<int, int> ht;
        for (int i = 0; i < n; ++i) {
            ht[inorder[i]] = i;
        }
        return f(preorder, inorder, ht, 0, n - 1, 0, n - 1);
    }

    TreeNode* f(vector<int>& preorder, vector<int>& inorder, unordered_map<int, int>& ht, int l1, int r1, int l2, int r2) {
        if (l1 > r1) {
            return nullptr;
        }
        if (l1 == r1) {
            return new TreeNode(preorder[l1]);
        }
        int k = ht[preorder[l1]];
        return new TreeNode(preorder[l1],
                            f(preorder, inorder, ht, l1 + 1, l1 + k - l2, l2, k - 1),
                            f(preorder, inorder, ht, l1 + k - l2 + 1, r1, k + 1, r2
                            ));
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

### 二叉树的完全性检验

https://leetcode.cn/problems/check-completeness-of-a-binary-tree

```c++
class Solution {
private:
    TreeNode* q[100];
    int h;
    int t;

public:
    Solution(): q{}, h(0), t(0) {
    }

    bool isCompleteTree(TreeNode* root) {
        h = t = 0;
        q[t++] = root;
        TreeNode* node;
        bool mustLeaf = false;
        while (h < t) {
            node = q[h++];
            // 判断是否违规
            if (!node->left && node->right || mustLeaf && (node->left || node->right)) {
                return false;
            }
            if (node->left) {
                q[t++] = node->left;
            }
            if (node->right) {
                q[t++] = node->right;
            }
            if (!node->left || !node->right) {
                mustLeaf = true;
            }
        }
        return true; // 不违规返回 true
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

### 完全二叉树的节点个数

https://leetcode.cn/problems/count-complete-tree-nodes

```c++
class Solution {
public:
    int countNodes(TreeNode* root) {
        return f(root, 0, mostLeftDepth(root, 0));
    }

    // 以 root 为根节点的子树上共有多少个节点
    int f(TreeNode* root, int currDepth, int totalDepth) {
        if (!root) {
            return 0;
        }
        if (currDepth == totalDepth) {
            return 1;
        }
        // root 右子树的左边界扎到底了，此时 root 的左子树是满二叉树
        if (mostLeftDepth(root->right, currDepth + 1) == totalDepth) {
            return (1 << (totalDepth - currDepth)) + f(root->right, currDepth + 1, totalDepth);
        }
        // root 右子树的左边界没扎到底，证明 root 的右子树是满二叉树
        else {
            return (1 << (totalDepth - currDepth - 1)) + f(root->left, currDepth + 1, totalDepth);
        }
    }

    // root 的左边界能扎到第几层
    int mostLeftDepth(TreeNode* root, int currDepth) {
        TreeNode* node = root;
        int res = currDepth - 1;
        while (node) {
            ++res;
            node = node->left;
        }
        return res;
    }
};
```

时间复杂度：$O\left(\log^2 n\right)$

空间复杂度：$O\left(\log n\right)$

### 二叉树的最近公共祖先

https://leetcode.cn/problems/lowest-common-ancestor-of-a-binary-tree

https://www.bilibili.com/video/BV1194y16727

```c++
class Solution {
public:
    TreeNode* lowestCommonAncestor(TreeNode* root, TreeNode* p, TreeNode* q) {
        if (root == nullptr || root == p || root == q) {
            return root;
        }
        TreeNode* l = lowestCommonAncestor(root->left, p, q);
        TreeNode* r = lowestCommonAncestor(root->right, p, q);
        if (l != nullptr && r != nullptr) {
            return root;
        }
        if (l == nullptr && r == nullptr) {
            return nullptr;
        }
        return l != nullptr ? l : r;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：最坏情况下，$O\left(n\right)$

### 二叉搜索树的最近公共祖先

https://leetcode.cn/problems/lowest-common-ancestor-of-a-binary-search-tree

```c++
class Solution {
public:
    TreeNode* lowestCommonAncestor(TreeNode* root, TreeNode* p, TreeNode* q) {
        TreeNode* node = root;
        int minVal = min(p->val, q->val);
        int maxVal = max(p->val, q->val);
        // node 从上到下
        // 如果先遇到了 p，说明 p 是答案
        // 如果先遇到了 q，说明 q 是答案
        // 如果 root 在 p~q 的值之间，不用管 p 和 q 谁大谁小，只要 root 在中间，那么此时的 root 就是答案
        // 如果 root 在 p~q 的值的左侧，那么 root 往右移动
        // 如果 root 在 p~q 的值的右侧，那么 root 往左移动
        while (node && node->val != p->val && node->val != q->val) {
            if (node->val > minVal && node->val < maxVal) {
                break;
            }
            node = node->val < minVal ? node->right : node->left;
        }
        return node;
    }
};
```

时间复杂度：最坏情况下，$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 路径总和 II

https://leetcode.cn/problems/path-sum-ii

```c++
class Solution {
public:
    vector<vector<int>> pathSum(TreeNode* root, int targetSum) {
        vector<int> path;
        vector<vector<int>> res;
        if (!root) {
            return res;
        }
        backtrack(root, targetSum, path, res);
        return res;
    }

    // 以 root 为根节点的子树的哪些路径和为 remain，前面加上 path 后放到 res 里，函数执行完后 path 不变
    void backtrack(TreeNode* root, int remain, vector<int>& path, vector<vector<int>>& res) {
        if (!root->left && !root->right && root->val == remain) {
            path.push_back(root->val);
            res.push_back(path);
            path.pop_back();
        } else {
            path.push_back(root->val);
            // 要求 path 不允许改变的原因是，我希望下面两个 backtrack 中的 path 是一样的
            // 但是如果下面第一个 backtrack 改变了 path，那么这两个 backtrack 中的 path 就不一样了
            if (root->left) {
                backtrack(root->left, remain - root->val, path, res);
            }
            if (root->right) {
                backtrack(root->right, remain - root->val, path, res);
            }
            path.pop_back();
        }
    }
};
```

时间复杂度：最坏情况下，树的上半部分为链状，下半部分为完全二叉树，并且从根节点到每一个叶子节点的路径都符合题目要求，路径个数为 $O\left(n\right)$，每条路径长度为 $O\left(n\right)$，要将这些路径全部添加进答案中，总体时间复杂度为 $O\left(n^2\right)$

空间复杂度：最坏情况下，$O\left(n\right)$

### 平衡二叉树

https://leetcode.cn/problems/balanced-binary-tree

```c++
class Solution {
private:
    bool balanced;

public:
    Solution(): balanced(true) {
    }

    bool isBalanced(TreeNode* root) {
        // balanced 是全局变量，所有调用过程共享
        // 所以每次判断开始时，设置为 true
        balanced = true;
        getHeight(root);
        return balanced;
    }

    int getHeight(TreeNode* root) {
        if (!balanced || !root) {
            return 0; // 一旦发现不平衡，返回什么高度已经不重要了
        }
        int leftHeight = getHeight(root->left);
        int rightHeight = getHeight(root->right);
        if (abs(leftHeight - rightHeight) > 1) {
            balanced = false;
        }
        return max(leftHeight, rightHeight) + 1;
    }
};
```

时间复杂度：最坏情况下，$O\left(n^2\right)$

空间复杂度：最坏情况下，$O\left(n\right)$

### 验证二叉搜索树

https://leetcode.cn/problems/validate-binary-search-tree

中序遍历：

```c++
const int MAXN = 1e4;

class Solution {
private:
    TreeNode* stk[MAXN];
    int t;

public:
    // 非递归版的中序遍历，如果中序遍历是严格升序，则是 BST
    bool isValidBST(TreeNode* root) {
        t = 0;
        TreeNode* prevNode = nullptr;
        while (t > 0 || root != nullptr) {
            if (root != nullptr) {
                stk[t++] = root;
                root = root->left;
            } else {
                root = stk[--t];
                if (prevNode != nullptr && root->val <= prevNode->val) {
                    return false;
                }
                prevNode = root;
                root = root->right;
            }
        }
        return true;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：最坏情况下，$O\left(n\right)$

递归：

```c++
class Solution {
private:
    typedef long long ll;
    ll minVal;
    ll maxVal;

public:
    bool isValidBST(TreeNode* root) {
        if (!root) {
            minVal = LLONG_MAX;
            maxVal = LLONG_MIN;
            return true;
        }
        bool leftIsBST = isValidBST(root->left);
        ll leftMin = minVal;
        ll leftMax = maxVal;
        bool rightIsBST = isValidBST(root->right);
        ll rightMin = minVal;
        ll rightMax = maxVal;
        minVal = min((ll) root->val, min(leftMin, rightMin));
        maxVal = max((ll) root->val, max(leftMax, rightMax));
        return leftIsBST && rightIsBST && root->val > leftMax && root->val < rightMin;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：最坏情况下，$O\left(n\right)$

### 修剪二叉搜索树

https://leetcode.cn/problems/trim-a-binary-search-tree

```c++
class Solution {
public:
    TreeNode* trimBST(TreeNode* root, int low, int high) {
        if (!root) {
            return nullptr;
        }
        if (root->val < low) {
            return trimBST(root->right, low, high);
        }
        if (root->val > high) {
            return trimBST(root->left, low, high);
        }
        root->left = trimBST(root->left, low, high);
        root->right = trimBST(root->right, low, high);
        return root;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：最坏情况下，$O\left(n\right)$

### 打家劫舍 III

https://leetcode.cn/problems/house-robber-iii

```c++
class Solution {
private:
    // 全局变量，完成了 X 子树的遍历，返回之后
    // robMax 变成，X 子树在偷头节点的情况下，最大的收益
    // notRobMax 意思相反
    int robMax;
    int notRobMax;

public:
    int rob(TreeNode* root) {
        if (!root) {
            robMax = 0;
            notRobMax = 0;
            return 0;
        }
        int currRobMax = root->val;
        int currNotRobMax = 0;
        rob(root->left);
        currRobMax += notRobMax;
        currNotRobMax += max(robMax, notRobMax);
        rob(root->right);
        currRobMax += notRobMax;
        currNotRobMax += max(robMax, notRobMax);
        robMax = currRobMax;
        notRobMax = currNotRobMax;
        return max(robMax, notRobMax);
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：最坏情况下，$O\left(n\right)$

# 哈希表、有序表、堆

## 总结

### 哈希表、有序表、堆的对比

|            | 哈希表                                                  | 有序表                                                       | 堆（即优先级队列）                                           |
| ---------- | ------------------------------------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 时间复杂度 | 增删改查：$O\left(1\right)$                             | 增删改查：$O\left(\log n\right)$                             | 删最大/小值和增改：$O\left(\log n\right)$；查最大/小值：$O\left(1\right)$ |
| 是否去重   | 是                                                      | 是                                                           | 否                                                           |
| 备注       | 当 key 的范围是固定且可控的情况下，可以用数组替代哈希表 | 可以通过红黑树、AVL 树、SB 树、替罪羊树、Treap、Splay、跳表等很多结构都可实现同样功能 | 更多有关堆的内容见「排序」章节中的「堆」部分                 |

### C++ 中的比较器

有序表：

```c++
#include <bits/stdc++.h>
using namespace std;

struct Student {
    int id;
    string name;

    Student(int id, const string& name): id(id), name(name) {
    }
};

struct Comp {
    bool operator()(const Student& student1, const Student& student2) const {
        // 按 id 升序排列
        // return student1.id < student2.id;
        // 如果 id 相同，则按 name 升序排列
        return student1.id != student2.id ? student1.id < student2.id : student1.name < student2.name;
    }
};

int main() {
    set<Student, Comp> studentSet;
    Student student1(0, "b");
    Student student2(0, "a");
    studentSet.insert(student1);
    studentSet.insert(student2);
    for (const Student& student: studentSet) {
        cout << student.id << ' ' << student.name << endl;
    }
    // 0 a
    // 0 b
    return 0;
}
```

堆（比较器中的比较顺序变了）：

```c++
#include <bits/stdc++.h>
using namespace std;

struct Student {
    int id;
    string name;

    Student(int id, const string& name): id(id), name(name) {
    }
};

struct Comp {
    bool operator()(const Student& student1, const Student& student2) const {
        // 按 id 升序排列
        // return student1.id > student2.id;
        // 如果 id 相同，则按 name 升序排列
        return student1.id != student2.id ? student1.id > student2.id : student1.name > student2.name;
    }
};

int main() {
    priority_queue<Student, vector<Student>, Comp> pq;
    Student student1(0, "b");
    Student student2(0, "a");
    pq.push(student1);
    pq.push(student2);
    while (!pq.empty()) {
        cout << pq.top().id << ' ' << pq.top().name << endl;
        pq.pop();
    }
    // 0 a
    // 0 b
    return 0;
}
```

排序：

```c++
#include <bits/stdc++.h>
using namespace std;

struct Student {
    int id;
    string name;

    Student(int id, const string& name): id(id), name(name) {
    }
};

int main() {
    vector<Student> students;
    students.push_back(Student(0, "b"));
    students.push_back(Student(0, "a"));
    sort(students.begin(), students.end(), [](Student& student1, Student& student2) {
        return student1.id != student2.id ? student1.id < student2.id : student1.name < student2.name;
    });
    for (const Student& student: students) {
        cout << student.id << ' ' << student.name << endl;
    }
    // 0 a
    // 0 b
    return 0;
}
```

## 题目

### 合并 K 个升序链表

https://leetcode.cn/problems/merge-k-sorted-lists

```c++
// 这里的参数类型不要写成 ListNode*& 否则会报错
struct Comp {
    bool operator()(const ListNode* p1, const ListNode* p2) const {
        return p1->val > p2->val;
    }
};

class Solution {
public:
    ListNode* mergeKLists(vector<ListNode*>& lsts) {
        priority_queue<ListNode*, vector<ListNode*>, Comp> pq;
        for (ListNode*& h: lsts) {
            if (h != nullptr) {
                pq.push(h);
            }
        }
        ListNode* dh = new ListNode();
        ListNode* p = dh;
        while (!pq.empty()) {
            p->next = pq.top();
            pq.pop();
            p = p->next;
            if (p->next != nullptr) {
                pq.push(p->next);
            }
        }
        return dh->next;
    }
};
```

时间复杂度：$O\left(n\log k\right)$

空间复杂度：$O\left(k\right)$

### 线段重合

https://www.nowcoder.com/practice/1ae8d0b6bb4e4bcdbf64ec491f63fc37

内置堆：

```c++
#include <bits/stdc++.h>
using namespace std;

int MAXN = 10000;
int n;
vector<vector<int>> lines(MAXN, vector<int>(2));

bool comp(vector<int>& line1, vector<int>& line2) {
    return line1[0] < line2[0];
}

int getMaxCover() {
    sort(lines.begin(), lines.begin() + n, comp);
    priority_queue<int, vector<int>, greater<int>> pq; // 小根堆，存放线段的右端位置
    int res = 0;
    for (int i = 0; i < n; ++i) { // 答案区域一定是某条线段的左端位置，枚举所有可能性
        while (!pq.empty() && pq.top() <= lines[i][0]) {
            pq.pop();
        }
        pq.push(lines[i][1]);
        // 有 pq.size() 条线段可以「穿过」当前线段的左端
        // 也就是说如果答案区域的左端是当前线段的左端时，此时重合的线段有 pq.size() 条
        res = max(res, (int) pq.size());
    }
    return res;
}

int main() {
    while (cin >> n) {
        for (int i = 0; i < n; ++i) {
            cin >> lines[i][0] >> lines[i][1];
        }
        cout << getMaxCover() << endl;
    }
    return 0;
}
```

时间复杂度：$O\left(n\log n\right)$

空间复杂度：$O\left(n\right)$

### 将数组和减半的最少操作次数

https://leetcode.cn/problems/minimum-operations-to-halve-array-sum

```c++
class Solution {
public:
    int halveArray(vector<int>& nums) {
        priority_queue<double> pq;
        double target = 0.0;
        for (int& num: nums) {
            pq.push(num);
            target += num;
        }
        target /= 2.0;
        double curr;
        int res = 0;
        while (target > 0.0) {
            curr = pq.top(); // 贪心
            pq.pop();
            curr /= 2.0;
            pq.push(curr);
            target -= curr;
            ++res;
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\log n\right)$

空间复杂度：$O\left(n\right)$

手写堆优化：

```c++
#define ll long long

class Solution {
private:
    ll heap[100000];
    int size;

public:
    int halveArray(vector<int>& nums) {
        int n = nums.size();
        double target = 0.0;
        for (int i = 0; i < n; ++i) {
            heap[i] = (ll) nums[i] << 20; // 自己控制精度，现在无需使用 double
            target += heap[i];
        }
        size = n;
        target /= 2.0;
        for (int i = (n >> 1) - 1; i >= 0; --i) {
            heapify(i);
        }
        int res = 0;
        while (target > 0.0) {
            heap[0] /= 2.0;
            target -= heap[0];
            heapify(0);
            ++res;
        }
        return res;
    }

    void heapify(int idx) {
        int l = (idx << 1) + 1, bestIdx;
        while (l < size) {
            bestIdx = l + 1 < size && heap[l + 1] > heap[l] ? l + 1 : l;
            bestIdx = heap[bestIdx] > heap[idx] ? bestIdx : idx;
            if (bestIdx == idx) {
                break;
            }
            swap(heap[idx], heap[bestIdx]);
            idx = bestIdx;
            l = (idx << 1) + 1;
        }
    }
};
```

时间复杂度：$O\left(n\log n\right)$

空间复杂度：$O\left(n\right)$

# 前缀树

## 总结

前缀树的使用场景：需要根据前缀信息进行查询

前缀树的优点：根据前缀信息选择树上的分支，可以节省大量的时间

前缀树的缺点：比较浪费空间，和总字符数量有关，字符的种类有关

前缀树题目中的 `MAXN` 需要自己试出来

## 题目

### 字典树的实现

https://www.nowcoder.com/practice/7f8a8553ddbf4eaab749ec988726702b

推荐使用下面的静态变量的实现：

```c++
#include <bits/stdc++.h>
using namespace std;

const int MAXN = 1e6;  // 试出来的

class Trie {
public:
    // tree、pass、end_ 中的元素全是 0
    int tree[MAXN][26];
    int pass[MAXN];
    int end_[MAXN];
    int cnt; // 当前用到了 tree 中的哪个空间

    Trie(): tree{}, pass{}, end_{}, cnt(1) {
    }

    void insert(string& str) {
        int curr = 1;
        ++pass[curr];
        for (int i = 0, path; i < str.size(); ++i) {
            path = str[i] - 'a';
            if (tree[curr][path] == 0) {
                tree[curr][path] = ++cnt;
            }
            curr = tree[curr][path];
            ++pass[curr];
        }
        ++end_[curr];
    }

    int search(string& str) {
        int curr = 1;
        for (int i = 0, path; i < str.size(); ++i) {
            path = str[i] - 'a';
            if (tree[curr][path] == 0) {
                return 0;
            }
            curr = tree[curr][path];
        }
        return end_[curr];
    }

    int numPrefixes(string& str) {
        int curr = 1;
        for (int i = 0, path; i < str.size(); ++i) {
            path = str[i] - 'a';
            if (tree[curr][path] == 0) {
                return 0;
            }
            curr = tree[curr][path];
        }
        return pass[curr];
    }

    void remove(string& str) {
        if (search(str) == 0) {
            return;
        }
        int curr = 1;
        for (int i = 0, path; i < str.size(); ++i) {
            path = str[i] - 'a';
            if (--pass[tree[curr][path]] == 0) {
                tree[curr][path] = 0;
                return;
            }
            curr = tree[curr][path];
        }
        --end_[curr];
    }

    void clear() {
        for (int i = 1; i <= cnt; ++i) {
            fill_n(tree[i], 26, 0);
            pass[i] = 0;
            end_[i] = 0;
        }
        cnt = 1;
    }
};


int main() {
    int n;
    int op;
    string str;
    Trie trie;
    while (cin >> n) {
        while (n--) {
            cin >> op >> str;
            if (op == 1) {
                trie.insert(str);
            } else if (op == 2) {
                trie.remove(str);
            } else if (op == 3) {
                cout << (trie.search(str) > 0 ? "YES" : "NO") << endl;
            } else if (op == 4) {
                cout << trie.numPrefixes(str) << endl;
            }
        }
        trie.clear();
    }
    return 0;
}
```

$n$ 为操作次数、$m$ 为所有单词的最大长度：

时间复杂度：$O\left(nm\right)$

空间复杂度：$O\left(nm\right)$

### 接头密匙

https://www.nowcoder.com/practice/c552d3b4dfda49ccb883a6371d9a6932

```c++
const int MAXN = 1e5;

class Solution {
public:
    int tree[MAXN][12];
    int pass[MAXN];
    int cnt = 1;

    Solution(): tree{}, pass{} {
    }

    vector<int> countConsistentKeys(vector<vector<int>>& b, vector<vector<int>>& a) {
        string str;
        for (vector<int>& nums: a) {
            for (int i = 1; i < nums.size(); ++i) {
                str += to_string(nums[i] - nums[i - 1]) + "#";
            }
            insert(str);
            str.clear();
        }
        vector<int> res(b.size());
        for (int i = 0; i < b.size(); ++i) {
            for (int j = 1; j < b[i].size(); ++j) {
                str += to_string(b[i][j] - b[i][j - 1]) + "#";
            }
            res[i] = numPrefixes(str);
            str.clear();
        }
        clear();
        return res;
    }

    int getPath(char c) {
        if (c == '-') {
            return 10;
        }
        if (c == '#') {
            return 11;
        }
        return c - '0';
    }

    void insert(string& str) {
        int curr = 1;
        ++pass[curr];
        for (int i = 0, path; i < str.size(); ++i) {
            path = getPath(str[i]);
            if (tree[curr][path] == 0) {
                tree[curr][path] = ++cnt;
            }
            curr = tree[curr][path];
            ++pass[curr];
        }
    }

    int numPrefixes(string& str) {
        int curr = 1;
        for (int i = 0, path; i < str.size(); ++i) {
            path = getPath(str[i]);
            if (tree[curr][path] == 0) {
                return 0;
            }
            curr = tree[curr][path];
        }
        return pass[curr];
    }

    void clear() {
        for (int i = 1; i <= cnt; ++i) {
            fill_n(tree[i], 12, 0);
            pass[i] = 0;
        }
        cnt = 1;
    }
};
```

$n_a$ 为 `a` 的大小、$n_b$ 为 `b` 的大小、$m$ 为所有字符串的最大长度：

时间复杂度：$O\left(n_am+n_bm\right)$

空间复杂度：$O\left(n_am\right)$

### 数组中两个数的最大异或值

https://leetcode.cn/problems/maximum-xor-of-two-numbers-in-an-array

```c++
const int MAXN = 3e6;

class Solution {
public:
    int tree[MAXN][2];
    int cnt;
    int high;

    Solution(): tree{}, cnt(1), high(32) {
    }

    int findMaximumXOR(vector<int>& nums) {
        build(nums);
        int res = 0;
        for (int& num: nums) {
            res = max(res, maxXor(num));
        }
        clear();
        return res;
    }

    void build(vector<int>& nums) {
        high = 31 - __countl_zero(*max_element(nums.begin(), nums.end()));
        for (int& num: nums) {
            insert(num);
        }
    }

    void insert(int num) {
        int curr = 1;
        for (int i = high, path; i >= 0; --i) {
            path = (num >> i) & 1;
            if (tree[curr][path] == 0) {
                tree[curr][path] = ++cnt;
            }
            curr = tree[curr][path];
        }
    }

    int maxXor(int num) {
        int curr = 1;
        int res = 0;
        for (int i = high, status, path; i >= 0; --i) {
            status = (num >> i) & 1;
            path = status ^ 1;
            if (tree[curr][path] == 0) {
                path ^= 1;
            }
            res |= (status ^ path) << i;
            curr = tree[curr][path];
        }
        return res;
    }

    void clear() {
        for (int i = 1; i <= cnt; ++i) {
            fill_n(tree[i], 2, 0);
        }
        cnt = 1;
    }
};
```

时间复杂度：$O\left(\right)$

空间复杂度：$O\left(n_am\right)$

# 数据结构设计

## 总结

## 题目

### 设计有 setAll 功能的哈希表

https://www.nowcoder.com/practice/7c4559f138e74ceb9ba57d76fd169967

```c++
#include <bits/stdc++.h>
using namespace std;

// 加时间戳
class MyHashTable {
private:
    unordered_map<int, vector<int>> ht;
    int currTime;
    int setAllValue;
    int setAllTime;

public:
    MyHashTable(): currTime(0), setAllTime(-1), setAllValue(-1) {
    }

    void put(int key, int value) {
        if (!ht.count(key)) {
            ht[key].resize(2);
        }
        ht[key][0] = value;
        ht[key][1] = currTime++;
    }

    int get(int key) {
        if (!ht.count(key)) {
            return -1;
        }
        return ht[key][1] > setAllTime ? ht[key][0] : setAllValue;
    }

    void setAll(int value) {
        setAllValue = value;
        setAllTime = currTime++;
    }
};

int main() {
    int n, op, key, value;
    while (cin >> n) {
        MyHashTable myHashTable;
        while (n--) {
            cin >> op;
            if (op == 1) {
                cin >> key >> value;
                myHashTable.put(key, value);
            } else if (op == 2) {
                cin >> key;
                cout << myHashTable.get(key) << endl;
            } else if (op == 3) {
                cin >> value;
                myHashTable.setAll(value);
            } else {
                cout << "无效操作" << endl;
            }
        }
    }
    return 0;
}
```

`put`、`get`、`setAll` 方法：

时间复杂度：$O\left(1\right)$

空间复杂度：$O\left(1\right)$

整个数据结构：

空间复杂度：$O\left(n\right)$，$n$ 为哈希表中的数据量

### LRU 缓存

https://leetcode.cn/problems/lru-cache

哈希表 + 双向链表

分析思路：

记录每条记录的操作顺序 -> 链表

以 $O\left(1\right)$ 的时间复杂度查找记录 -> 哈希表

以 $O\left(1\right)$ 的时间复杂度将节点添加到链表尾部 -> 双向链表

**分析的过程中出现什么需求就添加/修改对应的数据结构**

```c++
struct BiListNode {
    int k;
    int v;
    BiListNode* prev;
    BiListNode* next;

    BiListNode() : k(-1), v(-1), prev(nullptr), next(nullptr) {
    }

    BiListNode(int k, int v) : k(k), v(v), prev(nullptr), next(nullptr) {
    }
};

class LRUCache {
public:
    unordered_map<int, BiListNode*> ht;
    BiListNode* dh;
    BiListNode* dt;
    int c;

    LRUCache(int c): dh(new BiListNode()), dt(new BiListNode()), c(c) {
        dh->next = dt;
        dt->prev = dh;
    }

    int get(int k) {
        if (ht.count(k) == 0) {
            return -1;
        }
        BiListNode* p = ht[k];
        moveToTail(p);
        return p->v;
    }

    void put(int k, int v) {
        BiListNode* p = nullptr;
        if (ht.count(k) == 0) {
            if (ht.size() == c) {
                ht.erase(removeHead());
            }
            p = new BiListNode(k, v);
            ht[k] = p;
            addToTail(p);
        } else {
            p = ht[k];
            p->v = v;
            moveToTail(p);
        }
    }

    void moveToTail(BiListNode* p) {
        if (p == dt->prev) {
            return;
        }
        skipNodes(p);
        addToTail(p);
    }

    int removeHead() {
        int res = dh->next->k;
        skipNodes(dh->next);
        return res;
    }

    void addToTail(BiListNode* p) {
        p->prev = dt->prev;
        p->next = dt;
        p->prev->next = p;
        dt->prev = p;
    }

    void skipNodes(BiListNode* p) {
        p->prev->next = p->next;
        p->next->prev = p->prev;
    }
};
```

`get`、`put` 方法：

时间复杂度：$O\left(1\right)$

空间复杂度：$O\left(1\right)$

整个数据结构：

空间复杂度：$O\left(c\right)$，$c$ 为 `capacity`

### O(1) 时间插入、删除和获取随机元素

https://leetcode.cn/problems/insert-delete-getrandom-o1

```c++
class RandomizedSet {
private:
    unordered_map<int, int> ht; // key：num，value：nums 中的下标
    vector<int> nums;

public:
    RandomizedSet() {
        nums.resize(200000);
        srand(time(nullptr));
    }

    bool insert(int num) {
        if (ht.count(num)) {
            return false;
        }
        ht[num] = ht.size();
        nums[ht.size() - 1] = num;
        return true;
    }

    bool remove(int num) {
        if (!ht.count(num)) {
            return false;
        }
        int idx = ht[num];
        int endVal = nums[ht.size() - 1];
        ht[endVal] = idx;
        ht.erase(num);
        nums[idx] = endVal;
        return true;
    }

    int getRandom() {
        return nums[rand() % ht.size()];
    }
};
```

`insert`、`remove`、`getRandom` 方法：

时间复杂度：$O\left(1\right)$

空间复杂度：$O\left(1\right)$

整个数据结构：

空间复杂度：$O\left(n\right)$，$n$ 为最多同时存在多少个元素

### O(1) 时间插入、删除和获取随机元素 - 允许重复

https://leetcode.cn/problems/insert-delete-getrandom-o1-duplicates-allowed

```c++
class RandomizedCollection {
private:
    unordered_map<int, unordered_set<int>> ht;
    vector<int> nums;
    int size;

public:
    RandomizedCollection() {
        nums.resize(200000);
        size = 0;
        srand(time(nullptr));
    }

    bool insert(int num) {
        ht[num].insert(size);
        nums[size] = num;
        ++size;
        return ht[num].size() == 1;
    }

    bool remove(int num) {
        if (!ht.count(num)) {
            return false;
        }
        int idx = *ht[num].begin();
        int endVal = nums[size - 1];
        if (num == endVal) {
            ht[num].erase(size - 1);
        } else {
            ht[num].erase(idx);
            ht[endVal].erase(size - 1);
            ht[endVal].insert(idx);
            nums[idx] = endVal;
        }
        if (ht[num].empty()) {
            ht.erase(num);
        }
        --size;
        return true;
    }

    int getRandom() {
        return nums[rand() % size];
    }
};
```

`insert`、`remove`、`getRandom` 方法：

时间复杂度：$O\left(1\right)$

空间复杂度：$O\left(1\right)$

整个数据结构：

空间复杂度：$O\left(n\right)$，$n$ 为最多同时存在多少个元素

### 数据流的中位数

https://leetcode.cn/problems/find-median-from-data-stream

```c++
class MedianFinder {
private:
    priority_queue<int> maxHeap;
    priority_queue<int, vector<int>, greater<int>> minHeap;

public:
    MedianFinder() {
    }

    void addNum(int num) {
        if (maxHeap.empty() || num <= maxHeap.top()) {
            maxHeap.push(num);
        } else {
            minHeap.push(num);
        }
        if (minHeap.size() - maxHeap.size() == 2) {
            maxHeap.push(minHeap.top());
            minHeap.pop();
        } else if (maxHeap.size() - minHeap.size() == 2) {
            minHeap.push(maxHeap.top());
            maxHeap.pop();
        }
    }

    double findMedian() {
        return maxHeap.size() == minHeap.size() ? (maxHeap.top() + minHeap.top()) / 2.0 : maxHeap.size() > minHeap.size() ? maxHeap.top() : minHeap.top();
    }
};
```

`addNum` 方法：

时间复杂度：$O\left(\log n\right)$

空间复杂度：$O\left(1\right)$

`findMedian` 方法：

时间复杂度：$O\left(1\right)$

空间复杂度：$O\left(1\right)$

整个数据结构：

空间复杂度：$O\left(n\right)$，$n$ 为最多同时存在多少个元素

### 最大频率栈

https://leetcode.cn/problems/maximum-frequency-stack

```c++
class FreqStack {
private:
    unordered_map<int, int> counts;
    unordered_map<int, vector<int>> ht;
    int maxFreq; // 最大的频次值，即 ht 中 key 的最大值

public:
    FreqStack(): maxFreq(0) {
    }

    void push(int num) {
        int currFreq = ++counts[num];
        ht[currFreq].push_back(num);
        maxFreq = max(maxFreq, currFreq);
    }

    int pop() {
        int res = ht[maxFreq].back();
        if (!--counts[res]) {
            counts.erase(res);
        }
        ht[maxFreq].pop_back();
        if (!ht[maxFreq].size()) {
            ht.erase(maxFreq);
            --maxFreq;
        }
        return res;
    }
};
```

`push`、`pop` 方法：

时间复杂度：$O\left(1\right)$

空间复杂度：$O\left(1\right)$

整个数据结构：

空间复杂度：$O\left(n\right)$，$n$ 为最多同时存在多少个元素

### 全 O(1) 的数据结构

https://leetcode.cn/problems/all-oone-data-structure

```c++
struct BiListNode {
    int cnt;
    unordered_set<string> hs;
    BiListNode* prev;
    BiListNode* next;

    BiListNode(int cnt) : cnt(cnt), hs({}), prev(nullptr), next(nullptr) {
    }

    BiListNode(int cnt, string str) : cnt(cnt), hs({str}), prev(nullptr), next(nullptr) {
    }
};

class AllOne {
private:
    unordered_map<string, BiListNode*> ht;
    BiListNode* dummyHead;
    BiListNode* dummyTail;

public:
    AllOne() {
        dummyHead = new BiListNode(0, "");
        dummyTail = new BiListNode(INT_MAX, "");
        dummyHead->next = dummyTail;
        dummyTail->prev = dummyHead;
    }

    void inc(string str) {
        if (!ht.count(str)) {
            if (dummyHead->next->cnt == 1) {
                ht[str] = dummyHead->next;
                dummyHead->next->hs.insert(str);
            } else {
                ht[str] = new BiListNode(1, str);
                insert(dummyHead, ht[str]);
            }
        } else {
            BiListNode* node = ht[str];
            if (node->next->cnt == node->cnt + 1) {
                ht[str] = node->next;
                node->next->hs.insert(str);
            } else {
                ht[str] = new BiListNode(node->cnt + 1, str);
                insert(node, ht[str]);
            }
            node->hs.erase(str);
            if (node->hs.empty()) {
                remove(node);
            }
        }
    }

    void dec(string str) {
        BiListNode* node = ht[str];
        if (node->cnt == 1) {
            ht.erase(str);
        } else {
            if (node->prev->cnt == node->cnt - 1) {
                ht[str] = node->prev;
                node->prev->hs.insert(str);
            } else {
                ht[str] = new BiListNode(node->cnt - 1, str);
                insert(node->prev, ht[str]);
            }
        }
        node->hs.erase(str);
        if (node->hs.empty()) {
            remove(node);
        }
    }

    string getMaxKey() {
        return *dummyTail->prev->hs.begin();
    }

    string getMinKey() {
        return *dummyHead->next->hs.begin();
    }

    void insert(BiListNode* before, BiListNode* newNode) {
        newNode->prev = before;
        newNode->next = before->next;
        before->next = newNode;
        newNode->next->prev = newNode;
    }

    void remove(BiListNode* node) {
        node->prev->next = node->next;
        node->next->prev = node->prev;
        delete node;
    }
};
```

`inc`、`dec`、`getMaxKey`、`getMinKey` 方法：

时间复杂度：$O\left(1\right)$

空间复杂度：$O\left(1\right)$

整个数据结构：

空间复杂度：$O\left(n\right)$，$n$ 为最多同时存在多少个元素

# HOT 100 待补充

## 哈希

### 两数之和

https://leetcode.cn/problems/two-sum

```c++
class Solution {
public:
    vector<int> twoSum(vector<int>& nums, int target) {
        int n = nums.size();
        unordered_map<int, int> ht;
        vector<int> res;
        for (int i = 0; i < n; ++i) {
            if (ht.count(target - nums[i])) {
                res = {ht[target - nums[i]], i};
                break;
            } else {
                ht[nums[i]] = i;
            }
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

### *字母异位词分组

https://leetcode.cn/problems/group-anagrams

```c++
class Solution {
public:
    vector<vector<string>> groupAnagrams(vector<string>& strs) {
        unordered_map<string, vector<string>> ht;
        string key;
        for (string& str: strs) {
            key = str;
            // 把排序后的字符串当作 key
            sort(key.begin(), key.end());
            ht[key].push_back(str);
        }
        vector<vector<string>> res(ht.size());
        int i = 0;
        for (auto& kv: ht) {
            res[i++] = kv.second;
        }
        return res;
    }
};
```

时间复杂度：$O\left(nk\log k\right)$，$k$ 为字符串最大长度

空间复杂度：$O\left(nk\right)$

```c++
class Solution {
public:
    vector<vector<string>> groupAnagrams(vector<string>& strs) {
        unordered_map<string, vector<string>> ht;
        string key;
        for (string& str: strs) {
            key = getKey(str);
            ht[key].push_back(str);
        }
        vector<vector<string>> res(ht.size());
        int i = 0;
        for (auto& kv: ht) {
            res[i++] = kv.second;
        }
        return res;
    }

    // 自定义 key
    string getKey(string& str) {
        int counts[26] = {};
        for (char& c: str) {
            ++counts[c - 'a'];
        }
        string res;
        for (char i = 0; i < 26; ++i) {
            res.push_back(i + 'a');
            res.append(to_string(counts[i]));
        }
        return res;
    }
};
```

时间复杂度：$O\left(nk\right)$

空间复杂度：$O\left(nk\right)$

### *最长连续序列

https://leetcode.cn/problems/longest-consecutive-sequence

```c++
class Solution {
public:
    int longestConsecutive(vector<int>& nums) {
        unordered_set<int> hs;
        for (int& num: nums) {
            hs.insert(num);
        }
        int currNum = -1e9 - 1;
        int currLen = -1;
        int res = 0;
        for (const int& start: hs) {
            if (hs.find(start - 1) != hs.end()) {
                continue;
            }
            currNum = start;
            currLen = 1;
            while (hs.find(currNum + 1) != hs.end()) {
                ++currNum;
                ++currLen;
            }
            res = max(res, currLen);
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

## 双指针

### 移动零

https://leetcode.cn/problems/move-zeroes

```c++
class Solution {
public:
    void moveZeroes(vector<int>& nums) {
        int n = nums.size(), l = 0, r = 0; // 0 区域是 [i,j)
        while (r < n) {
            if (nums[r]) {
                swap(nums[l++], nums[r++]);
            } else {
                ++r;
            }
        }
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 盛最多水的容器

https://leetcode.cn/problems/container-with-most-water

https://leetcode.cn/problems/container-with-most-water/solutions/11491/container-with-most-water-shuang-zhi-zhen-fa-yi-do

```c++
class Solution {
public:
    int maxArea(vector<int>& heights) {
        int n = heights.size();
        int l = 0;
        int r = n - 1;
        int res = 0;
        while (l < r) {
            res = max(res, (r - l) * min(heights[l], heights[r]));
            heights[l] < heights[r] ? ++l : --r;
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### *三数之和

https://leetcode.cn/problems/3sum

```c++
class Solution {
public:
    vector<vector<int>> threeSum(vector<int>& nums) {
        sort(nums.begin(), nums.end());
        int n = nums.size();
        vector<vector<int>> res;
        for (int i = 0, j, k, sum; i < n - 2; ++i) {
            if (nums[i] > 0) {
                break;
            }
            if (i && nums[i] == nums[i - 1]) {
                continue;
            }
            j = i + 1;
            k = n - 1;
            while (j < k) {
                sum = nums[i] + nums[j] + nums[k];
                if (sum < 0) {
                    while (j < k && nums[j] == nums[++j]) {
                    }
                } else if (sum > 0) {
                    while (j < k && nums[k] == nums[--k]) {
                    }
                } else {
                    res.push_back(vector<int>{nums[i], nums[j], nums[k]});
                    while (j < k && nums[j] == nums[++j]) {
                    }
                    while (j < k && nums[k] == nums[--k]) {
                    }
                }
            }
        }
        return res;
    }
};
```

时间复杂度：$O\left(n^2\right)$

空间复杂度：$O\left(\log n\right)$，如果用随机快速排序的话

### *接雨水

https://leetcode.cn/problems/trapping-rain-water

```c++
class Solution {
public:
	int trap(vector<int>& heights) {
		int n = heights.size();
		vector<int> lmhs(n); // leftMaxHeights，i 位置左边的最大高度是多少，下同
		vector<int> rmhs(n); // rightMaxHeights
		for (int i = 1; i < n; ++i) {
			lmhs[i] = max(lmhs[i - 1], heights[i - 1]);
		}
		for (int i = n - 2; i >= 0; --i) {
			rmhs[i] = max(rmhs[i + 1], heights[i + 1]);
		}
		int res = 0;
		for (int i = 1; i < n - 1; ++i) { // i 位置上有多少水
			res += max(0, min(lmhs[i], rmhs[i]) - heights[i]);
		}
		return res;
	}
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

由上面的代码优化而来，如果 `leftMax <= rightMax`，此时 `[0, i - 1]` 范围上的最大值是 `leftMax`，`[i + 1, n - 1]` 范围上的最大值一定大于等于 `rightMax`，所以 `min(max([0, i - 1]), max(i + 1, n - 1))` 一定是 `leftMax`，结算更小的一边即可。

```c++
class Solution {
public:
	int trap(vector<int>& heights) {
		int n = heights.size();
		int l = 1;
		int r = n - 2;
		int lMax = heights[0];
		int rMax = heights[n - 1];
		int res = 0;
		while (l <= r) {
			if (lMax < rMax) {
				res += max(0, lMax - heights[l]);
				lMax = max(lMax, heights[l++]);
			}
			else {
				res += max(0, rMax - heights[r]);
				rMax = max(rMax, heights[r--]);
			}
		}
		return res;
	}
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

## 滑动窗口

### *无重复字符的最长子串

https://leetcode.cn/problems/longest-substring-without-repeating-characters

```c++
class Solution {
public:
	int lengthOfLongestSubstring(string str) {
		int n = str.size();
		int ht[256];
		fill_n(ht, 256, -1);
		int res = 0;
		for (int l = 0, r = 0; r < n; ++r) { // 以 r 位置结尾，最多往前扩多远
			l = max(l, ht[str[r]] + 1);
			res = max(res, r - l + 1);
			ht[str[r]] = r;
		}
		return res;
	}
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 找到字符串中所有字母异位词

https://leetcode.cn/problems/find-all-anagrams-in-a-string

```c++
class Solution {
public:
	vector<int> findAnagrams(string str1, string str2) {
		int n1 = str1.size();
		int n2 = str2.size();
		vector<int> res;
		if (n1 < n2) {
			return res;
		}
		vector<int> cnts1(26);
		vector<int> cnts2(26);
		for (int i = 0; i < n2 - 1; ++i) {
			++cnts1[str1[i] - 'a'];
		}
		for (char& c : str2) {
			++cnts2[c - 'a'];
		}
		for (int l = 0, r = n2 - 1; r < n1; ++r, ++l) {
			++cnts1[str1[r] - 'a'];
			if (cnts1 == cnts2) {
				res.push_back(l);
			}
			--cnts1[str1[l] - 'a'];
		}
		return res;
	}
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

另一种双指针的方法：

```c++
class Solution {
public:
	vector<int> findAnagrams(string str1, string str2) {
		int n1 = str1.size();
		int n2 = str2.size();
		vector<int> res;
		if (n1 < n2) {
			return res;
		}
		vector<int> cnts1(26);
		vector<int> cnts2(26);
		for (char& c : str2) {
			++cnts2[c - 'a'];
		}
		for (int l = 0, r = 0, currR = -1; r < n1; ++r) {
			currR = str1[r] - 'a';
            // 此时要确保 [l,r] 上的每个字符数量都不超过 str2 中的这种字符数量
			++cnts1[currR];
			while (cnts1[currR] > cnts2[currR]) {
				--cnts1[str1[l++] - 'a'];
			}
			if (r - l + 1 == n2) {
                // 因为 [l,r] 上的每个字符数量都不超过 str2 中的这种字符数量
                // 又因为 [l,r] 的长度等于 str2 的长度
                // 所以 str1[l...r] 是 str2 的异位词子串
				res.push_back(l);
			}
		}
		return res;
	}
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

## 子串

### *和为 K 的子数组

https://leetcode.cn/problems/subarray-sum-equals-k

```c++
class Solution {
public:
	int subarraySum(vector<int>& nums, int k) {
		int sum = 0;
		unordered_map<int, int> um; // kv 分别是 [0,i) 上前缀和及其出现的次数
		um[0] = 1;
		int res = 0;
		for (int& num : nums) {
			sum += num;
			if (um.find(sum - k) != um.end()) {
				res += um[sum - k];
			}
			++um[sum];
		}
		return res;
	}
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

### *滑动窗口最大值

https://leetcode.cn/problems/sliding-window-maximum

```c++
class Solution {
public:
	vector<int> maxSlidingWindow(vector<int>& nums, int k) {
		deque<int> dq;
		for (int i = 0; i < k - 1; ++i) {
			while (!dq.empty() && nums[i] >= nums[dq.back()]) {
				dq.pop_back();
			}
			dq.push_back(i);
		}
		int n = nums.size();
		vector<int> res(n - k + 1);
		for (int l = 0, r = k - 1; r < n; ++l, ++r) {
			while (!dq.empty() && nums[r] >= nums[dq.back()]) {
				dq.pop_back();
			}
			dq.push_back(r);
			res[l] = nums[dq.front()];
			if (l == dq.front()) {
				dq.pop_front();
			}
		}
		return res;
	}
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

自定义数组比 STL 内置的 `deque` 速度更快。

```c++
class Solution {
private:
    int dq[100000];
    int h;
    int t;

public:
    vector<int> maxSlidingWindow(vector<int>& nums, int k) {
        h = t = 0;
        for (int j = 0; j < k - 1; ++j) {
            while (h < t && nums[j] >= nums[dq[t - 1]]) {
                --t;
            }
            dq[t++] = j;
        }
        int n = nums.size();
        vector<int> res;
        for (int i = 0, j = k - 1; j < n; ++i, ++j) {
            while (h < t && nums[j] >= nums[dq[t - 1]]) {
                --t;
            }
            dq[t++] = j;
            res.push_back(nums[dq[h]]);
            if (dq[h] == i) {
                ++h;
            }
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

### *最小覆盖子串

https://leetcode.cn/problems/minimum-window-substring

双指针。

```c++
class Solution {
public:
	string minWindow(string str1, string str2) {
		int debts[256] = {}; // 欠债表
		for (char& c : str2) {
			++debts[c];
		}
		int n = str1.size();
		int totalDebt = str2.size(); // 总欠债数
		int start = -1;
		int minLen = INT_MAX;
		for (int l = 0, r = 0; r < n; ++r) { // 以 r 位置结尾最短往左边扩多少
			if (debts[str1[r]]-- > 0) { // 一次有效的还款
				--totalDebt;
			}
			// 如果 totalDebt>0，则这时以 r 位置结尾没有答案
			if (totalDebt == 0) { // 还请债务了，不欠债了
				while (debts[str1[l]] < 0) { // 不欠债的前提下，往右移动左边界，找以 r 位置结尾时最短的子串
					++debts[str1[l++]];
				}
				if (r - l + 1 < minLen) {
					start = l;
					minLen = r - l + 1;
				}
			}
		}
		return start != -1 ? str1.substr(start, minLen) : "";
	}
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

## 普通数组

### 最大子数组和

https://leetcode.cn/problems/maximum-subarray

方法一，动态规划。

```c++
class Solution {
public:
    int maxSubArray(vector<int>& nums) {
        int n = nums.size();
        vector<int> dp(n);  // 子数组必须以 nums[i] 结尾，最大累加和是 dp[i]
        dp[0] = nums[0];
        int res = dp[0];
        for (int i = 1; i < n; ++i) {
            dp[i] = max(nums[i], dp[i - 1] + nums[i]);
            res = max(res, dp[i]);
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

空间压缩：

```c++
class Solution {
public:
	int maxSubArray(vector<int>& nums) {
		int n = nums.size();
		int res = nums[0];
		for (int curr = nums[0], i = 1; i < n; ++i) {
			curr = max(nums[i], curr + nums[i]);
			res = max(res, curr);
		}
		return res;
	}
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

拓展：找到拥有最大累加和的子数组时，返回子数组的位置和最大累加和。

```c++
class Solution {
public:
    vector<int> maxSubArray(vector<int>& nums) {
        int n = nums.size();
        int left = -1;
        int right = -1;
        int res = INT_MIN;
        for (int l = -1, r = 0, prev = INT_MIN; r < n; ++r) {
            // prev 是动态规划的前一个位置
            if (prev > 0) {  // 往左扩有利可图
                prev += nums[r];
            } else {  // 往左扩无利可图
                l = r;
                prev = nums[r];
            }
            if (prev > res) {
                left = l;
                right = r;
                res = prev;
            }
        }
        return vector<int>{left, right, res};
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 合并区间

https://leetcode.cn/problems/merge-intervals

```c++
class Solution {
public:
	vector<vector<int>> merge(vector<vector<int>>& intervals) {
		sort(intervals.begin(), intervals.end(), [](const vector<int>& p1, const vector<int>& p2) { return p1[0] < p2[0]; });
		// 也可以直接写成 sort(intervals.begin(), intervals.end());
		vector<vector<int>> res;
		for (vector<int>& interval : intervals) {
			if (res.empty() || interval[0] > res.back()[1]) {
				res.push_back(interval);
			} else {
				res.back()[1] = max(res.back()[1], interval[1]);
			}
		}
		return res;
	}
};
```

时间复杂度：$O\left(n\log n\right)$

空间复杂度：$O\left(\log n\right)$，如果用随机快速排序的话

### 轮转数组

https://leetcode.cn/problems/rotate-array

```c++
class Solution {
public:
    void rotate(vector<int>& nums, int k) {
        int n = nums.size();
        k %= n;
        reverse(nums.begin(), nums.end());
        reverse(nums.begin(), nums.begin() + k);
        reverse(nums.begin() + k, nums.end());
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### [curr]除自身以外数组的乘积

https://leetcode.cn/problems/product-of-array-except-self

方法一：

事先准备好前缀和后缀乘积的数组。

```c++
class Solution {
public:
    vector<int> productExceptSelf(vector<int>& nums) {
        int n = nums.size();
        vector<int> prefixProducts(n, 1);
        vector<int> suffixProducts(n, 1);
        for (int i = 1; i < n; ++i) {
            prefixProducts[i] = nums[i - 1] * prefixProducts[i - 1];
        }
        for (int i = n - 2; i >= 0; --i) {
            suffixProducts[i] = nums[i + 1] * suffixProducts[i + 1];
        }
        
        vector<int> res(n);
        for (int i = 0; i < n; ++i) {
            res[i] = prefixProducts[i] * suffixProducts[i];
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

方法二：

复用 `res` 数组。

```c++
class Solution {
public:
    vector<int> productExceptSelf(vector<int>& nums) {
        int n = nums.size();
        vector<int> res(n, 1);
        for (int i = 1; i < n; ++i) {
            res[i] = nums[i - 1] * res[i - 1];
        }
        int temp = 1;
        for (int i = n - 1; i >= 0; --i) {
            res[i] *= temp;
            temp *= nums[i];
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

方法三：

双指针。

```c++
class Solution {
public:
    vector<int> productExceptSelf(vector<int>& nums) {
        int n = nums.size();
        vector<int> res(n, 1);
        for (int i = 0, j = n - 1, preProd = 1, sufProd = 1; i < n; ++i, --j) {
            res[i] *= preProd;
            res[j] *= sufProd;
            preProd *= nums[i];
            sufProd *= nums[j];
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 缺失的第一个正数

https://leetcode.cn/problems/first-missing-positive

视频讲解：https://www.bilibili.com/video/BV1V841167Rg

大思路：让数组的左边部分满足 `nums[i] == i + 1`，直到发现一个边界 `l` 导致 `nums[l] != l + 1`，那么 `l + 1` 就是答案。

```c++
class Solution {
public:
    int firstMissingPositive(vector<int>& nums) {
        int l = 0; // [0,l) 范围满足 nums[i]==i+1
        int r = nums.size(); // [r,nums.size()) 范围是「垃圾区」，也表示目前最多可以做到 1~r 这 r 个正整数是存在的
        while (l < r) {
            if (nums[l] == l + 1) {
                ++l;
            } else if (nums[l] <= l || nums[l] > r // nums[l] 的范围在 (l,r] 之外
                       || nums[nums[l] - 1] == nums[l] // 后面有一个位置 j 已经满足 nums[j]==j+1 了
            ) {
                swap(nums[l], nums[--r]); // 把 nums[l] 放到「垃圾区」
            } else {
                swap(nums[l], nums[nums[l] - 1]);
            }
        }
        return l + 1;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

## 矩阵

### 矩阵置零

https://leetcode.cn/problems/set-matrix-zeroes

使用 `row0` 和 `col0` 这两个变量记录第 0 行和第 0 列上是否有 0，然后使用第 0 行和第 0 列的空间（**利用已有数据结构的空间**）记录第 `i` 行和第 `j` 列是否需要置零。

```c++
class Solution {
public:
    void setZeroes(vector<vector<int>>& matrix) {
        int n = matrix.size();
        int m = matrix[0].size();
        bool row0 = false;
        bool col0 = false;
        for (int i = 0; i < n; ++i) {
            if (matrix[i][0] == 0) {
                col0 = true;
                break;
            }
        }
        for (int j = 0; j < m; ++j) {
            if (matrix[0][j] == 0) {
                row0 = true;
                break;
            }
        }
        for (int i = 1; i < n; ++i) {
            for (int j = 1; j < m; ++j) {
                if (matrix[i][j] == 0) {
                    matrix[0][j] = 0;
                    matrix[i][0] = 0;
                }
            }
        }
        for (int i = 1; i < n; ++i) {
            for (int j = 1; j < m; ++j) {
                if (matrix[0][j] == 0 || matrix[i][0] == 0) {
                    matrix[i][j] = 0;
                }
            }
        }
        if (row0) {
            for (int j = 0; j < m; ++j) {
                matrix[0][j] = 0;
            }
        }
        if (col0) {
            for (int i = 0; i < n; ++i) {
                matrix[i][0] = 0;
            }
        }
    }
};
```

时间复杂度：$O\left(nm\right)$

空间复杂度：$O\left(1\right)$

------

模拟 3：

可以再节省 `row0` 这个变量，用 `matrix[0][0]` 来表示第 0 行元素是否需要置零。

```c++
class Solution {
public:
    void setZeroes(vector<vector<int>>& matrix) {
        int n = matrix.size();
        int m = matrix[0].size();
        bool col0 = false;
        for (int i = 0; i < n; ++i) {
            if (matrix[i][0] == 0) {
                col0 = true;
                break;
            }
        }
        for (int j = 0; j < m; ++j) {
            if (matrix[0][j] == 0) {
                matrix[0][0] = 0;
                break;
            }
        }
        for (int i = 1; i < n; ++i) {
            for (int j = 1; j < m; ++j) {
                if (matrix[i][j] == 0) {
                    matrix[0][j] = 0;
                    matrix[i][0] = 0;
                }
            }
        }
        for (int i = 1; i < n; ++i) {
            for (int j = 1; j < m; ++j) {
                if (matrix[0][j] == 0 || matrix[i][0] == 0) {
                    matrix[i][j] = 0;
                }
            }
        }
        if (matrix[0][0] == 0) {
            for (int j = 0; j < m; ++j) {
                matrix[0][j] = 0;
            }
        }
        if (col0) {
            for (int i = 0; i < n; ++i) {
                matrix[i][0] = 0;
            }
        }
    }
};
```

时间复杂度：$O\left(nm\right)$

空间复杂度：$O\left(1\right)$

### 螺旋矩阵

https://leetcode.cn/problems/spiral-matrix

不要关注局部位置怎么变，应该设计宏观调度过程。设计一个 `getFrame` 函数，接收左上点的坐标，打印出一个边框，主函数不断调用 `getFrame` 函数即可。

模拟：

```c++
class Solution {
public:
    vector<int> spiralOrder(vector<vector<int>>& matrix) {
        int n = matrix.size();
        int m = matrix[0].size();
        vector<int> res(n * m);
        for (int start = 0, idx = 0; start <= (min(n, m) - 1) >> 1; ++start) {
            idx = getFrame(matrix, start, res, idx);
        }
        return res;
    }

    int getFrame(vector<vector<int>>& matrix, int start, vector<int>& res, int idx) {
        int n = matrix.size();
        int m = matrix[0].size();
        int rowEnd = n - 1 - start;
        int colEnd = m - 1 - start;
        // idx 其实可以推导出来结果是 2*start*(M+N-2*start)，但是面试时没那么多时间推导，所以直接返回 res 中下一个下标的位置更方便
        if (start == rowEnd) {
            for (int j = start; j <= colEnd; ++j) {
                res[idx++] = matrix[start][j];
            }
        } else if (start == colEnd) {
            for (int i = start; i <= rowEnd; ++i) {
                res[idx++] = matrix[i][colEnd];
            }
        } else {
            for (int j = start; j < colEnd; ++j) {
                res[idx++] = matrix[start][j];
            }
            for (int i = start; i < rowEnd; ++i) {
                res[idx++] = matrix[i][colEnd];
            }
            for (int j = colEnd; j > start; --j) {
                res[idx++] = matrix[rowEnd][j];
            }
            for (int i = rowEnd; i > start; --i) {
                res[idx++] = matrix[i][start];
            }
        }
        return idx;
    }
};
```

时间复杂度：$O\left(nm\right)$

空间复杂度：$O\left(1\right)$

### 旋转图像

https://leetcode.cn/problems/rotate-image

方法一：

```c++
class Solution {
public:
    void rotate(vector<vector<int>>& matrix) {
        for (int start = 0; start <= (matrix.size() - 1) >> 1; ++start) {
            rotateFrame(matrix, start);
        }
    }

    void rotateFrame(vector<vector<int>>& matrix, int start) {
        int n = matrix.size();
        int end = n - 1 - start;
        for (int i = 0, temp; i < end - start; ++i) {
            temp = matrix[start][start + i];
            matrix[start][start + i] = matrix[end - i][start];
            matrix[end - i][start] = matrix[end][end - i];
            matrix[end][end - i] = matrix[start + i][end];
            matrix[start + i][end] = temp;
        }
    }
};
```

时间复杂度：$O\left(n^2\right)$

空间复杂度：$O\left(1\right)$

方法二：

先沿水平方向翻转再沿主对角线翻转。即 `M[i][j] -> M[N - 1 - i][j] -> M[j][N - 1 - i]`，`M[i][j] -> M[j][N - 1 - i]` 就是顺时针旋转 90° 的结果。

```c++
class Solution {
public:
    void rotate(vector<vector<int>>& matrix) {
        int N = matrix.size();
        for (int i = 0; i < (N >> 1); ++i) {
            for (int j = 0; j < N; ++j) {
                swap(matrix[i][j], matrix[N - 1 - i][j]);
            }
        }
        for (int i = 0; i < N; ++i) {
            for (int j = 0; j < i; ++j) {
                swap(matrix[i][j], matrix[j][i]);
            }
        }
    }
};
```

时间复杂度：$O\left(n^2\right)$

空间复杂度：$O\left(1\right)$

### 搜索二维矩阵 II

https://leetcode.cn/problems/search-a-2d-matrix-ii

对每一行用二分搜索：

```c++
class Solution {
public:
    bool searchMatrix(vector<vector<int>>& matrix, int target) {
        for (vector<int>& row: matrix) {
            if (binary_search(row.begin(), row.end(), target)) {
                return true;
            }
        }
        return false;
    }
};
```

时间复杂度：$O\left(n\log m\right)$

空间复杂度：$O\left(1\right)$

```c++
class Solution {
public:
    bool searchMatrix(vector<vector<int>>& matrix, int target) {
        int n = matrix.size();
        int m = matrix[0].size();
        for (int i = 0, j = m - 1; i < n && j >= 0;) {
            if (target < matrix[i][j]) {
                --j; // 舍弃第 j 列
            } else if (target > matrix[i][j]) {
                ++i; // 舍弃第 i 行
            } else {
                return true;
            }
        }
        return false;
    }
};
```

时间复杂度：$O\left(n+m\right)$

空间复杂度：$O\left(1\right)$

## 链表

### 相交链表

https://leetcode.cn/problems/intersection-of-two-linked-lists

前面已完成

### 反转链表

https://leetcode.cn/problems/reverse-linked-list

前面已完成

### 回文链表

https://leetcode.cn/problems/palindrome-linked-list

前面已完成

### 环形链表

https://leetcode.cn/problems/linked-list-cycle

同「环形链表 II」，前面已完成

### 环形链表 II

https://leetcode.cn/problems/linked-list-cycle-ii

前面已完成

### 合并两个有序链表

https://leetcode.cn/problems/merge-two-sorted-lists

前面已完成

### 两数相加

https://leetcode.cn/problems/add-two-numbers

前面已完成

### 删除链表的倒数第 N 个结点

https://leetcode.cn/problems/remove-nth-node-from-end-of-list

```c++
class Solution {
public:
    ListNode* removeNthFromEnd(ListNode* h, int n) {
        ListNode* dh = new ListNode(0, h);
        ListNode* s = dh;
        ListNode* f = dh;
        while (n--) {
            f = f->next;
        }
        while (f->next != nullptr) {
            s = s->next;
            f = f->next;
        }
        s->next = s->next->next;
        return dh->next;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 两两交换链表中的节点

https://leetcode.cn/problems/swap-nodes-in-pairs

```c++
class Solution {
public:
    ListNode* swapPairs(ListNode* h) {
        if (h == nullptr || h->next == nullptr) {
            return h;
        }
        ListNode* dh = new ListNode(0, h);
        ListNode* p1 = dh;
        ListNode* p2 = nullptr;
        ListNode* p3 = nullptr;
        while (p1->next != nullptr && p1->next->next != nullptr) {
            p2 = p1->next;
            p3 = p2->next;
            p1->next = p3;
            p2->next = p3->next;
            p3->next = p2;
            p1 = p2;
        }
        return dh->next;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### K 个一组翻转链表

https://leetcode.cn/problems/reverse-nodes-in-k-group

前面已完成

### 随机链表的复制

https://leetcode.cn/problems/copy-list-with-random-pointer

前面已完成

### 排序链表

https://leetcode.cn/problems/sort-list

前面已完成

### 合并 K 个升序链表

https://leetcode.cn/problems/merge-k-sorted-lists

前面已完成

### LRU 缓存

https://leetcode.cn/problems/lru-cache

前面已完成

## 二叉树

### 二叉树的中序遍历

https://leetcode.cn/problems/binary-tree-inorder-traversal

前面已完成

### 二叉树的最大深度

https://leetcode.cn/problems/maximum-depth-of-binary-tree

前面已完成

### 翻转二叉树

https://leetcode.cn/problems/invert-binary-tree

```c++
class Solution {
public:
    TreeNode* invertTree(TreeNode* r) {
        if (r == nullptr) {
            return nullptr;
        }
        invertTree(r->left);
        invertTree(r->right);
        swap(r->left, r->right);
        return r;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：最坏情况下，$O\left(n\right)$

### 对称二叉树

https://leetcode.cn/problems/symmetric-tree

```c++
class Solution {
public:
    bool isSymmetric(TreeNode* r) {
        if (r == nullptr) {
            return true;
        }
        return f(r->left, r->right);
    }

    bool f(TreeNode* r1, TreeNode* r2) {
        if (r1 == nullptr && r2 == nullptr) {
            return true;
        }
        if (r1 == nullptr || r2 == nullptr) {
            return false;
        }
        return r1->val == r2->val && f(r1->left, r2->right) && f(r1->right, r2->left);
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：最坏情况下，$O\left(n\right)$

### 二叉树的直径

https://leetcode.cn/problems/diameter-of-binary-tree

```c++
class Info {
public:
    int diameter;
    int depth;

    Info(int diameter, int depth): diameter(diameter), depth(depth) {
    }
};

class Solution {
public:
    int diameterOfBinaryTree(TreeNode* r) {
        return f(r).diameter;
    }

    Info f(TreeNode* r) {
        if (r == nullptr) {
            return Info(-1, 0);
        }
        Info left = f(r->left);
        Info right = f(r->right);
        return Info(max(left.depth + right.depth, max(left.diameter, right.diameter)), max(left.depth, right.depth) + 1);
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：最坏情况下，$O\left(n\right)$

```c++
class Solution {
public:
    int res;

    int diameterOfBinaryTree(TreeNode* r) {
        res = 0;
        getDepth(r);
        return res;
    }

    int getDepth(TreeNode* r) {
        if (r == nullptr) {
            return 0;
        }
        int left = getDepth(r->left);
        int right = getDepth(r->right);
        res = max(res, left + right);
        return max(left, right) + 1;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：最坏情况下，$O\left(n\right)$

### 二叉树的层序遍历

https://leetcode.cn/problems/binary-tree-level-order-traversal

前面已完成

### 将有序数组转换为二叉搜索树

https://leetcode.cn/problems/convert-sorted-array-to-binary-search-tree

前面已完成

### 验证二叉搜索树

https://leetcode.cn/problems/validate-binary-search-tree

前面已完成

### 二叉搜索树中第 K 小的元素

https://leetcode.cn/problems/kth-smallest-element-in-a-bst

```c++
class Solution {
private:
    TreeNode* stk[MAXN];
    int t;

public:
    int kthSmallest(TreeNode* root, int k) {
        t = 0;
        while (t > 0 || root != nullptr) {
            if (root != nullptr) {
                stk[t++] = root;
                root = root->left;
            } else {
                root = stk[--t];
                if (--k == 0) {
                    return root->val;
                }
                root = root->right;
            }
        }
        return -1;
    }
};
```

时间复杂度：$O\left(h+k\right)$，$h$ 为二叉树的高度

空间复杂度：最坏情况下，$O\left(h\right)$

如果你需要频繁地查找第 `k` 小的值，你将如何优化算法？

```c++
class Solution {
private:
    unordered_map<TreeNode*, int> ht;

public:
    int kthSmallest(TreeNode* root, int k) {
        ht.clear();
        ht[nullptr] = 0;
        f(root);
        int leftNumNodes = -1;
        while (root != nullptr) {
            leftNumNodes = ht[root->left];
            if (k <= leftNumNodes) { // 目标节点在左子树
                root = root->left;
            } else if (k > leftNumNodes + 1) { // 目标节点在右子树
                k -= leftNumNodes + 1;
                root = root->right;
            } else { // 找到目标节点
                return root->val;
            }
        }
        return -1;
    }

    int f(TreeNode* root) {
        if (root == nullptr) {
            return 0;
        }
        ht[root] = f(root->left) + f(root->right) + 1;
        return ht[root];
    }
};
```

时间复杂度：预处理的时间复杂度为 $O\left(n\right)$，搜索的时间复杂度为 $O\left(h\right)$

空间复杂度：$O\left(n\right)$

如果二叉搜索树经常被修改（插入/删除操作）并且你需要频繁地查找第 `k` 小的值，你将如何优化算法？

用 AVL 树

### 二叉树的右视图

https://leetcode.cn/problems/binary-tree-right-side-view

先序遍历不断更新最右侧的节点：

```c++
class Solution {
public:
    vector<int> rightSideView(TreeNode* root) {
        vector<int> res;
        f(root, 0, res);
        return res;
    }

    void f(TreeNode* root, int depth, vector<int>& res) {
        if (root == nullptr) {
            return;
        }
        if (depth >= res.size()) {
            res.push_back(root->val);
        } else {
            res[depth] = root->val;
        }
        f(root->left, depth + 1, res);
        f(root->right, depth + 1, res);
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：最坏情况下，$O\left(n\right)$

层序遍历：

```c++
const int MAXN = 100;

class Solution {
private:
    TreeNode* q[MAXN];
    int h;
    int t;

public:
    vector<int> rightSideView(TreeNode* root) {
        h = t = 0;
        vector<int> res;
        if (root == nullptr) {
            return res;
        }
        q[t++] = root;
        int n = -1;
        while (h < t) {
            n = t - h;
            while (n--) {
                root = q[h++];
                if (root->left != nullptr) {
                    q[t++] = root->left;
                }
                if (root->right != nullptr) {
                    q[t++] = root->right;
                }
            }
            res.push_back(root->val);
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

### 二叉树展开为链表

https://leetcode.cn/problems/flatten-binary-tree-to-linked-list

对于每棵子树，节点出现的顺序都是根节点、左子树的节点、右子树的节点

```c++
class Solution {
public:
    void flatten(TreeNode* root) {
        TreeNode* rightMostNode = nullptr;
        while (root != nullptr) {
            if (root->left != nullptr) {
                rightMostNode = root->left;
                while (rightMostNode->right != nullptr) {
                    rightMostNode = rightMostNode->right;
                }
                rightMostNode->right = root->right;
                root->right = root->left;
                root->left = nullptr;
            }
            root = root->right;
        }
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 从前序与中序遍历序列构造二叉树

https://leetcode.cn/problems/construct-binary-tree-from-preorder-and-inorder-traversal

前面已完成

### 路径总和 III

https://leetcode.cn/problems/path-sum-iii

```c++
class Solution {
private:
    typedef long long ll;

    // 必须以 root 为开头，有多少条和为 targetSum 的路径
    int f(TreeNode* root, ll targetSum) {
        if (root == nullptr) {
            return 0;
        }
        int res = 0;
        if (root->val == targetSum) {
            ++res;
        }
        res += f(root->left, targetSum - root->val);
        res += f(root->right, targetSum - root->val);
        return res;
    }

public:
    int pathSum(TreeNode* root, int targetSum) {
        if (root == nullptr) {
            return 0;
        }
        return f(root, targetSum) // 要根结点
               + pathSum(root->left, targetSum) + pathSum(root->right, targetSum); // 不要根节点
    }
};
```

时间复杂度：画递归图分析得出，$O\left(n^2\right)$

空间复杂度：最坏情况下，$O\left(n\right)$

上面的方法存在很多重复计算，下面是记录前缀和的方法：

```c++
class Solution {
private:
    typedef long long ll;
    unordered_map<ll, int> ht; // key：当前这条路径上各节点的前缀和；value：有多少个这样的前缀和

    int f(TreeNode* root, ll prevSum, int targetSum) {
        if (root == nullptr) {
            return 0;
        }
        prevSum += root->val;
        ll remain = prevSum - targetSum;
        int res = 0;
        if (ht.count(remain) > 0) {
            res = ht[remain];
        }
        ++ht[prevSum];
        res += f(root->left, prevSum, targetSum);
        res += f(root->right, prevSum, targetSum);
        if (--ht[prevSum] == 0) {
            ht.erase(prevSum);
        }
        return res;
    }

public:
    int pathSum(TreeNode* root, int targetSum) {
        ht.clear();
        ht[0] = 1;
        return f(root, 0, targetSum);
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

### 二叉树的最近公共祖先

https://leetcode.cn/problems/lowest-common-ancestor-of-a-binary-tree

前面已完成

### 二叉树中的最大路径和

https://leetcode.cn/problems/binary-tree-maximum-path-sum

两个递归函数：

```c++
class Solution {
private:
    int res;

    // 前序遍历
    void f(TreeNode* root) {
        if (root == nullptr) {
            return;
        }
        res = max(res, root->val + max(0, g(root->left)) + max(0, g(root->right)));
        f(root->left);
        f(root->right);
    }

    // 从 root 出发往下扎，必须包含 root，最大和为多少
    int g(TreeNode* root) {
        if (root == nullptr) {
            return 0;
        }
        return root->val + max(0, max(g(root->left), g(root->right)));
    }

public:
    int maxPathSum(TreeNode* root) {
        res = INT_MIN;
        f(root);
        return res;
    }
};
```

时间复杂度：$O\left(n^2\right)$

空间复杂度：最坏情况下，$O\left(n\right)$

树形 DP：

```c++
class Info {
public:
    int maxDwnSum; // 必须包含当前节点，往下扎，最大和是多少
    int maxPthSum;

    Info(int maxDwnSum, int maxPthSum): maxDwnSum(maxDwnSum), maxPthSum(maxPthSum) {
    }
};

class Solution {
private:
    Info f(TreeNode* root) {
        if (root == nullptr) {
            return Info(0, INT_MIN);
        }
        Info l = f(root->left);
        Info r = f(root->right);
        return Info(root->val + max(0, max(l.maxDwnSum, r.maxDwnSum)),
                    max(root->val + max(0, l.maxDwnSum) + max(0, r.maxDwnSum), max(l.maxPthSum, r.maxPthSum)));
    }

public:
    int maxPathSum(TreeNode* root) {
        return f(root).maxPthSum;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：最坏情况下，$O\left(n\right)$

树形 DP 的全局静态变量写法：

```c++
class Solution {
private:
    int maxDwnSum;
    int maxPthSum;

public:
    int maxPathSum(TreeNode* root) {
        if (root == nullptr) {
            maxDwnSum = 0;
            maxPthSum = INT_MIN;
            return maxPthSum;
        }
        maxPathSum(root->left);
        int leftMaxDwnSum = maxDwnSum;
        int leftMaxPthSum = maxPthSum;
        maxPathSum(root->right);
        int rightMaxDwnSum = maxDwnSum;
        int rightMaxPthSum = maxPthSum;
        maxDwnSum = root->val + max(0, max(leftMaxDwnSum, rightMaxDwnSum));
        maxPthSum = max(root->val + max(0, leftMaxDwnSum) + max(0, rightMaxDwnSum), max(leftMaxPthSum, rightMaxPthSum));
        return maxPthSum;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：最坏情况下，$O\left(n\right)$

遍历的过程中顺便记录答案（其实就是上面的树形 DP 简化掉了 `maxPthSum` 这个信息，不断更新全局 `res` 的值即可）：

```c++
class Solution {
private:
    int res;

    // 必须包含当前节点，往下扎，最大和是多少
    int f(TreeNode* root) {
        if (root == nullptr) {
            return 0;
        }
        int l = max(0, f(root->left));
        int r = max(0, f(root->right));
        res = max(res, root->val + l + r);
        return root->val + max(l, r);
    }

public:
    int maxPathSum(TreeNode* root) {
        res = INT_MIN;
        f(root);
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：最坏情况下，$O\left(n\right)$

## 图论

### 岛屿数量

https://leetcode.cn/problems/number-of-islands

```c++
class Solution {
private:
    void f(vector<vector<char>>& grid, int i, int j) {
        if (i < 0 || i >= grid.size() || j < 0 || j >= grid[0].size() || grid[i][j] == '0') {
            return;
        }
        grid[i][j] = '0';
        f(grid, i - 1, j);
        f(grid, i + 1, j);
        f(grid, i, j - 1);
        f(grid, i, j + 1);
    }

public:
    int numIslands(vector<vector<char>>& grid) {
        int res = 0;
        for (int i = 0; i < grid.size(); ++i) {
            for (int j = 0; j < grid[0].size(); ++j) {
                if (grid[i][j] == '1') {
                    ++res;
                    f(grid, i, j); // 把当前这个岛屿上的所有 '1' 变成 '0'
                }
            }
        }
        return res;
    }
};
```

时间复杂度：$O\left(nm\right)$

空间复杂度：$O\left(nm\right)$

### 腐烂的橘子

https://leetcode.cn/problems/rotting-oranges

```c++
const int MAXN = 100;

class Solution {
private:
    int q[MAXN];
    int h;
    int t;

public:
    int orangesRotting(vector<vector<int>>& grid) {
        h = t = 0;
        int n = grid.size();
        int m = grid[0].size();
        int numFresh = 0;
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j < m; ++j) {
                if (grid[i][j] == 1) {
                    ++numFresh;
                } else if (grid[i][j] == 2) {
                    q[t++] = i * m + j;
                }
            }
        }
        int k = -1;
        int i = -1;
        int j = -1;
        int x = -1;
        int y = -1;
        const int xs[4] = {-1, 1, 0, 0};
        const int ys[4] = {0, 0, -1, 1};
        int res = 0;
        while (h < t) {
            k = t - h;
            while (k--) {
                i = q[h] / m;
                j = q[h++] % m;
                for (int d = 0; d < 4; ++d) {
                    x = i + xs[d];
                    y = j + ys[d];
                    if (x >= 0 && x < n && y >= 0 && y < m && grid[x][y] == 1) {
                        grid[x][y] = 2;
                        q[t++] = x * m + y;
                        --numFresh;
                    }
                }
            }
            if (h < t) {
                ++res;
            }
        }
        return numFresh == 0 ? res : -1;
    }
};
```

时间复杂度：$O\left(nm\right)$

空间复杂度：$O\left(nm\right)$

### 课程表

https://leetcode.cn/problems/course-schedule

```c++
const int MAXN = 2e3;

class Solution {
private:
    list<int> g[MAXN];
    int ins[MAXN];
    int q[MAXN];
    int h;
    int t;

public:
    Solution(): ins{}, q{}, h(0), t(0) {
    }

    bool canFinish(int numCourses, vector<vector<int>>& prerequisites) {
        for (vector<int>& p: prerequisites) {
            g[p[1]].push_back(p[0]);
            ++ins[p[0]];
        }
        for (int id = 0; id < numCourses; ++id) {
            if (ins[id] == 0) {
                q[t++] = id;
            }
        }
        int id = -1;
        while (h < t) {
            id = q[h++];
            for (int& toId: g[id]) {
                if (--ins[toId] == 0) {
                    q[t++] = toId;
                }
            }
        }
        bool res = t == numCourses;
        clear(numCourses);
        return res;
    }

    void clear(int numCourses) {
        for (int i = 0; i < numCourses; ++i) {
            g[i].clear();
            ins[i] = 0;
        }
        h = t = 0;
    }
};
```

假设节点和边的数量分别为 $v$ 和 $e$：

时间复杂度：$O\left(v+e\right)$

空间复杂度：$O\left(v+e\right)$

### 实现 Trie (前缀树)

https://leetcode.cn/problems/implement-trie-prefix-tree

```c++
const int MAXN = 5e4; // 试出来的

class Trie {
private:
    int tree[MAXN][26];
    int pass[MAXN];
    int end_[MAXN];
    int cnt;

public:
    Trie(): tree{}, pass{}, end_{}, cnt(1) {
    }

    void insert(string str) {
        int curr = 1;
        ++pass[curr];
        int path = -1;
        for (char& c: str) {
            path = c - 'a';
            if (tree[curr][path] == 0) {
                tree[curr][path] = ++cnt;
            }
            curr = tree[curr][path];
            ++pass[curr];
        }
        ++end_[curr];
    }

    bool search(string str) {
        int curr = 1;
        int path = -1;
        for (char& c: str) {
            path = c - 'a';
            if (tree[curr][path] == 0) {
                return false;
            }
            curr = tree[curr][path];
        }
        return end_[curr] > 0;
    }

    bool startsWith(string prefix) {
        int curr = 1;
        int path = -1;
        for (char& c: prefix) {
            path = c - 'a';
            if (tree[curr][path] == 0) {
                return false;
            }
            curr = tree[curr][path];
        }
        return true;
    }
};
```

$n$ 为操作次数、$m$ 为所有单词的最大长度：

时间复杂度：$O\left(nm\right)$

空间复杂度：$O\left(nm\right)$

## 回溯

### 全排列

https://leetcode.cn/problems/permutations

前面已完成

### 子集

https://leetcode.cn/problems/subsets

```c++
class Solution {
private:
    // [i...] 范围上的所有组合前面加上 path 后添加到 res 中，path[0...pathSize-1] 部分不允许改变
    void f(vector<int>& nums, int i, int path[], int pathSize, vector<vector<int>>& res) {
        if (i == nums.size()) {
            res.push_back(vector<int>(path, path + pathSize));
            return;
        }
        f(nums, i + 1, path, pathSize, res);
        path[pathSize++] = nums[i];
        f(nums, i + 1, path, pathSize, res);
    }

public:
    vector<vector<int>> subsets(vector<int>& nums) {
        int path[nums.size()];
        vector<vector<int>> res;
        f(nums, 0, path, 0, res);
        return res;
    }
};
```

时间复杂度：组合个数 $O\left(2^n\right)$，拼出每一个组合的时间复杂度为 $O\left(n\right)$，总体时间复杂度为 $O\left(n2^n\right)$

空间复杂度：$O\left(n\right)$

### 电话号码的字母组合

https://leetcode.cn/problems/letter-combinations-of-a-phone-number

```c++
class Solution {
private:
    string ht[8] = {"abc", "def", "ghi", "jkl", "mno", "pqrs", "tuv", "wxyz"};

    void f(string& str, int i, char path[], int pathSize, vector<string>& res) {
        if (i == str.size()) {
            res.push_back(string(path, pathSize));
            return;
        }
        for (char& c: ht[str[i] - '2']) {
            path[pathSize] = c;
            f(str, i + 1, path, pathSize + 1, res);
        }
    }

public:
    vector<string> letterCombinations(string str) {
        vector<string> res;
        if (str.empty()) {
            return res;
        }
        char path[str.size()];
        f(str, 0, path, 0, res);
        return res;
    }
};
```

时间复杂度：$O\left(n3^a4^b\right)$，其中 $n$ 是 `str` 的长度，$a$ 是输入中对应 3 个字母的数字个数，$b$ 是输入中对应 4 个字母的数字个数，$a+b$ 是输入数字的总个数

空间复杂度：$O\left(n\right)$

### 组合总和

https://leetcode.cn/problems/combination-sum

```c++
const int MAX_PATH_SIZE = 20;

class Solution {
private:
    // nums[i...] 范围上找出所有不重复的组合拼上 path 之后放到 res 中
    void f(vector<int>& nums, int i, int target, int path[], int pathSize, vector<vector<int>>& res) {
        if (target == 0) {
            res.push_back(vector<int>(path, path + pathSize));
            return;
        }
        if (i == nums.size()) {
            return;
        }
        f(nums, i + 1, target, path, pathSize, res);
        if (target >= nums[i]) {
            path[pathSize++] = nums[i];
            f(nums, i, target - nums[i], path, pathSize, res);
        }
    }

public:
    vector<vector<int>> combinationSum(vector<int>& nums, int target) {
        int path[MAX_PATH_SIZE] = {};
        vector<vector<int>> res;
        f(nums, 0, target, path, 0, res);
        return res;
    }
};
```

假设 $t$ 为 `target`，$m$ 为 `nums` 中的最小值

时间复杂度：收集 `path` 的时间复杂度为 $O\left(\frac{t}{m}\right)$，遍历树的时间复杂度为 $O\left(2^n\right)$，总体时间复杂度为 $O\left(\frac{t}{m}2^n\right)$

空间复杂度：$O\left(\frac{t}{m}\right)$

### 括号生成

https://leetcode.cn/problems/generate-parentheses

```c++
class Solution {
private:
    // numLeft：还需要放多少个左括号
    // numRight：还需要放多少个右括号
    void f(int numLeft, int numRight, char path[], int pathSize, vector<string>& res) {
        if (numLeft == 0 && numRight == 0) {
            res.push_back(string(path, path + pathSize));
            return;
        }
        if (numLeft > 0) {
            path[pathSize] = '(';
            f(numLeft - 1, numRight, path, pathSize + 1, res);
        }
        if (numLeft < numRight) {
            path[pathSize] = ')';
            f(numLeft, numRight - 1, path, pathSize + 1, res);
        }
    }

public:
    vector<string> generateParenthesis(int n) {
        char path[n << 1];
        vector<string> res;
        f(n, n, path, 0, res);
        return res;
    }
};
```

时间复杂度：$O\left(\frac{4^n}{\sqrt{n}}\right)$，即回溯函数的执行次数，证明略

空间复杂度：$O\left(n\right)$

> 下面是暴力递归和动态规划的内容，可不看
>
> 暴力递归：
>
> 一个合法的括号组合一定符合 $\left(S_1\right)S_2$ 这样的形式，其中 $S_1$ 和 $S_2$ 可以为空。当 $S_1$ 的长度为 $i$ 时（其中 $0\le i<n$），$S_2$ 的长度为 $n-i-1$，在这 $n-1$ 种可能性下穷举所有情况即可。
>
> 对于不同的 $i$，不会有重复的答案。证明如下：假设 $\left(S_1\right)S_2$ 中，$S_1$ 有 $i$ 对括号，$\left(S_{1}^{'}\right)S_{2}^{'}$ 中 $S_{1}^{'}$ 有 $i^{'}$ 对括号（其中 $i^{'}>i$），$\left(S_1\right)S_2$ 中 $S_1$ 后面的右括号的下标为 $index$，$\left(S_{1}^{'}\right)S_{2}^{'}$ 中 $S_1^{'}$ 后面的右括号的下标为 $index^{'}$，如果要使 $\left(S_1\right)S_2$ 和 $\left(S_{1}^{'}\right)S_{2}^{'}$ 在 $\left[0,index\right]$ 范围上相同，那么 $S_1^{'}$ 需要贡献 $i$ 个左括号和 $i+1$ 个右括号，这时 $\left(S_{1}^{'}\right)S_{2}^{'}$ 在 $\left(index,index^{'}\right)$ 范围上只能贡献 $i^{'}-i$ 个左括号和 $i^{'}-i-1$ 个右括号。因为 $S_{1}^{'}$ 本身就是合法的括号组合且 $\left(S_{1}^{'}\right)S_{2}^{'}$ 在 $\left[0,index\right]$ 范围上也是合法的括号组合 ，这就要求 $\left(S_{1}^{'}\right)S_{2}^{'}$ 在 $\left(index,index^{'}\right)$ 也必须是合法的括号组合，但是 $\left(S_{1}^{'}\right)S_{2}^{'}$ 在 $\left(index,index^{'}\right)$ 范围上只能贡献 $i^{'}-i$ 个左括号和 $i^{'}-i-1$ 个右括号，这并不能构成一个合法的括号组合，反证法成立，所以对于不同的 $i$，不会有重复的答案。
>
> ```c++
> class Solution {
> public:
>     vector<string> generateParenthesis(int n) {
>         if (n == 0) {
>             return vector<string>{""};
>         }
>         vector<string> res;
>         for (int i = 0; i < n; ++i) {
>             vector<string> leftRes = generateParenthesis(i);
>             vector<string> rightRes = generateParenthesis(n - i - 1);
>             for (string& left: leftRes) {
>                 for (string& right: rightRes) {
>                     res.push_back("(" + left + ")" + right);
>                 }
>             }
>         }
>         return res;
>     }
> };
> ```
>
> 暴力递归改动态规划：
>
> ```c++
> class Solution {
> public:
>     vector<string> generateParenthesis(int n) {
>         vector<vector<string>> dp(n + 1);
>         dp[0].emplace_back("");
>         for (int n_ = 1; n_ <= n; ++n_) {
>             for (int i = 0; i < n_; ++i) {
>                 vector<string> leftRes = dp[i];
>                 vector<string> rightRes = dp[n_ - i - 1];
>                 for (string& left: leftRes) {
>                     for (string& right: rightRes) {
>                         dp[n_].emplace_back("(" + left + ")" + right);
>                     }
>                 }
>             }
>         }
>         return dp[n];
>     }
> };
> ```
>

### 单词搜索

https://leetcode.cn/problems/word-search

```c++
const int MAXNM = 6;

class Solution {
private:
    const int xs[4] = {-1, 1, 0, 0};
    const int ys[4] = {0, 0, -1, 1};
    bool visiteds[MAXNM][MAXNM];

    void clear(vector<vector<char>>& board) {
        for (int i = 0; i < board.size(); ++i) {
            fill_n(visiteds[i], board[i].size(), false);
        }
    }

    bool f(vector<vector<char>>& board, string& word, int i, int j, int k) {
        if (word[k] != board[i][j]) {
            return false;
        }
        if (k == word.size() - 1) {
            return true;
        }
        visiteds[i][j] = true;
        int x = -1;
        int y = -1;
        for (int d = 0; d < 4; ++d) {
            x = i + xs[d];
            y = j + ys[d];
            if (x >= 0 && x < board.size() && y >= 0 && y < board[0].size()
                && !visiteds[x][y]
                && f(board, word, x, y, k + 1)) {
                return true;
            }
        }
        visiteds[i][j] = false;
        return false;
    }

public:
    bool exist(vector<vector<char>>& board, string word) {
        clear(board);
        for (int i = 0; i < board.size(); ++i) {
            for (int j = 0; j < board[0].size(); ++j) {
                if (f(board, word, i, j, 0)) {
                    return true;
                }
            }
        }
        return false;
    }
};
```

假设字符串的长度为 $l$：

时间复杂度：$O\left(nm3^l\right)$

空间复杂度：$O\left(nm+l\right)$

### 分割回文串

https://leetcode.cn/problems/palindrome-partitioning

```c++
class Solution {
private:
    void f(string& str, int i, vector<string>& path, vector<vector<string>>& res) {
        if (i == str.size()) {
            res.push_back(path);
            return;
        }
        for (int j = i; j < str.size(); ++j) {
            if (isPalindrome(str, i, j)) {
                path.push_back(str.substr(i, j - i + 1));
                f(str, j + 1, path, res);
                path.pop_back();
            }
        }
    }

    bool isPalindrome(string& str, int i, int j) {
        while (i < j) {
            if (str[i] != str[j]) {
                return false;
            }
            ++i;
            --j;
        }
        return true;
    }

public:
    vector<vector<string>> partition(string str) {
        vector<string> path;
        vector<vector<string>> res;
        f(str, 0, path, res);
        return res;
    }
};
```

最坏情况：`str` 的所有字符都相同

时间复杂度：每个位置可以选择切或不切，$O\left(n\cdot2^n\right)$

空间复杂度：$O\left(n\right)$

预先用动态规划求出任意子串是否为回文串：

```c++
class Solution {
private:
    void f(string& str, int i, vector<vector<bool>>& dp, vector<string>& path, vector<vector<string>>& res) {
        if (i == str.size()) {
            res.push_back(path);
            return;
        }
        for (int j = i; j < str.size(); ++j) {
            if (dp[i][j]) {
                path.push_back(str.substr(i, j - i + 1));
                f(str, j + 1, dp, path, res);
                path.pop_back();
            }
        }
    }

public:
    vector<vector<string>> partition(string str) {
        vector<vector<bool>> dp(str.size(), vector<bool>(str.size(), true));
        for (int i = str.size() - 2; i >= 0; --i) {
            for (int j = i + 1; j < str.size(); ++j) {
                dp[i][j] = str[i] == str[j] && dp[i + 1][j - 1];
            }
        }
        vector<string> path;
        vector<vector<string>> res;
        f(str, 0, dp, path, res);
        return res;
    }
};
```

时间复杂度：$O\left(n\cdot2^n\right)$

空间复杂度：$O\left(n^2\right)$

### N 皇后

https://leetcode.cn/problems/n-queens

```c++
class Solution {
    void f(int n, int limit, int col, int left, int right, int queues[], vector<vector<string>>& res) {
        if (col == limit) {
            vector<string> board(n, string(n, '.'));
            for (int i = 0; i < n; ++i) {
                board[i][queues[i]] = 'Q';
            }
            res.push_back(board);
            return;
        }
        int ban = col | left | right;
        int candidates = ~ban & limit;
        int place = -1;
        int i = -1;
        while (candidates != 0) {
            place = candidates & -candidates;
            i = __builtin_popcount(col);
            queues[i] = __builtin_ctz(place);
            f(n, limit, col | place, (left | place) << 1, (right | place) >> 1, queues, res);
            queues[i] = -1;
            candidates ^= place;
        }
    }

public:
    vector<vector<string>> solveNQueens(int n) {
        int queues[n];
        vector<vector<string>> res;
        f(n, (1 << n) - 1, 0, 0, 0, queues, res);
        return res;
    }
};
```

时间复杂度：$O\left(n!\right)$

空间复杂度：$O\left(n\right)$

## 二分查找

https://leetcode.cn/problems/search-insert-position

```c++
class Solution {
public:
    int searchInsert(vector<int>& nums, int target) {
        int l = 0;
        int r = nums.size() - 1;
        int m = -1;
        while (l <= r) {
            m = l + ((r - l) >> 1);
            if (target < nums[m]) {
                r = m - 1;
            } else if (target > nums[m]) {
                l = m + 1;
            } else {
                return m;
            }
        }
        return l;
    }
};
```

时间复杂度：$O\left(\log n\right)$

空间复杂度：$O\left(1\right)$

### 搜索二维矩阵

https://leetcode.cn/problems/search-a-2d-matrix

```c++
class Solution {
public:
    bool searchMatrix(vector<vector<int>>& matrix, int target) {
        int n = matrix.size();
        int m = matrix[0].size();
        int left = 0;
        int right = n * m - 1;
        int mid = -1;
        int row = -1;
        int col = -1;
        while (left <= right) {
            mid = left + ((right - left) >> 1);
            row = mid / m;
            col = mid % m;
            if (target < matrix[row][col]) {
                right = mid - 1;
            } else if (target > matrix[row][col]) {
                left = mid + 1;
            } else {
                return true;
            }
        }
        return false;
    }
};
```

时间复杂度：$O\left(\log{nm}\right)$

空间复杂度：$O\left(1\right)$

也可以先确定行再确定列：

```c++
class Solution {
public:
    bool searchMatrix(vector<vector<int>>& matrix, int target) {
        auto it = upper_bound(matrix.begin(), matrix.end(), target
                              , [](const int& target_, const vector<int>& row)-> bool { return target_ < row[0]; });
        if (it == matrix.begin()) {
            return false;
        }
        --it;
        return binary_search(it->begin(), it->end(), target);
    }
};
```

时间复杂度：$O\left(\log n+\log m\right)=O\left(\log nm\right)$

空间复杂度：$O\left(1\right)$

### 在排序数组中查找元素的第一个和最后一个位置

https://leetcode.cn/problems/find-first-and-last-position-of-element-in-sorted-array

```c++
class Solution {
public:
    vector<int> searchRange(vector<int>& nums, int target) {
        int l = 0;
        int r = nums.size() - 1;
        int m = -1;
        vector<int> res(2, -1);
        while (l <= r) {
            m = l + ((r - l) >> 1);
            if (target <= nums[m]) {
                if (target == nums[m]) {
                    res[0] = m;
                }
                r = m - 1;
            } else {
                l = m + 1;
            }
        }
        l = 0;
        r = nums.size() - 1;
        while (l <= r) {
            m = l + ((r - l) >> 1);
            if (target < nums[m]) {
                r = m - 1;
            } else {
                if (target == nums[m]) {
                    res[1] = m;
                }
                l = m + 1;
            }
        }
        return res;
    }
};
```

时间复杂度：$O\left(\log n\right)$

空间复杂度：$O\left(1\right)$

### 搜索旋转排序数组

https://leetcode.cn/problems/search-in-rotated-sorted-array

![fig1](./刷题笔记.assets/1.png)

```c++
class Solution {
public:
    int search(vector<int>& nums, int target) {
        int l = 0;
        int r = nums.size() - 1;
        int m = -1;
        while (l <= r) {
            m = l + ((r - l) >> 1);
            if (target == nums[m]) {
                return m;
            }
            if (nums[m] >= nums[0]) { // m 在左侧
                if (target >= nums[0] && target < nums[m]) {
                    r = m - 1;
                } else {
                    l = m + 1;
                }
            } else { // m 在右侧
                if (target <= nums[nums.size() - 1] && target > nums[m]) {
                    l = m + 1;
                } else {
                    r = m - 1;
                }
            } bvcxA
        }
        return -1;
    }
};
```

时间复杂度：$O\left(\log n\right)$

空间复杂度：$O\left(1\right)$

### 寻找旋转排序数组中的最小值

https://leetcode.cn/problems/find-minimum-in-rotated-sorted-array

![fig1](./刷题笔记.assets/1.png)

分 `m` 在左侧还是右侧两种情况讨论：

```c++
class Solution {
public:
    int findMin(vector<int>& nums) {
        int l = 0;
        int r = nums.size() - 1;
        if (l == r || nums[0] < nums[r]) {
            return nums[0];
        }
        int m = -1;
        while (l <= r) {
            m = l + ((r - l) >> 1);
            if (m > 0 && nums[m] < nums[m - 1]) {
                break;
            }
            if (nums[m] >= nums[0]) { // m 在左侧
                l = m + 1;
            } else { // m 在右侧
                r = m - 1;
            }
        }
        return nums[m];
    }
};
```

时间复杂度：$O\left(\log n\right)$

空间复杂度：$O\left(1\right)$

### 寻找两个正序数组的中位数

https://leetcode.cn/problems/median-of-two-sorted-arrays

```c++
class Solution {
public:
    double findMedianSortedArrays(vector<int>& nums1, vector<int>& nums2) {
        int n = nums1.size();
        int m = nums2.size();
        // 在更短的数组上进行二分查找
        if (n > m) {
            return findMedianSortedArrays(nums2, nums1);
        }
        int l = 0;
        int r = n - 1;
        int e1 = -1;
        int e2 = -1;
        int total = n + m;
        int leftNum = total >> 1;
        int left1 = INT_MIN;
        int right1 = INT_MAX;
        int left2 = INT_MIN;
        int right2 = INT_MAX;
        while (true) {
            // 不能写成 (l+r)/2，因为 l==0 且 r==-1 时算出来的结果是 0，陷入死循环，下面这样写算出来的结果是 -1
            e1 = l + ((r - l) >> 1);
            e2 = leftNum - e1 - 2;
            left1 = e1 >= 0 ? nums1[e1] : INT_MIN;
            right1 = e1 + 1 < n ? nums1[e1 + 1] : INT_MAX;
            left2 = e2 >= 0 ? nums2[e2] : INT_MIN;
            right2 = e2 + 1 < m ? nums2[e2 + 1] : INT_MAX;
            if (left1 > right2) {
                r = e1 - 1;
            } else if (left2 > right1) {
                l = e1 + 1;
            } else {
                break;
            }
        }
        return (total & 1) == 1
                   ? min(right1, right2)
                   : (max(left1, left2) + min(right1, right2)) / 2.0;
    }
};
```

时间复杂度：$O\left(\log\min\left(n,m\right)\right)$

空间复杂度：$O\left(1\right)$

## 栈

### 有效的括号

https://leetcode.cn/problems/valid-parentheses

```c++
const int MAXN = 1e4;

class Solution {
private:
    char stk[MAXN];
    int t;

public:
    bool isValid(string str) {
        t = 0;
        for (char& c: str) {
            if (c == '(') {
                stk[t++] = ')';
            } else if (c == '[') {
                stk[t++] = ']';
            } else if (c == '{') {
                stk[t++] = '}';
            }
            // ')',']','}'
            else if (t == 0 || c != stk[t - 1]) {
                return false;
            } else {
                --t;
            }
        }
        return t == 0;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

### 最小栈

https://leetcode.cn/problems/min-stack

前面已完成

### 字符串解码

https://leetcode.cn/problems/decode-string

前面已完成

### 每日温度

https://leetcode.cn/problems/daily-temperatures

单调栈

```c++
const int MAXN = 1e5;

class Solution {
private:
    int stk[MAXN]; // 放等待结算的下标
    int t;

public:
    vector<int> dailyTemperatures(vector<int>& ts) {
        t = 0;
        vector<int> res(ts.size());
        for (int i = 0; i < ts.size(); ++i) {
            while (t > 0 && ts[i] > ts[stk[t - 1]]) {
                res[stk[t - 1]] = i - stk[t - 1]; // 找到了下一个最高的温度，开始结算下标 stk[t-1]
                --t;
            }
            stk[t++] = i;
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

### 柱状图中最大的矩形

https://leetcode.cn/problems/largest-rectangle-in-histogram

https://www.bilibili.com/video/BV1HH4y1X7T9

```c++
const int MAXN = 1e5;

class Solution {
private:
    int stk[MAXN];
    int t;

public:
    int largestRectangleArea(vector<int>& heights) {
        t = 0;
        int res = 0;
        int hidx = -1;
        int l = -1;
        for (int i = 0; i < heights.size(); ++i) {
            while (t > 0 && heights[i] <= heights[stk[t - 1]]) {
                // 以 hidx 位置的 heights[hidx] 为高，往两边扩，最远能扩到哪里
                hidx = stk[--t];
                l = t > 0 ? stk[t - 1] : -1;
                res = max(res, (i - l - 1) * heights[hidx]); // 在 (l,i) 上求面积
            }
            stk[t++] = i;
        }
        while (t > 0) {
            hidx = stk[--t];
            l = t > 0 ? stk[t - 1] : -1;
            res = max(res, ((int) heights.size() - l - 1) * heights[hidx]);
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

## 堆

### 数组中的第 K 个最大元素

https://leetcode.cn/problems/kth-largest-element-in-an-array

前面已完成

### 前 K 个高频元素

https://leetcode.cn/problems/top-k-frequent-elements

```c++
class Solution {
private:
    struct Comp {
        bool operator()(const pair<int, int>& p1, const pair<int, int>& p2) const {
            return p1.second > p2.second;
        }
    };

public:
    vector<int> topKFrequent(vector<int>& nums, int k) {
        unordered_map<int, int> ht;
        for (int& num: nums) {
            ++ht[num];
        }
        priority_queue<pair<int, int>, vector<pair<int, int>>, Comp> pq;
        for (auto& p: ht) {
            if (pq.size() < k) {
                pq.push(p);
            } else if (pq.size() == k && p.second > pq.top().second) {
                pq.pop();
                pq.push(p);
            }
        }
        vector<int> res(k);
        for (int i = 0; i < k; ++i) {
            res[i] = pq.top().first;
            pq.pop();
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\log k\right)$

空间复杂度：$O\left(n\right)$

和 https://leetcode.cn/problems/kth-largest-element-in-an-array 一样，仍然可以用快排的思想

```c++
class Solution {
private:
    // 在 arr 的 [l,r] 范围上将频率最高的前 k 个元素放到 [0,k) 的范围上
    void f(vector<vector<int>>& arr, int l, int r, int k) {
        if (k == r - l + 1) {
            return;
        }
        int bounds[2] = {};
        partition(arr, l, r, arr[l + rand() % (r - l + 1)][1], bounds);
        if (bounds[1] < l + k - 1) {
            f(arr, bounds[1] + 1, r, l + k - bounds[1] - 1);
        } else if (bounds[0] > l + k - 1) {
            f(arr, l, bounds[0] - 1, k);
        }
    }

    void partition(vector<vector<int>>& arr, int l, int r, int x, int bounds[]) {
        // [l,i)：大于区
        // [i,j)：等于区
        // [j,k]：待看区
        // (k,r]：小于区
        int i = l;
        int j = l;
        int k = r;
        while (j <= k) {
            if (arr[j][1] < x) {
                swap(arr[j], arr[k--]);
            } else if (arr[j][1] > x) {
                swap(arr[i++], arr[j++]);
            } else {
                ++j;
            }
        }
        bounds[0] = i;
        bounds[1] = k;
    }

public:
    vector<int> topKFrequent(vector<int>& nums, int k) {
        unordered_map<int, int> ht;
        for (int& num: nums) {
            ++ht[num];
        }
        int i = 0;
        vector<vector<int>> arr(ht.size(), vector<int>(2));
        for (auto& p: ht) {
            arr[i][0] = p.first;
            arr[i++][1] = p.second;
        }
        srand(time(nullptr));
        f(arr, 0, arr.size() - 1, k);
        vector<int> res(k);
        for (i = 0; i < k; ++i) {
            res[i] = arr[i][0];
        }
        return res;
    }
};
```

时间复杂度：构建哈希表 + 递归（定量证明略），$O\left(n\right)+O\left(\log n\right)=O\left(n\right)$

空间复杂度：哈希表 + 递归栈，$O\left(n\right)+O\left(\log n\right)=O\left(n\right)$

### 数据流的中位数

https://leetcode.cn/problems/find-median-from-data-stream

前面已完成

## 贪心算法

### 买卖股票的最佳时机

https://leetcode.cn/problems/best-time-to-buy-and-sell-stock

```c++
class Solution {
public:
    int maxProfit(vector<int>& prices) {
        int res = 0;
        for (int i = 1, prevMinPrice = prices[0]; i < prices.size(); ++i) {
            res = max(res, prices[i] - prevMinPrice);
            prevMinPrice = min(prevMinPrice, prices[i]);
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 跳跃游戏

https://leetcode.cn/problems/jump-game

```c++
class Solution {
public:
    bool canJump(vector<int>& nums) {
        int maxIdx = 0;
        bool res = true;
        for (int i = 0; i < nums.size(); ++i) {
            if (i > maxIdx) {
                res = false;
                break;
            }
            maxIdx = max(maxIdx, i + nums[i]);
            if (maxIdx >= nums.size() - 1) {
                break;
            }
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 跳跃游戏 II

https://leetcode.cn/problems/jump-game-ii

```c++
class Solution {
public:
    int jump(vector<int>& nums) {
        int res = 0;
        int rmi = 0; // right most index，跳 res 步，最远能来到 rmi
        int nrmi = -1; // next right most index，跳 res+1 步，最远能来到 nrmi
        for (int i = 0; i < nums.size(); ++i) {
            if (i > rmi) {
                ++res;
                rmi = nrmi;
            }
            nrmi = max(nrmi, i + nums[i]);
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 划分字母区间

https://leetcode.cn/problems/partition-labels

```c++
class Solution {
public:
    vector<int> partitionLabels(string str) {
        int lasts[26] = {};
        for (int i = 0; i < str.size(); ++i) {
            lasts[str[i] - 'a'] = i;
        }
        vector<int> res;
        for (int i = 0, j = 0, k = -1; j < str.size(); ++j) {
            k = max(k, lasts[str[j] - 'a']);
            if (j == k) {
                res.push_back(j - i + 1);
                i = j + 1;
            }
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(\left|\Sigma\right|\right)$，$\Sigma$ 是字符集

## 动态规划

### 爬楼梯

https://leetcode.cn/problems/climbing-stairs

```c++
class Solution {
public:
    int climbStairs(int n) {
        int prev1 = 1;
        int prev2 = 1;
        for (int i = 2, curr = -1; i <= n; ++i) {
            curr = prev1 + prev2;
            prev1 = prev2;
            prev2 = curr;
        }
        return prev2;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 杨辉三角

https://leetcode.cn/problems/pascals-triangle

```c++
class Solution {
public:
    vector<vector<int>> generate(int numRows) {
        vector<vector<int>> res(numRows);
        for (int i = 0; i < numRows; ++i) {
            res[i].resize(i + 1);
            res[i][0] = res[i][i] = 1;
            for (int j = 1; j < i; ++j) {
                res[i][j] = res[i - 1][j - 1] + res[i - 1][j];
            }
        }
        return res;
    }
};
```

时间复杂度：$O\left(n^2\right)$

空间复杂度：$O\left(1\right)$

### 打家劫舍

https://leetcode.cn/problems/house-robber

下面的解答都适用于数组中存在负数的情况。

动态规划，第一种写法：

```c++
class Solution {
public:
    // 子序列最大累加和问题
    int rob(vector<int>& nums) {
        int n = nums.size();
        if (n == 1) {
            return nums[0];
        }
        vector<int> dp(n); // 子序列必须以 nums[i] 结尾，最大累加和是 dp[i]
        dp[0] = nums[0];
        dp[1] = nums[1];
        int prevMax = dp[0], // dp[0...i-2] 上的最大值
                res = max(dp[0], dp[1]);
        for (int i = 2; i < n; ++i) {
            dp[i] = max(nums[i], prevMax + nums[i]);
            prevMax = max(prevMax, dp[i - 1]);
            res = max(res, dp[i]);
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

动态规划，第二种写法：

```c++
class Solution {
public:
    int rob(vector<int>& nums) {
        int n = nums.size();
        if (n == 1) {
            return nums[0];
        }
        vector<int> dp(n);  // nums[0...i] 上的最大累加和为 dp[i]，nums[i] 不一定非要选
        dp[0] = nums[0];
        dp[1] = max(nums[0], nums[1]);
        for (int i = 2; i < n; ++i) {
            dp[i] = max(dp[i - 1],                         // 不选 nums[i]
                        max(nums[i], dp[i - 2] + nums[i])  // 选 nums[i]
            );
        }
        return dp[n - 1];
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

空间压缩：

```c++
class Solution {
public:
    int rob(vector<int>& nums) {
        int n = nums.size();
        if (n == 1) {
            return nums[0];
        }
        int prev1 = nums[0];
        int prev2 = max(nums[0], nums[1]);
        for (int i = 2, curr = -1; i < n; ++i) {
            curr = max(prev2, prev1 + nums[i]);
            prev1 = prev2;
            prev2 = curr;
        }
        return prev2;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 完全平方数

https://leetcode.cn/problems/perfect-squares

```c++
class Solution {
public:
    int numSquares(int n) {
        vector<int> dp(n + 1, INT_MAX);
        dp[0] = 0;
        for (int i = 1; i <= n; ++i) {
            for (int b = 1; b * b <= i; ++b) {
                dp[i] = min(dp[i], dp[i - b * b] + 1);
            }
        }
        return dp[n];
    }
};
```

时间复杂度：$O\left(n\sqrt n\right)=O\left(n^{\frac{3}{2}}\right)$

空间复杂度：$O\left(n\right)$

### 零钱兑换

https://leetcode.cn/problems/coin-change

未优化版本：

```c++
class Solution {
public:
    int coinChange(vector<int>& coins, int amount) {
        int n = coins.size();
        vector<vector<int>> dp(n, vector<int>(amount + 1, -1));
        for (int num = 0, j = 0; j <= amount; ++num, j += coins[0]) {
            dp[0][j] = num;
        }
        for (int i = 1; i < n; ++i) {
            for (int j = 0; j <= amount; ++j) {
                for (int num = 0; num * coins[i] <= j; ++num) {
                    if (dp[i - 1][j - num * coins[i]] != -1) {
                        dp[i][j] = dp[i][j] != -1
                                       ? min(dp[i][j], dp[i - 1][j - num * coins[i]] + num)
                                       : dp[i - 1][j - num * coins[i]] + num;
                    }
                }
            }
        }
        return dp[n - 1][amount];
    }
};
```

时间复杂度：$O\left(na^2\right)$

空间复杂度：$O\left(na\right)$

动态规划 + 斜率优化：

```c++
class Solution {
public:
    int coinChange(vector<int>& coins, int amount) {
        int n = coins.size();
        vector<vector<int>> dp(n, vector<int>(amount + 1, -1));
        for (int num = 0, j = 0; j <= amount; ++num, j += coins[0]) {
            dp[0][j] = num;
        }
        for (int i = 1; i < n; ++i) {
            for (int j = 0; j <= amount; ++j) {
                dp[i][j] = dp[i - 1][j];
                if (coins[i] <= j && dp[i][j - coins[i]] != -1) {
                    dp[i][j] = dp[i][j] != -1
                                   ? min(dp[i][j], dp[i][j - coins[i]] + 1)
                                   : dp[i][j - coins[i]] + 1;
                }
            }
        }
        return dp[n - 1][amount];
    }
};
```

时间复杂度：$O\left(na\right)$

空间复杂度：$O\left(na\right)$

空间压缩：

```c++
class Solution {
public:
    int coinChange(vector<int>& coins, int amount) {
        int n = coins.size();
        vector<int> dp(amount + 1, -1);
        dp[0] = 0;
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j <= amount; ++j) {
                if (coins[i] <= j && dp[j - coins[i]] != -1) {
                    dp[j] = dp[j] != -1
                                ? min(dp[j], dp[j - coins[i]] + 1)
                                : dp[j - coins[i]] + 1;
                }
            }
        }
        return dp[amount];
    }
};
```

时间复杂度：$O\left(na\right)$

空间复杂度：$O\left(a\right)$

### 单词拆分

https://leetcode.cn/problems/word-break

回溯（超出时间限制）：

```c++
class Solution {
private:
    bool f(string& str, int i, vector<string>& wordDict) {
        if (i == str.size()) {
            return true;
        }
        for (string& word: wordDict) {
            if (i + word.size() <= str.size()
                && str.substr(i, word.size()) == word
                && f(str, i + word.size(), wordDict)) {
                return true;
            }
        }
        return false;
    }

public:
    bool wordBreak(string str, vector<string>& wordDict) {
        return f(str, 0, wordDict);
    }
};
```

时间复杂度：$O\left(n^m\right)$

空间复杂度：$O\left(n\right)$

其中 $n$ 和 $m$ 分别为 `s` 和 `wordDict` 的大小。

完全背包（可以使用前缀树进一步优化）：

```c++
class Solution {
public:
    bool wordBreak(string str, vector<string>& wordDict) {
        int n = str.size();
        vector<bool> dp(n + 1);
        dp[n] = true;
        for (int i = n - 1; i >= 0; --i) {
            for (string& word: wordDict) {
                if (i + word.size() <= str.size()
                    && str.substr(i, word.size()) == word
                    && dp[i + word.size()]) {
                    dp[i] = true;
                    break;
                }
            }
        }
        return dp[0];
    }
};
```

时间复杂度：$O\left(nm\right)$

空间复杂度：$O\left(n\right)$

### 最长递增子序列

https://leetcode.cn/problems/longest-increasing-subsequence

未优化版本：

```c++
class Solution {
public:
    int lengthOfLIS(vector<int>& nums) {
        int n = nums.size();
        vector<int> dp(n, 1); // 子序列必须以 nums[i] 结尾，LIS 是多少
        int res = 1;
        for (int i = 1; i < n; ++i) {
            for (int j = 0; j < i; ++j) { // 枚举子序列的倒数第二个数
                if (nums[j] < nums[i]) {
                    dp[i] = max(dp[i], dp[j] + 1);
                }
            }
            res = max(res, dp[i]);
        }
        return res;
    }
};
```

时间复杂度：$O\left(n^2\right)$

空间复杂度：$O\left(n\right)$

优化版本：

https://www.bilibili.com/video/BV1ne411D7CQ

```c++
class Solution {
public:
    int lengthOfLIS(vector<int>& nums) {
        int n = nums.size();
        vector<int> ends(n); // ends 数组用于维护递增子序列的最小末尾值，其中 ends[i] 表示当前长度为 i+1 的子序列的最小末尾
        int len = 0; // len 表示当前找到的最长递增子序列的长度
        for (int i = 0, j = -1; i < n; ++i) {
            j = bs(ends, len, nums[i]); // 在 ends[0...len-1] 范围上查找大于等于 nums[i] 的最小值位置，-1 表示不存在
            if (j != -1) {
                ends[j] = nums[i];
            } else {
                ends[len++] = nums[i];
            }
        }
        return len;
    }

    int bs(vector<int>& ends, int len, int x) {
        int l = 0;
        int r = len - 1;
        int m = -1;
        int res = -1;
        while (l <= r) {
            m = l + ((r - l) >> 1);
            if (x > ends[m]) {
                l = m + 1;
            } else {
                res = m;
                r = m - 1;
            }
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\log n\right)$

空间复杂度：$O\left(n\right)$

### 乘积最大子数组

https://leetcode.cn/problems/maximum-product-subarray

下面的代码也适用于数组类型为 `double` 的情况。

注意 `double` 改成 `long long` 中间结果会溢出，但是 `double` 会用 **科学计数法** 表示非常大的数值，这可以避免溢出问题。

```c++
class Solution {
public:
    int maxProduct(vector<int>& nums) {
        int n = nums.size();
        vector<double> mindp(n);
        vector<double> maxdp(n);
        mindp[0] = maxdp[0] = nums[0];
        double res = nums[0];
        for (int i = 1; i < n; ++i) {
            mindp[i] = min((double) nums[i], min(mindp[i - 1] * nums[i], maxdp[i - 1] * nums[i]));
            maxdp[i] = max((double) nums[i], max(mindp[i - 1] * nums[i], maxdp[i - 1] * nums[i]));
            res = max(res, maxdp[i]);
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

空间压缩：

```c++
class Solution {
public:
    int maxProduct(vector<int>& nums) {
        int n = nums.size();
        double prev1 = nums[0];
        double prev2 = nums[0];
        double res = nums[0];
        double curr1 = 0;
        double curr2 = 0;
        for (int i = 1; i < n; ++i) {
            curr1 = min((double) nums[i], min(prev1 * nums[i], prev2 * nums[i]));
            curr2 = max((double) nums[i], max(prev1 * nums[i], prev2 * nums[i]));
            res = max(res, curr2);
            prev1 = curr1;
            prev2 = curr2;
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 分割等和子集

https://leetcode.cn/problems/partition-equal-subset-sum

动态规划：这个问题等价于，记数组元素累加和为 $s$，$t=s/2$，是否能在数组中找出累加和为 $t$ 的元素集合。相似的题目为零钱兑换。

未优化版本：

```c++
class Solution {
public:
    bool canPartition(vector<int>& nums) {
        int sum = accumulate(nums.begin(), nums.end(), 0);
        if ((sum & 1) == 1) {
            return false;
        }
        int n = nums.size();
        int target = sum >> 1;
        vector<vector<int>> dp(n, vector<int>(target + 1)); // nums[0...i] 范围上，容量为 j 的背包，最多能装 dp[j]
        for (int j = nums[0]; j <= target; ++j) {
            dp[0][j] = nums[0];
        }
        for (int i = 1; i < n; ++i) {
            for (int j = 1; j <= target; ++j) {
                dp[i][j] = dp[i - 1][j];
                if (j >= nums[i]) {
                    dp[i][j] = max(dp[i][j], dp[i - 1][j - nums[i]] + nums[i]);
                }
            }
        }
        return dp[n - 1][target] == target;
    }
};
```

时间复杂度：$O\left(nt\right)$

空间复杂度：$O\left(nt\right)$

其中，$t$ 为 `target`

```c++
class Solution {
public:
    bool canPartition(vector<int>& nums) {
        int sum = accumulate(nums.begin(), nums.end(), 0);
        if ((sum & 1) == 1) {
            return false;
        }
        int n = nums.size();
        int target = sum >> 1;
        vector<int> dp(target + 1);
        for (int j = nums[0]; j <= target; ++j) {
            dp[j] = nums[0];
        }
        for (int i = 1; i < n; ++i) {
            for (int j = target; j >= 0; --j) {
                if (j >= nums[i]) {
                    dp[j] = max(dp[j], dp[j - nums[i]] + nums[i]);
                }
            }
        }
        return dp[target] == target;
    }
};
```

时间复杂度：$O\left(nt\right)$

空间复杂度：$O\left(t\right)$

### 最长有效括号

https://leetcode.cn/problems/longest-valid-parentheses

https://www.bilibili.com/video/BV1Ww41167Ac

```c++
class Solution {
public:
    int longestValidParentheses(string str) {
        int n = str.size();
        vector<int> dp(n); // 必须以 str[i] 结尾
        int res = 0;
        for (int i = 1, p = -1; i < n; ++i) {
            if (str[i] == '(') {
                continue;
            }
            p = i - dp[i - 1] - 1; // 把 dp[i - 1] 表示的子串夹在中间后，和 str[i] 匹配的左括号（也可能是右括号）的下标
            if (p >= 0 && str[p] == '(') {
                dp[i] = (p > 0 ? dp[p - 1] : 0) + dp[i - 1] + 2;
            }
            res = max(res, dp[i]);
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

## 多维动态规划

### 不同路径

https://leetcode.cn/problems/unique-paths

未优化版本：

```c++
class Solution {
public:
    int uniquePaths(int n, int m) {
        vector<vector<int>> dp(n, vector<int>(m));
        for (int i = 0; i < n; ++i) {
            dp[i][0] = 1;
        }
        for (int j = 1; j < m; ++j) {
            dp[0][j] = 1;
        }
        for (int i = 1; i < n; ++i) {
            for (int j = 1; j < m; ++j) {
                dp[i][j] = dp[i - 1][j] + dp[i][j - 1];
            }
        }
        return dp[n - 1][m - 1];
    }
};
```

时间复杂度：$O\left(nm\right)$

空间复杂度：$O\left(nm\right)$

空间压缩：

```c++
class Solution {
public:
    int uniquePaths(int n, int m) {
        vector<int> dp(m);
        for (int j = 0; j < m; ++j) {
            dp[j] = 1;
        }
        for (int i = 1; i < n; ++i) {
            for (int j = 1; j < m; ++j) {
                dp[j] += dp[j - 1];
            }
        }
        return dp[m - 1];
    }
};
```

时间复杂度：$O\left(nm\right)$

空间复杂度：$O\left(m\right)$

排列组合：

$C_{m+n-2}^{m-1}$ 表示在 $m+n-2$ 步中选 $m-1$ 步向下走，其余的步数向右走。其中：
$$
C_{m+n-2}^{m-1}=\frac{\left(m+n-2\right)!}{\left(m-1\right)!\left(n-1\right)!}=\frac{\left(m+n-2\right)\left(m+n-3\right)\cdots n}{\left(m-1\right)!}=\frac{\left(m+n-2\right)\left(m+n-3\right)\cdots m}{\left(n-1\right)!}
$$

```c++
class Solution {
private:
    typedef long long ll;

public:
    int uniquePaths(int n, int m) {
        ll res = 1;
        for (ll numerator = n, denominator = 1; denominator < m; ++numerator, ++denominator) {
            res = res * numerator / denominator;
        }
        return res;
    }
};
```

时间复杂度：$O\left(m\right)$

空间复杂度：$O\left(1\right)$

### 最小路径和

https://leetcode.cn/problems/minimum-path-sum

未优化版本：

```c++
class Solution {
public:
    int minPathSum(vector<vector<int>>& grid) {
        int n = grid.size();
        int m = grid[0].size();
        vector<vector<int>> dp(n, vector<int>(m));
        dp[0][0] = grid[0][0];
        for (int i = 1; i < n; ++i) {
            dp[i][0] = dp[i - 1][0] + grid[i][0];
        }
        for (int j = 1; j < m; ++j) {
            dp[0][j] += dp[0][j - 1] + grid[0][j];
        }
        for (int i = 1; i < n; ++i) {
            for (int j = 1; j < m; ++j) {
                dp[i][j] = min(dp[i - 1][j], dp[i][j - 1]) + grid[i][j];
            }
        }
        return dp[n - 1][m - 1];
    }
};
```

时间复杂度：$O\left(nm\right)$

空间复杂度：$O\left(nm\right)$

空间压缩：

```c++
class Solution {
public:
    int minPathSum(vector<vector<int>>& grid) {
        int n = grid.size();
        int m = grid[0].size();
        vector<int> dp(m);
        dp[0] = grid[0][0];
        for (int j = 1; j < m; ++j) {
            dp[j] += dp[j - 1] + grid[0][j];
        }
        for (int i = 1; i < n; ++i) {
            dp[0] += grid[i][0];
            for (int j = 1; j < m; ++j) {
                dp[j] = min(dp[j], dp[j - 1]) + grid[i][j];
            }
        }
        return dp[m - 1];
    }
};
```

时间复杂度：$O\left(nm\right)$

空间复杂度：$O\left(m\right)$

### 最长回文子串

https://leetcode.cn/problems/longest-palindromic-substring

未优化版本：

```c++
class Solution {
public:
    string longestPalindrome(string str) {
        int n = str.size();
        vector<vector<bool>> dp(n, vector<bool>(n)); // str[i...j] 是否是回文串
        for (int i = 0; i < n; ++i) {
            dp[i][i] = true;
        }
        int l = 0;
        int r = 0;
        for (int i = n - 2; i >= 0; --i) {
            for (int j = i + 1; j < n; ++j) {
                if (str[i] == str[j] && (i + 1 == j || dp[i + 1][j - 1])) {
                    dp[i][j] = true;
                    if (j - i > r - l) {
                        l = i;
                        r = j;
                    }
                }
            }
        }
        return str.substr(l, r - l + 1);
    }
};
```

时间复杂度：$O\left(n^2\right)$

空间复杂度：$O\left(n^2\right)$

空间压缩：

```c++
class Solution {
public:
    string longestPalindrome(string str) {
        int n = str.size();
        vector<bool> dp(n);
        dp[n - 1] = true;
        int l = 0;
        int r = 0;
        for (int i = n - 2; i >= 0; --i) {
            for (int j = n - 1; j > i; --j) {
                dp[j] = str[i] == str[j] && (i + 1 == j || dp[j - 1]); // 这里抄的时候注意改一下
                if (dp[j] && j - i > r - l) {
                    l = i;
                    r = j;
                }
            }
            dp[i] = true;
        }
        return str.substr(l, r - l + 1);
    }
};
```

时间复杂度：$O\left(n^2\right)$

空间复杂度：$O\left(n\right)$

中心扩展：

```c++
class Solution {
public:
    string longestPalindrome(string str) {
        int n = str.size();
        int s = 0;
        int e = 0;
        for (int i = 0, l = -1, r = -1; i < n; ++i) {
            l = i - 1;
            r = i + 1;
            // 奇数长度
            while (l >= 0 && r < n && str[l] == str[r]) {
                if (r - l > e - s) {
                    s = l;
                    e = r;
                }
                --l;
                ++r;
            }
            // 偶数长度
            l = i;
            r = i + 1;
            while (l >= 0 && r < n && str[l] == str[r]) {
                if (r - l > e - s) {
                    s = l;
                    e = r;
                }
                --l;
                ++r;
            }
        }
        return str.substr(s, e - s + 1);
    }
};
```

时间复杂度：$O\left(n^2\right)$

空间复杂度：$O\left(1\right)$

Manacher 算法：

略……

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(n\right)$

### 最长公共子序列

https://leetcode.cn/problems/longest-common-subsequence

`dp[i][j]` 表示 `str1[0...i]` 和 `str2[0...j]` 上的最长公共子序列的长度。

算 `dp[i][j]` 时这样分类：

1. `str1[i]` 不是最长公共子序列的一部分，这时 `dp[i][j] = dp[i - 1][j]`；
2. `str2[j]` 不是最长公共子序列的一部分，这时 `dp[i][j] = dp[i][j - 1]`；
3. `str1[i]` 和 `str2[j]` 都是最长公共子序列的一部分（要求 `str1[i] == str2[j]`），这时 `dp[i][j] = dp[i - 1][j - 1] + 1`；

未优化版本：

```c++
class Solution {
public:
    int longestCommonSubsequence(string str1, string str2) {
        int n = str1.size();
        int m = str2.size();
        vector<vector<int>> dp(n, vector<int>(m));
        dp[0][0] = str1[0] == str2[0] ? 1 : 0;
        for (int i = 1; i < n; ++i) {
            dp[i][0] = str1[i] == str2[0] ? 1 : dp[i - 1][0];
        }
        for (int j = 1; j < m; ++j) {
            dp[0][j] = str1[0] == str2[j] ? 1 : dp[0][j - 1];
        }
        for (int i = 1; i < n; ++i) {
            for (int j = 1; j < m; ++j) {
                dp[i][j] = max(dp[i - 1][j], dp[i][j - 1]);
                if (str1[i] == str2[j]) {
                    dp[i][j] = max(dp[i][j], dp[i - 1][j - 1] + 1);
                }
            }
        }
        return dp[n - 1][m - 1];
    }
};
```

时间复杂度：$O\left(nm\right)$

空间复杂度：$O\left(nm\right)$

空间压缩：

```c++
class Solution {
public:
    int longestCommonSubsequence(string str1, string str2) {
        int n = str1.size();
        int m = str2.size();
        vector<int> dp(m);
        dp[0] = str1[0] == str2[0] ? 1 : 0;
        for (int j = 1; j < m; ++j) {
            dp[j] = str1[0] == str2[j] ? 1 : dp[j - 1];
        }
        for (int i = 1, temp1 = -1, temp2 = -1; i < n; ++i) {
            temp1 = dp[0];
            if (str1[i] == str2[0]) {
                dp[0] = str1[i] == str2[0];
            }
            for (int j = 1; j < m; ++j) {
                temp2 = dp[j];
                dp[j] = max(dp[j], dp[j - 1]);
                if (str1[i] == str2[j]) {
                    dp[j] = max(dp[j], temp1 + 1);
                }
                temp1 = temp2;
            }
        }
        return dp[m - 1];
    }
};
```

时间复杂度：$O\left(nm\right)$

空间复杂度：$O\left(m\right)$

### 编辑距离

https://leetcode.cn/problems/edit-distance

```c++
class Solution {
public:
    int minDistance(string word1, string word2) {
        // 插入、删除和替换的代价
        int a = 1;
        int d = 1;
        int r = 1;
        int n1 = word1.size();
        int n2 = word2.size();
        vector<vector<int>> dp(n1 + 1, vector<int>(n2 + 1)); // word1 的前 i 个字符变到 word2 的前 j 个字符，需要的最小代价是 dp[i][j]
        for (int i = 1; i <= n1; ++i) {
            dp[i][0] = i * d;
        }
        for (int j = 1; j <= n2; ++j) {
            dp[0][j] = j * a;
        }
        // word1[i-1]==word2[j-1] 时，word1[0...i-1] 匹配 word2[0...j-1]
        // word1[i-1]!=word2[j-1] 时，word1[0...i-1] 匹配 word2[0...j-1]，word1[i-1] 替换为 word2[j-1]
        // word1[0...i-1] 匹配 word2[0...j-2]，word1[0...i-1] 后面再插入 word2[j-1]
        // word1[0...i-2] 匹配 word2[0...j-1]，word1[0...i-1] 再删除 word1[i-1]
        // 以上的 p1、p2 和 p3 为 word1[i-1] 参与匹配的情况，p4 为 word1[i-1] 不参与匹配的情况
        for (int i = 1, p1 = -1, p2 = -1, p3 = -1, p4 = -1; i <= n1; ++i) {
            for (int j = 1; j <= n2; ++j) {
                p1 = INT_MAX;
                p2 = INT_MAX;
                if (word1[i - 1] == word2[j - 1]) {
                    p1 = dp[i - 1][j - 1];
                } else {
                    p2 = dp[i - 1][j - 1] + r;
                }
                p3 = dp[i][j - 1] + a;
                p4 = dp[i - 1][j] + d;
                dp[i][j] = min(min(p1, p2), min(p3, p4));
            }
        }
        return dp[n1][n2];
    }
};
```

时间复杂度：$O\left(n_1n_2\right)$

空间复杂度：$O\left(n_1n_2\right)$

枚举小优化版：

```c++
class Solution {
public:
    int minDistance(string word1, string word2) {
        int a = 1;
        int d = 1;
        int r = 1;
        int n1 = word1.size();
        int n2 = word2.size();
        vector<vector<int>> dp(n1 + 1, vector<int>(n2 + 1));
        for (int i = 1; i <= n1; ++i) {
            dp[i][0] = i * d;
        }
        for (int j = 1; j <= n2; ++j) {
            dp[0][j] = j * a;
        }
        for (int i = 1; i <= n1; ++i) {
            for (int j = 1; j <= n2; ++j) {
                if (word1[i - 1] == word2[j - 1]) { // 这时 word1[i-1] 直接匹配 word2[j-1] 是最优的情况（贪心）
                    dp[i][j] = dp[i - 1][j - 1];
                } else {
                    dp[i][j] = min(dp[i - 1][j - 1] + r, min(dp[i][j - 1] + a, dp[i - 1][j] + d));
                }
            }
        }
        return dp[n1][n2];
    }
};
```

时间复杂度：$O\left(nm\right)$

空间复杂度：$O\left(nm\right)$

空间压缩：

```c++
class Solution {
public:
    int minDistance(string word1, string word2) {
        int a = 1;
        int d = 1;
        int r = 1;
        int n1 = word1.size();
        int n2 = word2.size();
        vector<int> dp(n2 + 1);
        for (int j = 1; j <= n2; ++j) {
            dp[j] = j * a;
        }
        for (int i = 1, temp1 = -1, temp2 = -1; i <= n1; ++i) {
            temp1 = dp[0];
            dp[0] = i * d;
            for (int j = 1; j <= n2; ++j) {
                temp2 = dp[j];
                if (word1[i - 1] == word2[j - 1]) { // 这时 word1[i-1] 直接匹配 word2[j-1] 是最优的情况（贪心）
                    dp[j] = temp1;
                } else {
                    dp[j] = min(temp1 + r, min(dp[j - 1] + a, dp[j] + d));
                }
                temp1 = temp2;
            }
        }
        return dp[n2];
    }
};
```

时间复杂度：$O\left(nm\right)$

空间复杂度：$O\left(m\right)$

## 技巧

### 只出现一次的数字

https://leetcode.cn/problems/single-number

前面已完成

### 多数元素

https://leetcode.cn/problems/majority-element

https://www.bilibili.com/video/BV1Br421G7hB

```c++
class Solution {
private:
    const int INVALIDE_TAG = -1e9 - 1;

public:
    int majorityElement(vector<int>& nums) {
        int cand = INVALIDE_TAG; // 候选
        int hp = 0; // 血量，为 0 表示没有 cand
        for (int& num: nums) {
            if (hp == 0) {
                cand = num;
                hp = 1;
            } else if (num != cand) {
                --hp; // num 和 cand 被消掉了
            } else {
                ++hp;
            }
        }
        if (hp == 0) {
            return INVALIDE_TAG;
        }
        hp = 0; // 复用变量用于计数
        for (int& num: nums) {
            if (num == cand) {
                ++hp;
            }
        }
        return hp > nums.size() / 2 ? cand : INVALIDE_TAG;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 颜色分类

https://leetcode.cn/problems/sort-colors

```c++
class Solution {
public:
    void sortColors(vector<int>& nums) {
        int i = 0;
        int j = 0;
        int k = nums.size() - 1;
        while (j <= k) {
            if (nums[j] == 0) {
                swap(nums[i++], nums[j++]);
            } else if (nums[j] == 2) {
                swap(nums[j], nums[k--]);
            } else {
                ++j;
            }
        }
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 下一个排列

https://leetcode.cn/problems/next-permutation

```c++
class Solution {
public:
    void nextPermutation(vector<int>& nums) {
        int n = nums.size();
        int i = n - 2;
        while (i >= 0 && nums[i] >= nums[i + 1]) { // 从后往前找第一个严格降的下标
            --i;
        }
        if (i == -1) { // 此时是字典序最大的排列
            reverse(nums.begin(), nums.end());
            return;
        }
        int j = n - 1;
        while (nums[j] <= nums[i]) { // 从后往前找第一个严格大于 nums[i] 的数的下标
            --j;
        }
        swap(nums[i], nums[j]); // 画图可以看出 nums[i+1...] 是非递增的
        reverse(nums.begin() + i + 1, nums.end());
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

### 寻找重复数

https://leetcode.cn/problems/find-the-duplicate-number

二分查找，在 `[1...n]` 中二分查找，因为题目要找的是一个整数，并且这个整数 **有明确的范围**，所以可以使用二分查找。实际工作中并不会使用这种时间换空间的做法

```c++
class Solution {
public:
    int findDuplicate(vector<int>& nums) {
        int n = nums.size() - 1;
        int l = 1;
        int r = n;
        while (l <= r) {
            if (l == r) {
                return l;
            }
            int m = l + ((r - l) >> 1);
            int cnt = 0;
            for (int i = 0; i < n + 1; ++i) {
                if (nums[i] <= m) {
                    ++cnt;
                }
            }
            if (cnt > m) {
                r = m;
            } else {  // cnt == m
                l = m + 1;
            }
        }
        return 0;
    }
};
```

时间复杂度：$O\left(n\log n\right)$

空间复杂度：$O\left(1\right)$

不断把元素放到正确的位置，即 `i` 位置上的元素为 `i+1`，相似的题目是 https://leetcode.cn/problems/first-missing-positive

```c++
class Solution {
public:
    int findDuplicate(vector<int>& nums) {
        int n = nums.size();
        int res = 0;
        for (int i = 0; i < n; ++i) {
            while (i + 1 != nums[i]) {
                if (nums[i] == nums[nums[i] - 1]) {
                    res = nums[i];
                    break;
                }
                swap(nums[i], nums[nums[i] - 1]);
            }
        }
        return res;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

快慢指针，这种方法把数组看成链表，这样做的前提是数组中的元素和下标在同一范围内

https://leetcode.cn/problems/find-the-duplicate-number/solutions/18952/kuai-man-zhi-zhen-de-jie-shi-cong-damien_undoxie-d

```c++
class Solution {
public:
    int findDuplicate(vector<int>& nums) {
        // i 是节点的 value，nums[i] 是节点的 next 指针，0 是头结点的 value
        int f = nums[nums[0]];
        int s = nums[0];
        while (f != s) {
            f = nums[nums[f]];
            s = nums[s];
        }
        f = 0;
        while (f != s) {
            f = nums[f];
            s = nums[s];
        }
        return f;
    }
};
```

时间复杂度：$O\left(n\right)$

空间复杂度：$O\left(1\right)$

# 待处理

https://leetcode.cn/problems/longest-increasing-subsequence
