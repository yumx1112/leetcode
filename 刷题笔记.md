...
public:
    ListNode* reverseList(ListNode* head) {
        ListNode* p1 = nullptr,* p2 = head,* p3;
        while (p2) {
            p3 = p2->next;
            p2->next = p1;
            p1 = p2;
...
public:
    ListNode* mergeTwoLists(ListNode* list1, ListNode* list2) {
        ListNode* p1 = list1,* p2 = list2,* dummyHead = new ListNode(),* p3 = dummyHead;
        while (p1 && p2) {
            if (p1->val < p2->val) {
                p3->next = p1;
                p1 = p1->next;
...
public:
    ListNode* addTwoNumbers(ListNode* l1, ListNode* l2) {
        ListNode* p1 = l1,* p2 = l2,* dummyHead = new ListNode(),* p3 = dummyHead;
        int sum, val, carry = 0;
        while (p1 || p2) {
...
public:
    ListNode* reverseKGroup(ListNode* head, int k) {
        ListNode* dummyHead = new ListNode(0, head),* s1 = dummyHead,* s2 = s1->next,* e = getTeamEnd(head, k);
        while (e) {
            s1->next = e;
            s1 = s2;
            s2 = e->next;
    ...
    ListNode* getTeamEnd(ListNode* s, int k) {
        while (--k && s) {
            s = s->next;
        }
        return s;
...
public:
    bool isPalindrome(ListNode* h) {
        ...
        ListNode* next = s->next,* t = reverseLinkedList(next);
        s->next = nullptr;
        ListNode* l = h,* r = t;
        bool res = true;
        while (l && r) {
...
public:
    ListNode* detectCycle(ListNode* h) {
        ...
        ListNode* s = h->next,* f = h->next->next;
        while (s != f) {
...
public:
    ...
    ListNode* sortList(ListNode* h) {
        ...
        ListNode* dummyHead = new ListNode(0, h),* prevEnd,* l1,* r1,* l2,* r2,* next;
        for (int step = 1; step < n; step <<= 1) {
            prevEnd = dummyHead;
            next = dummyHead->next;
            // 迭代版本的归并排序
            while (next) {
                l1 = next, r1 = getGroupEnd(l1, step), l2 = r1->next;
                if (!l2) {
                    prevEnd->next = l1;
                    break;
                }
                r2 = getGroupEnd(l2, step);
                next = r2->next;
    ...
    ListNode* getGroupEnd(ListNode* h, int k) {
        while (--k && h->next) {
            h = h->next;
        }
        return h;
