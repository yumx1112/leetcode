...
int maxSumDivisibleBy7(vector<int>& nums) {
    int n = nums.size();
    vector<vector<int>> dp(n, vector<int>(7, -1));  // nums[0...i] 上余数为 j 的子序列中最大累加和为 dp[i][j]，-1 表示不存在这样的序列
    dp[0][nums[0] % 7] = nums[0];
    for (int i = 1; i < n; ++i) {
        for (int j = 0; j < 7; ++j) {
            dp[i][j] = max(dp[i - 1][j],                               // 不要 nums[i]
                           dp[i - 1][(j - nums[i] % 7 + 7)] + nums[i]  // 要 nums[i]
            );
        }
    }
    return dp[n - 1][0];
...
int main() {
    ...
    for (int i = 0, res1, res2; i < numTests; ++i) {
        nums = generateRandomArray(15, 30);
        res1 = comparator(nums);
        res2 = comparator(nums);
        if (res1 != res2) {
            cout << "Failed!";
            return 0;
        }
