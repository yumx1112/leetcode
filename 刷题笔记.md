...
public:
    int coinChange(vector<int>& coins, int amount) {
        ...
        for (int num = 0, j = 0; j <= amount; j += coins[0], ++num) {
            dp[0][j] = num;
        }
        for (int i = 1; i < n; ++i) {
            for (int j = 0; j <= amount; ++j) {
                ...
                if (coins[i] <= j && dp[i][j - coins[i]] != -1) {
                    dp[i][j] = dp[i][j] != -1 ? min(dp[i][j], dp[i][j - coins[i]] + 1) : dp[i][j - coins[i]] + 1;
                }
            }
        }
        ...
        for (int i = 0; i < n; ++i) {
            for (int j = coins[i]; j <= amount; ++j) {
                if (coins[i] <= j && dp[j - coins[i]] != -1) {
                    dp[j] = dp[j] != -1 ? min(dp[j], dp[j - coins[i]] + 1) : dp[j - coins[i]] + 1;
                }
            }
        }
        return dp[amount];
    ...
    int numSquares(int sum) {
        ...
        for (int i = 1; i <= n; ++i) {
            ...
            for (int j = square; j <= sum; ++j) {
                if (dp[j - square] != -1) {
                    dp[j] = dp[j] != -1 ? min(dp[j], dp[j - square] + 1) : dp[j - square] + 1;
                }
            }
        }
        return dp[sum];
...
public:
    ...
    bool backtrack(string& s, vector<string>& wordDict, int i) {
        ...
        int wordSize, startIdx;
        for (string& word: wordDict) {
            wordSize = word.size();
            startIdx = i - wordSize;
            if (startIdx >= 0 && s.substr(startIdx, wordSize) == word && backtrack(s, wordDict, startIdx)) {
                return true;
            }
        }
        return false;
...
public:
    bool wordBreak(string s, vector<string>& wordDict) {
        ...
        vector<bool> dp(n + 1);
        dp[0] = true;
        for (int i = 1, wordSize, startIdx; i <= n; ++i) {
            for (string& word: wordDict) {
                wordSize = word.size();
                startIdx = i - wordSize;
                if (startIdx >= 0 && s.substr(startIdx, wordSize) == word && dp[startIdx]) {
                    dp[i] = true;
                    break;
                }
            }
...
public:
    int rob(vector<int>& nums) {
        ...
        for (int i = 2, curr; i < n; ++i) {
            curr = max(prev2, max(nums[i], prev1 + nums[i]));
            prev1 = prev2;
            prev2 = curr;
        }
        return prev2;
