    ...
    int coinChange(vector<int>& coins, int amount) {
        ...
        for (int i = n - 1; i >= 0; --i) {
            for (int j = 0; j <= amount; ++j) {
                for (int currNum = 0; j - currNum * coins[i] >= 0; ++currNum) {
                    if (dp[i + 1][j - currNum * coins[i]] != -1) {
                        dp[i][j] = dp[i][j] == -1 ? dp[i + 1][j - currNum * coins[i]] + currNum : min(dp[i][j], dp[i + 1][j - currNum * coins[i]] + currNum);
                    }
                }
            }
        }
