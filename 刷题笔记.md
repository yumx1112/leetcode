...
1. `str1[i]` 和 `str2[j]` 都不是最长公共子序列的一部分，这时 `dp[i][j] = dp[i - 1][j - 1]`；
2. `str1[i]` 是最长公共子序列的一部分，这时 `dp[i][j] = dp[i][j - 1]`；
3. `str2[j]` 是最长公共子序列的一部分，这时 `dp[i][j] = dp[i - 1][j]`；
4. `str1[i]` 和 `str2[j]` 都是最长公共子序列的一部分（要求 `str1[i] == str2[j]`），这时 `dp[i][j] = dp[i - 1][j - 1] + 1`；

以上 4 种可能性取最大值就是 `dp[i][j]` 的值。
