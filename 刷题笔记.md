...
时间复杂度：子序列个数 $O\left(2^n\right)$，拼出每一个子序列的时间复杂度为 $O\left(n\right)$，总体时间复杂度为 $O\left(n2^n\right)$
...
const int MAXN = 1e6;  // 1e5+2 不能通过所有测试用例，说明有的测试用例的数据量比题目给的数据量更大
...
```c++
const int MAXN = 3e6 + 2; // 试出来的值，MAXN 实际上比题目说的要更大一些
