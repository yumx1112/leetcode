...
不要关注局部位置怎么变，应该设计宏观调度过程。设计一个 `getFrame` 函数，接收左上点和右下点的坐标，打印出一个边框，主函数不断调用 `getFrame` 函数即可。
...
public:
    vector<int> spiralOrder(vector<vector<int>>& matrix) {
        ...
        vector<int> res(M * N);
        while (a <= c && b <= d) {
            getFrame(matrix, a++, b++, c--, d--, index, res);
        }

        return res;
    }

    void getFrame(vector<vector<int>>& matrix, int a, int b, int c, int d, int& index, vector<int>& res) {
        if (a == c) {
            for (int j = b; j <= d; j++) {
                res[index++] = matrix[a][j];
            }
        } else if (b == d) {
            for (int i = a; i <= c; i++) {
