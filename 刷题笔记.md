...
bool sortComp(vector<int>& line1, vector<int>& line2) {
    return line1[0] < line2[0];
}

struct PQComp {
    bool operator()(const vector<int>& line1, const vector<int>& line2) const {
        return line1[1] > line2[1];
    }
};

int getMaxCover() {
    sort(lines.begin(), lines.begin() + n, sortComp);
...
bool sortComp(Line& line1, Line& line2) {
...
void heapify(int idx) {
    int l = (idx << 1) + 1, bestIdx;
    while (l < heapSize) {
        bestIdx = l + 1 < heapSize && heap[l + 1] < heap[l] ? l + 1 : l;
...
void heapInsert(int x) {
    heap[heapSize++] = x;
    int idx = heapSize - 1, fatherIdx = (idx - 1) / 2;
    while (heap[idx] < heap[fatherIdx]) {
        swap(heap[idx], heap[fatherIdx]);
        idx = fatherIdx;
        fatherIdx = (idx - 1) / 2;
    }
}
...
int getMaxCover() {
    sort(lines, lines + n, sortComp);
    heapSize = 0;
    int res = 0;
    for (int i = 0; i < n; ++i) {
        while (heapSize && heap[0] <= lines[i].l) {
            heapPop();
        }
        heapInsert(lines[i].r);
