...
class Solution {
private:
    void f(vector<int>& nums, int i, int target, vector<int>& path, vector<vector<int>>& res) {
        if (i == nums.size()) {
            if (target == 0) {
                res.push_back(path);
            }
        ...
        if (target >= nums[i]) {
            ...
            f(nums, i, target - nums[i], path, res);
            path.pop_back();
        }
        f(nums, i + 1, target, path, res);
    }
